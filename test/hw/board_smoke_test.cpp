 ...
 * The producer drives nmi each cycle; the consumer records what it sees on `in`.
 * Because every device reads the committed bus, the consumer must observe the
 * PREVIOUS cycle's value, never the producer's same-cycle drive — regardless of
 * board order. A naive single ordered pass would fail this. */
struct Producer {
  bool level;
};
...
void producer_save(const void* self, void* buf) {
  *static_cast<uint8_t*>(buf) = static_cast<const Producer*>(self)->level ? 1 : 0;
}
...
Device producer_device(Producer* storage) {
  return Device{storage,             "producer",    producer_tick, producer_reset,
                producer_state_size, producer_save, producer_load};
}
...
Device consumer_device(Consumer* storage) {
  return Device{storage,             "consumer",    consumer_tick, consumer_reset,
                consumer_state_size, consumer_save, consumer_load};
}
...
void irqsource_reset(void* self) { static_cast<IrqSource*>(self)->armed = false; }
size_t irqsource_state_size(const void*) { return 1; }
void irqsource_save(const void* self, void* buf) {
  *static_cast<uint8_t*>(buf) = static_cast<const IrqSource*>(self)->armed ? 1 : 0;
}
...
Device irqsource_device(IrqSource* storage) {
  return Device{storage,              "irq-src",      irqsource_tick, irqsource_reset,
                irqsource_state_size, irqsource_save, irqsource_load};
}
...
TEST(BoardFoundation, MemoryReadWrite) {
  ...
  seed_cpu(&board, r);
  board_tick(&board);
  EXPECT_EQ(board.bus.cpu.data, 0x00) << "data bus is assigned, not OR-ed with the pull-up";
...
TEST(BoardFoundation, TwoPhaseReadsCommittedNotSameCycle) {
  ...
  // cycle-0 drive (true) — proving it read `in`, not `out`. Then it lags by one:
  // producer toggles true,false,true,false → consumer sees 0,1,0,1.
  EXPECT_EQ(a[0], 0) << "consumer did not see the producer's same-cycle drive";
  EXPECT_EQ((std::array<uint8_t, 4>{0, 1, 0, 1}), a) << "consumer lags the producer by one cycle";
...
TEST(BoardFoundation, WiredOrInterrupt) {
  ...
  board_tick(&board);
  EXPECT_FALSE(board.bus.cpu.irq) << "irq rests deasserted when no source is armed";
  ...
  src2->armed = false;
  board_tick(&board);
  EXPECT_FALSE(board.bus.cpu.irq) << "irq clears the cycle after all sources disarm";
}
