/* z80_test.cpp — Round Z80-a: the cycle-stepped Z80 device on a board,
 * validating final register/flag state and exact T-state totals for the 8-bit
 * register/ immediate group. (Per-T bus-trace vs FUSE is offset by the bus
 * model's one-hop latency — see hw-spec §2 — so we check totals + final state
 * here.)
 */
...
/* Free-running WAIT generator: clk.cpu always on, cpu.wait asserted on
 * alternate master cycles (NOT reactive to mreq — WAIT on the real GA is
 * free-running). */
...
Device waitclock_device(uint8_t* ctr) {
  return Device{ctr,      "waitclk", waitclk_tick, clk_reset,
                clk_size, clk_save,  clk_load};
}

/* Realistic ÷4 clock: clk.cpu enabled 1 master cycle in 4 (the Gate Array's 4
 * MHz enable within the 16 MHz fabric). Exercises the Z80's drive-and-hold path
 * — the CPU must keep a memory transaction asserted across the 3 idle cycles.
 */
...
Device div4_device(uint8_t* ctr) {
  return Device{ctr,      "div4",   div4_tick, clk_reset,
                clk_size, clk_save, clk_load};
}
...
void intline_save(const void* s, void* b) {
  std::memcpy(b, s, sizeof(IntLine));
}
void intline_load(void* s, const void* b) {
  std::memcpy(s, b, sizeof(IntLine));
}
...
Z80Regs run_core(const std::vector<uint8_t>& program, bool irq, int nmi_at) {
  ...
  rig.ram =
      std::make_unique<Ram>();  // value-initialized → cells already zeroed
  size_t i = 0;
  ...
  for (int tick = 0; tick < 4000; ++tick) {
    ...
    z80_peek(&rig.zdev, &r);
    // Stop at HALT — but if an NMI edge is scheduled, run well past it first so
    // a HALT meant to be *woken* by that NMI doesn't end the run prematurely.
...
Z80Regs run(const std::vector<uint8_t>& program) {
  return run_core(program, false, -1);
}
...
Z80Regs run_int(const std::vector<uint8_t>& program, bool irq,
                int nmi_at = -1) {
...
TEST(Z80a, NopTimingAndHalt) {
  ...
  EXPECT_EQ(r.pc, 0x0001)
      << "PC points AT the HALT opcode while halted (matches FUSE)";
...
TEST(Z80a, AddSetsCarryZeroHalf) {
  Z80Regs r =
      run({0x3E, 0xFF, 0xC6, 0x01, 0x76});  // LD A,0xFF ; ADD A,1 ; HALT
  EXPECT_EQ(hi(r.af), 0x00) << "0xFF + 1 = 0x00";
  EXPECT_EQ(lo(r.af), static_cast<uint8_t>(ZF | HF | CF))
      << "Z,H,C set; S,N,P clear";
}
...
TEST(Z80a, CompareDoesNotChangeA) {
  Z80Regs r =
      run({0x3E, 0x42, 0xFE, 0x42, 0x76});  // LD A,0x42 ; CP 0x42 ; HALT
...
TEST(Z80a, IncOverflowAndDecHalf) {
  ...
  Z80Regs ri =
      run({0xAF, 0x3E, 0x7F, 0x3C, 0x76});  // XOR A ; LD A,0x7F ; INC A ; HALT
  EXPECT_EQ(hi(ri.af), 0x80);
  EXPECT_EQ(lo(ri.af), static_cast<uint8_t>(SF | HF | PF))
      << "INC 0x7F→0x80: S,H,P/V; N,C clear";

  Z80Regs rd =
      run({0xAF, 0x06, 0x00, 0x05, 0x76});  // XOR A ; LD B,0 ; DEC B ; HALT
  EXPECT_EQ(hi(rd.bc), 0xFF);
  EXPECT_EQ(lo(rd.af), static_cast<uint8_t>(SF | YF | HF | XF | NF))
      << "DEC 0→0xFF flags";
}
...
TEST(Z80a, XorAClearsToKnownState) {
  ...
  EXPECT_EQ(lo(r.af), static_cast<uint8_t>(ZF | PF))
      << "Z + even parity; H,N,C clear";
}
...
TEST(Z80a, EveryDefinedOpcodeIsDecoded) {
  ...
  EXPECT_FALSE(run({0xFD, 0x21, 0x00, 0x00, 0x76}).unimplemented)
      << "FD: LD IY,nn";
  EXPECT_FALSE(run({0xDD, 0xCB, 0x00, 0x46, 0x76}).unimplemented)
      << "DDCB: BIT 0,(IX+0)";
}

TEST(Z80a, QClearedByNonFlagInstruction) {
  // Rak's Q must commit to 0 after an instruction that writes no flags (else
  // the NMOS SCF/CCF formula breaks in Z80-e). Reset F is 0xFF, so the old init
  // bug (qq = F) would leave Q = 0xFF. Run NOPs with no HALT so nothing
  // re-commits Q. (HALT itself is a non-flag op and would also force Q=0,
  // masking the bug.)
...
TEST(Z80a, RefreshIncrements) {
  ...
  EXPECT_EQ(r.r & 0x7F, 4u)
      << "R bumped once per M1 (4 instructions incl. HALT)";
}
...
TEST(Z80b, StoreImmediateAndLoadFromHL) {
  ...
  EXPECT_EQ(r.tstates, 7u + 7u + 10u + 7u + 4u)
      << "7+7 (LD r,n) + 10 (LD (HL),n) + 7 (LD A,(HL)) + 4";
}

TEST(Z80b, StoreRegisterToHL) {
  // LD H,0 ; LD L,0x40 ; LD A,0x99 ; LD (HL),A ; LD A,0 ; LD A,(HL) ; HALT
  Z80Regs r =
      run({0x26, 0x00, 0x2E, 0x40, 0x3E, 0x99, 0x77, 0x3E, 0x00, 0x7E, 0x76});
...
TEST(Z80b, IncHLReadModifyWrite) {
  ...
  EXPECT_EQ(r.tstates, 7u + 7u + 10u + 11u + 7u + 4u)
      << "INC (HL) is 11T (4+3+1+3)";
}
...
TEST(Z80b, WaitStallExtendsMemoryCyclesNotState) {
  ...
  auto ram =
      std::make_unique<Ram>();  // value-initialized → cells already zeroed
  const uint8_t prog[] = {0x26, 0x00, 0x2E, 0x40,
                          0x36, 0xAB, 0x7E, 0x76};  // store + load
  ...
  EXPECT_EQ(hi(r.af), 0xAB)
      << "value correct despite WAIT stalls (one latch, right byte)";
  EXPECT_GT(r.tstates, 35u)
      << "WAIT inserted Tw cycles → more than the 35T no-wait total";
}

TEST(Z80b, StoreAToDESetsMemptr) {
  // LD DE,0x0041 ; LD A,0x88 ; LD (DE),A ; LD HL,0x0041 ; LD A,0 ; LD A,(HL) ;
  // HALT
  Z80Regs r = run({0x11, 0x41, 0x00, 0x3E, 0x88, 0x12, 0x21, 0x41, 0x00, 0x3E,
                   0x00, 0x7E, 0x76});
...
TEST(Z80c, RlcRegisterAndDoubleM1Refresh) {
  ...
  EXPECT_EQ(r.r & 0x7F, 4u)
      << "LD(1) + RLC(2, double-M1) + HALT(1) = 4 refreshes";
}
...
TEST(Z80c, SetAndResRegister) {
  ...
  Z80Regs rr =
      run({0x06, 0xFF, 0xCB, 0x98, 0x76});  // LD B,0xFF ; RES 3,B ; HALT
...
TEST(Z80c, RlcHLReadModifyWrite) {
  ...
  EXPECT_EQ(r.tstates, 10u + 10u + 15u + 7u + 4u)
      << "RLC (HL) = 15T (4+4+3+1+3)";
}
...
TEST(Z80c, BitFromHLTiming) {
  ...
  EXPECT_EQ(r.tstates, 10u + 10u + 12u + 4u)
      << "BIT n,(HL) = 12T (4+4+3+1, no write)";
}

// ---- Round Z80-c (2/2): the ED prefix (NEG, 16-bit ADC/SBC, LD A,I/R, IM,
// RRD/RLD) ----
...
TEST(Z80c, Rrd) {
  ...
  Z80Regs r =
      run({0x21, 0x40, 0x00, 0x36, 0x12, 0x3E, 0x34, 0xED, 0x67, 0x46, 0x76});
...
TEST(Z80c, Rld) {
  ...
  Z80Regs r =
      run({0x21, 0x40, 0x00, 0x36, 0x12, 0x3E, 0x34, 0xED, 0x6F, 0x46, 0x76});
...
TEST(Z80c, BitFromHLTakesXYFromMemptr) {
  // Seed WZ via LD A,(BC) (WZ=BC+1=0x2800), then BIT 0,(HL): undoc X/Y come
  // from WZ.hi (0x28), NOT from the tested value.
  Z80Regs r = run({0x06, 0x27, 0x0E, 0xFF,
                   0x0A,  // LD B,0x27; LD C,0xFF; LD A,(BC) → WZ=0x2800
                   0x21, 0x40, 0x00, 0x36, 0x01,  // LD HL,0x40; LD (HL),0x01
                   0xCB, 0x46, 0x76});            // BIT 0,(HL); HALT
  EXPECT_EQ(r.wz, 0x2800u);
  EXPECT_EQ(lo(r.af) & static_cast<uint8_t>(YF | XF), 0x28)
      << "BIT n,(HL): X/Y from WZ.hi";
}
...
TEST(Z80d, ScfCcfCplDaa) {
  ...
  Z80Regs daa =
      run({0xAF, 0x3E, 0x09, 0x3C, 0x27, 0x76});  // XOR A;LD A,9;INC A;DAA
  EXPECT_EQ(hi(daa.af), 0x10) << "0x0A DAA → 0x10 (BCD adjust)";
}

TEST(Z80d, AddHL16AndExAf) {
  Z80Regs r = run({0x21, 0x00, 0x10, 0x01, 0x34, 0x02, 0x09,
                   0x76});  // LD HL,0x1000;LD BC,0x0234;ADD HL,BC
...
TEST(Z80e, JrTakenAndConditionalNotTaken) {
  ...
  EXPECT_EQ(hi(nz.af), 0xFF)
      << "JR NZ not taken (Z set) → fell through to LD A,0xFF";
}
...
TEST(Z80e, CallRetRoundTrip) {
  ...
  EXPECT_EQ(r.tstates, 17u + 7u + 10u + 4u)
      << "CALL 17T + LD 7T + RET 10T + HALT 4T";
...
TEST(Z80e, PushPopAndExSp) {
  ...
  Z80Regs ex =
      run({0x21, 0xAA, 0xAA, 0xE5, 0x21, 0xBB, 0xBB, 0xE3, 0xC1, 0x76});
...
TEST(Z80f, AbsoluteMemoryLoads) {
  ...
  Z80Regs hl = run({0x21, 0x34, 0x12, 0x22, 0x00, 0x40, 0x21, 0x00, 0x00, 0x2A,
                    0x00, 0x40, 0x76});
  ...
  Z80Regs a =
      run({0x3E, 0x77, 0x32, 0x01, 0x40, 0x3E, 0x00, 0x3A, 0x01, 0x40, 0x76});
...
TEST(Z80f, InOutPortIO) {
  ...
  EXPECT_EQ(hi(rt.af), 0x3C)
      << "OUT (n),A then IN A,(n) round-trips through the latch";
  ...
  EXPECT_FALSE(lo(c.af) & ZF)
      << "IN r,(C) sets flags from the value (9 → not zero)";
}
...
TEST(Z80g, Ldir) {
  ...
  Z80Regs r = run({0x21, 0x0F, 0x00, 0x11, 0x00, 0x40, 0x01, 0x03, 0x00, 0xED,
                   0xB0, 0x3A, 0x02, 0x40, 0x76, 0xAA, 0xBB, 0xCC});
...
TEST(Z80g, IniAndOuti) {
  ...
  Z80Regs ini = run(
      {0x21, 0x00, 0x40, 0x01, 0x42, 0x01, 0xED, 0xA2, 0x3A, 0x00, 0x40, 0x76});
  ...
  Z80Regs outi = run({0x21, 0x0C, 0x00, 0x01, 0x33, 0x01, 0xED, 0xA3, 0xDB,
                      0x33, 0x76, 0x00, 0x77});
  EXPECT_EQ(hi(outi.af), 0x77)
      << "OUTI wrote (HL) to the port; IN reads it back";
}
...
TEST(Z80g, RetiPopsLikeRet) {
  ...
  EXPECT_EQ(r.pc, 0x0003u)
      << "RETI returned to 0x0003 (the HALT it then halts on)";
}
...
TEST(Z80h, LoadIxAndPairOps) {
  ...
  Z80Regs r = run(
      {0xDD, 0x21, 0x34, 0x12, 0xDD, 0x23, 0x01, 0x01, 0x00, 0xDD, 0x09, 0x76});
  EXPECT_EQ(r.ix, 0x1236u)
      << "LD IX,nn (0x1234) + INC IX (0x1235) + ADD IX,BC(1) = 0x1236";
  EXPECT_EQ(r.hl, 0x0000u) << "HL untouched — the prefix retargets IX only";
  // Exact prefixed timing: LD IX,nn 14T + INC IX 10T + LD BC,nn 10T + ADD IX,BC
  // 15T + HALT 4T
  EXPECT_EQ(r.tstates, 14u + 10u + 10u + 15u + 4u)
      << "DD-prefixed timings (each +4 for the prefix M1)";
  // FD 21 LD IY,0xBEEF
...
TEST(Z80h, IxDisplacementLoadStore) {
  ...
  Z80Regs r = run(
      {0xDD, 0x21, 0x00, 0x40, 0xDD, 0x36, 0x02, 0x99, 0xDD, 0x7E, 0x02, 0x76});
  ...
  Z80Regs rb = run({0xDD, 0x21, 0x00, 0x40, 0x06, 0x55, 0xDD, 0x70, 0x01, 0xDD,
                    0x4E, 0x01, 0x76});
  EXPECT_EQ(lo(rb.bc), 0x55)
      << "LD (IX+d),B then LD C,(IX+d): register operand keeps real B/C";
}
...
TEST(Z80h, IxhHalfRegisterAndNegativeDisplacement) {
  ...
  EXPECT_EQ(hi(r.af), 0xABu)
      << "LD IXH,n / LD A,IXH use the index high half, not H";
  EXPECT_EQ(hi(r.hl), 0x00) << "real H is untouched";
  // negative displacement: LD IX,0x4010 ; LD (IX-1),0x7E ; LD A,(IX-1)
  Z80Regs neg = run(
      {0xDD, 0x21, 0x10, 0x40, 0xDD, 0x36, 0xFF, 0x7E, 0xDD, 0x7E, 0xFF, 0x76});
  EXPECT_EQ(hi(neg.af), 0x7E)
      << "displacement is signed: (IX-1) addresses 0x400F";
  EXPECT_EQ(neg.wz, 0x400Fu);
}

TEST(Z80h, DdcbBitSetAndRes) {
  // LD IX,0x4000 ; LD (IX+0),0x00 ; SET 3,(IX+0) ; LD A,(IX+0)
  Z80Regs set = run({0xDD, 0x21, 0x00, 0x40, 0xDD, 0x36, 0x00, 0x00, 0xDD, 0xCB,
                     0x00, 0xDE, 0xDD, 0x7E, 0x00, 0x76});
  EXPECT_EQ(hi(set.af), 0x08) << "SET 3,(IX+0) set bit 3";
  // LD IX,0x4000 ; LD (IX+0),0xFF ; RES 0,(IX+0) ; LD A,(IX+0)
  Z80Regs res = run({0xDD, 0x21, 0x00, 0x40, 0xDD, 0x36, 0x00, 0xFF, 0xDD, 0xCB,
                     0x00, 0x86, 0xDD, 0x7E, 0x00, 0x76});
...
TEST(Z80i, MaskableIm1) {
  ...
  prog[2] = 0xFB;  // EI
  prog[3] = 0x18;
  prog[4] = 0xFE;  // JR -2 (self loop)
  prog[0x38] = 0x3E;
  prog[0x39] = 0x42;  // LD A,0x42
  prog[0x3A] = 0x76;  // HALT
...
TEST(Z80i, Im1AcceptanceTStatesAndRefresh) {
  ...
  //   IM1 (8T, 2 M1/refreshes) ; EI (4T,1) ; NOP (4T,1; ei_delay defers INT
  //   here) ; <NOP slot> — INT accepted at this boundary ; handler@0x38: HALT.
  ...
  prog[0x38] = 0x76;  // HALT
  Z80Regs r = run_int(prog, /*irq=*/true);
  EXPECT_EQ(r.tstates, 33u)
      << "IM1 acknowledge is 13T (a +1 here would mean 34)";
...
TEST(Z80i, MaskableIm2Vectors) {
  ...
  prog[0] = 0x21;
  prog[1] = 0x50;
  prog[2] = 0x00;  // LD HL,0x0050
  prog[3] = 0x22;
  prog[4] = 0xFF;
  prog[5] = 0x90;  // LD (0x90FF),HL
  prog[6] = 0x3E;
  prog[7] = 0x90;  // LD A,0x90
  prog[8] = 0xED;
  prog[9] = 0x47;  // LD I,A  (I=0x90)
  prog[10] = 0xED;
  prog[11] = 0x5E;  // IM 2
  prog[12] = 0xFB;  // EI
  prog[13] = 0x18;
  prog[14] = 0xFE;  // JR -2
  prog[0x50] = 0x3E;
  prog[0x51] = 0x99;  // LD A,0x99
  prog[0x52] = 0x76;  // HALT
  Z80Regs r = run_int(prog, /*irq=*/true);
  EXPECT_EQ(hi(r.af), 0x99)
      << "IM2 read the table at (I<<8|0xFF) and jumped to the handler";
}
...
TEST(Z80i, NmiVectorsTo0066) {
  ...
  prog[0] = 0x18;
  prog[1] = 0xFE;  // JR -2 (self loop)
  prog[0x66] = 0x3E;
  prog[0x67] = 0x77;  // LD A,0x77
  prog[0x68] = 0x76;  // HALT
...
TEST(Z80i, NmiWakesFromHalt) {
  ...
  prog[0] = 0x76;  // HALT
  prog[0x66] = 0x3E;
  prog[0x67] = 0x33;  // LD A,0x33
  prog[0x68] = 0x76;  // HALT
  Z80Regs r = run_int(prog, /*irq=*/false, /*nmi_at=*/30);
  EXPECT_EQ(hi(r.af), 0x33)
      << "NMI woke the CPU out of HALT and ran the handler";
}
...
TEST(Z80j, ConditionalCallAndJumpsTaken) {
  ...
  Z80Regs call =
      run({0x3E, 0x01, 0xB7, 0xC4, 0x08, 0x00, 0x76, 0x00, 0x3E, 0x42, 0xC9});
...
TEST(Z80j, LdirUndocumentedXYFlags) {
  // n = A + transferred byte = 0x01 + 0x05 = 0x06 → YF (n bit1) set, XF (n
  // bit3) clear. Source byte 0x05 sits at index 14 = 0x000E, so HL must be
  // 0x000E.
  Z80Regs r = run({0x3E, 0x01, 0x21, 0x0E, 0x00, 0x11, 0x00, 0x40, 0x01, 0x01,
                   0x00, 0xED, 0xB0, 0x76, 0x05});
...
TEST(Z80j, ScfUsesPriorInstructionQ) {
  // LD A,0 ; CP 0x28 (writes F incl. YF|XF from operand; A stays 0; Q = that F)
  // ; SCF. SCF X/Y = ((Q ^ F) | A) & 0x28; with Q==F and A==0 the term cancels
  // → X/Y = 0. A regression using Q=0 would instead yield 0x28 — so this pins
  // the Q source.
...
TEST(Z80j, QLatchIsTransparentAcrossPrefix) {
  // Same as above but with a DD prefix before SCF. A DD/FD prefix is part of
  // its own instruction and must NOT reset the Q latch — so DD SCF sees CP's Q
  // just like plain SCF, and X/Y still cancel to 0. (If the prefix wrongly
  // committed Q=0, this would read 0x28.)
...
TEST(Z80j, DdcbCopiesResultIntoRegister) {
  // DD CB 00 00 = RLC (IX+0) with z=0 → undocumented: result also stored into
  // B. RLC 0x81 = 0x03 (carry from bit 7).
  Z80Regs r = run({0xDD, 0x21, 0x00, 0x40, 0xDD, 0x36, 0x00, 0x81, 0xDD, 0xCB,
                   0x00, 0x00, 0x78, 0x76});
  EXPECT_EQ(hi(r.af), 0x03) << "LD A,B reads the DDCB result copied into B";
  EXPECT_EQ(hi(r.bc), 0x03)
      << "DDCB z!=6 stores the result into register z (B)";
  EXPECT_TRUE(lo(r.af) & CF) << "RLC 0x81 sets carry";
}

TEST(Z80j, DecrementingBlockOpAndIY) {
  // LDD: (DE)<-(HL), then HL--, DE--, BC--.
  Z80Regs ldd = run({0x21, 0x0D, 0x00, 0x11, 0x00, 0x40, 0x01, 0x01, 0x00, 0xED,
                     0xA8, 0x76, 0x00, 0xBB});
  ...
  Z80Regs iy = run(
      {0xFD, 0x21, 0x00, 0x40, 0xFD, 0x36, 0x02, 0x99, 0xFD, 0x7E, 0x02, 0x76});
  EXPECT_EQ(hi(iy.af), 0x99) << "LD (IY+2),n then LD A,(IY+2)";
  EXPECT_EQ(iy.wz, 0x4002u);
  // FDCB: RES 0,(IY+0).
  Z80Regs fdcb = run({0xFD, 0x21, 0x00, 0x40, 0xFD, 0x36, 0x00, 0xFF, 0xFD,
                      0xCB, 0x00, 0x86, 0xFD, 0x7E, 0x00, 0x76});
...
TEST(Z80j, BitAndCpUndocumentedSources) {
  ...
  EXPECT_EQ(lo(bit.af) & static_cast<uint8_t>(YF | XF), 0x28)
      << "BIT r X/Y from the register value";
  EXPECT_TRUE(lo(bit.af) & HF);
  // CP 0x28 with A=0: X/Y come from the OPERAND (0x28), not the result (0xD8,
  // bit5=0).
...
TEST(Z80i, Im0VectorsViaBusRst) {
  ...
  prog[0x3A] = 0x76;  // HALT
  Z80Regs r = run_int(prog, /*irq=*/true);
  EXPECT_EQ(hi(r.af), 0x42)
      << "IM0 executed RST 0x38 from the floating-0xFF bus";
}
...
TEST(Z80i, InterruptBlockedBetweenPrefixAndOpcode) {
  ...
  // between a DD prefix and its opcode. IM1 ; EI ; then a tight loop built from
  // a DD-prefixed op so a boundary recurs with index briefly set; the handler
  // runs (proving acceptance still happens) and A is set — the point is it does
  // not hang or mis-accept mid-prefix.
  std::vector<uint8_t> prog(0x3B, 0x00);
  prog[0] = 0xED;
  prog[1] = 0x56;  // IM 1
  prog[2] = 0xFB;  // EI
  prog[3] = 0xDD;
  prog[4] = 0x23;  // INC IX (DD-prefixed)
  prog[5] = 0x18;
  prog[6] = 0xFB;  // JR -5 → back to the DD INC IX
  prog[0x38] = 0x3E;
  prog[0x39] = 0x42;  // LD A,0x42
  prog[0x3A] = 0x76;  // HALT
  Z80Regs r = run_int(prog, /*irq=*/true);
  EXPECT_EQ(hi(r.af), 0x42)
      << "INT accepted at a clean boundary around the DD-prefixed loop";
}

// ============================================================================
// Z80k — drive-and-hold: the CPU must keep a bus transaction asserted across
// the ÷4 clock's idle master cycles (GA integration prerequisite). Runs the Z80
// with a realistic 1-in-4 clock instead of the always-on test clock.
// ============================================================================
...
TEST(Z80k, MemoryReadHoldsAcrossDiv4Clock) {
  // LD HL,0x0005 ; LD A,(HL) ; HALT ; data 0x99 @ 0x0005. Without
  // drive-and-hold the read byte is lost after one master cycle and A would
  // read 0xFF.
  Z80Regs r = run_div4({0x21, 0x05, 0x00, 0x7E, 0x76, 0x99});
  EXPECT_EQ(hi(r.af), 0x99)
      << "memory read survived the ÷4 clock's idle cycles";
  // T-states advance only on the enable, so datasheet timing is preserved:
  EXPECT_EQ(r.tstates, 10u + 7u + 4u)
      << "LD HL,nn(10) + LD A,(HL)(7) + HALT(4)";
}

TEST(Z80k, MemoryWriteHoldsAcrossDiv4Clock) {
  // LD HL,0x0008 ; LD (HL),0x33 ; LD A,(HL) ; HALT — write data must stay
  // driven through the idle cycles for the RAM to latch it; then read it back.
  Z80Regs r = run_div4({0x21, 0x08, 0x00, 0x36, 0x33, 0x7E, 0x76});
  EXPECT_EQ(hi(r.af), 0x33)
      << "write latched and read back across the ÷4 clock";
}
