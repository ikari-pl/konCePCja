...
TEST(AmsdosHeader, CreateAndParse) {
  auto hdr = disk_make_amsdos_header("TEST.BIN", AmsdosFileType::BINARY, 0x4000,
                                     0x4000, 1234);
...
TEST(AmsdosHeader, BasicType) {
  auto hdr = disk_make_amsdos_header("PROG.BAS", AmsdosFileType::BASIC, 0x0170,
                                     0x0000, 500);
  auto info = disk_parse_amsdos_header(hdr);
  EXPECT_TRUE(info.valid);
...
TEST_F(DiskFileEditorTest, ListAfterWrite) {
  ...
  std::string err =
      disk_write_file(&driveA, "TEST.BIN", data, true, 0x4000, 0x4000);
  ASSERT_EQ("", err);
...
TEST_F(DiskFileEditorTest, WriteWithHeaderAndReadBack) {
  ...
  std::string err =
      disk_write_file(&driveA, "HELLO.BIN", data, true, 0x8000, 0x8000);
  ASSERT_EQ("", err);
...
TEST_F(DiskFileEditorTest, WriteInvalidFilename) {
  std::vector<uint8_t> data(100, 0);
  std::string err =
      disk_write_file(&driveA, "TOOLONGNAME.TOOLONG", data, false);
  EXPECT_NE("", err);
}
...
TEST_F(DiskFileEditorTest, SaveLoadRoundTrip) {
  ...
  std::vector<uint8_t> data(256, 0x42);
  ASSERT_EQ("",
            disk_write_file(&driveA, "SAVE.BIN", data, true, 0x1000, 0x2000));
