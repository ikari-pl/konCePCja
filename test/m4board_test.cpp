...
static void send_command(uint16_t cmd, const std::vector<uint8_t>& data = {}) {
  ...
  g_m4board.cmd_buf.push_back(
      static_cast<uint8_t>(data.size() + 2));      // size prefix
...
TEST_F(M4BoardTest, VersionCommand) {
  ...
  EXPECT_NE(std::string(reinterpret_cast<char*>(g_m4board.response + 3))
                .find("konCePCja"),
            std::string::npos);
}
...
TEST_F(M4BoardTest, ReadResponseFormat) {
  ...
  std::vector<uint8_t> read_data = {static_cast<uint8_t>(handle), 0x04, 0x00};
...
TEST_F(M4BoardTest, Read2ResponseFormat) {
  ...
  std::vector<uint8_t> read_data = {static_cast<uint8_t>(handle), 0x02, 0x00};
...
TEST_F(M4BoardTest, FsizeResponseFormat) {
  ...
  uint32_t size = g_m4board.response[3] | (g_m4board.response[4] << 8) |
                  (g_m4board.response[5] << 16) | (g_m4board.response[6] << 24);
  EXPECT_EQ(size, 8u);
...
TEST_F(M4BoardTest, OpenReadCloseRoundTrip) {
  ...
  // Read all bytes
  std::vector<uint8_t> read_cmd = {static_cast<uint8_t>(handle),
                                   static_cast<uint8_t>(test_data.size()),
                                   0x00};
  send_command(C_READ, read_cmd);
  EXPECT_EQ(g_m4board.response[0], 0x00);
  std::string got(reinterpret_cast<char*>(g_m4board.response + 4),
                  test_data.size());
...
TEST_F(M4BoardTest, SaveLoadRoundTrip) {
  ...
  // Read back
  std::vector<uint8_t> read_data = {static_cast<uint8_t>(rh),
...
TEST_F(M4BoardTest, EofDetection) {
  ...
  // Read the full file
  std::vector<uint8_t> read_all = {static_cast<uint8_t>(handle), 0x02, 0x00};
  ...
  std::vector<uint8_t> read_more = {static_cast<uint8_t>(handle), 0x01, 0x00};
...
TEST_F(M4BoardTest, FtellAfterSeek) {
  ...
  // Seek to offset 5
  std::vector<uint8_t> seek_data = {static_cast<uint8_t>(handle), 0x05, 0x00,
                                    0x00, 0x00};
  ...
  EXPECT_EQ(g_m4board.response[0], 0x00);
  uint32_t pos = g_m4board.response[3] | (g_m4board.response[4] << 8) |
                 (g_m4board.response[5] << 16) | (g_m4board.response[6] << 24);
  EXPECT_EQ(pos, 5u);
...
static void create_test_dsk(const std::filesystem::path& path,
  ...
  if (cpc_name.size() >= 11) {
    ...
    for (int i = 0; i < 8; i++)
      dir_entry[1 + i] = static_cast<uint8_t>(cpc_name[i]);
    // Skip the dot at position 8
    int ext_start = (cpc_name[8] == '.') ? 9 : 8;
    for (int i = 0; i < 3 && ext_start + i < static_cast<int>(cpc_name.size());
         i++)
  ...
  size_t copy_len =
      std::min(file_data.size(), static_cast<size_t>(SECTOR_SIZE * 2));
  if (!file_data.empty()) {
...
TEST_F(M4BoardTest, OpenFileInsideDsk) {
  ...
  std::vector<uint8_t> read_cmd = {
      static_cast<uint8_t>(handle), 0x00,
      0x06  // count = 0x0600 (1536 bytes, more than our 5)
  };
  send_command(C_READ, read_cmd);
  EXPECT_EQ(g_m4board.response[0], 0x00);
...
TEST_F(M4BoardTest, NetstatReportsHostNetwork) {
  ...
  size_t i = 3;
  while (i < static_cast<size_t>(g_m4board.response_len) &&
         g_m4board.response[i] != 0)
    i++;
  ...
  EXPECT_TRUE(status == 0 || status == 5)
      << "Expected status 0 (disconnected) or 5 (connected), got "
      << (int)status;
...
TEST_F(M4BoardTest, NetHostIpResolvesHostname) {
  ...
  // response[3]=0 means resolved, response[3]=1 means lookup in progress
  // (failed)
...
TEST_F(M4BoardTest, CopyFileSuccess) {
  ...
  for (char c : std::string("source.txt"))
    data.push_back(static_cast<uint8_t>(c));
  data.push_back(0);
  for (char c : std::string("dest.txt"))
    data.push_back(static_cast<uint8_t>(c));
  data.push_back(0);
...
TEST_F(M4BoardTest, CopyFileMissingSource) {
  std::vector<uint8_t> data;
  for (char c : std::string("nonexistent.txt"))
    data.push_back(static_cast<uint8_t>(c));
  data.push_back(0);
  for (char c : std::string("dest.txt"))
    data.push_back(static_cast<uint8_t>(c));
