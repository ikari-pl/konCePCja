...
class ExprEvalTest : public ::testing::Test {
 protected:
  ...
  void expect_fail(const std::string& expr) {
    ...
    EXPECT_FALSE(ok) << "expected failure for '" << expr << "' but got "
                     << result;
  }
};
...
class Z80AssemblerTest : public ::testing::Test {
 protected:
  ...
  AsmResult check(const std::string& source) { return asm_.check(source); }

  void expect_bytes(word addr, std::initializer_list<byte> expected) {
    int i = 0;
    for (byte b : expected) {
      EXPECT_EQ(ram[addr + i], b)
          << "byte mismatch at $" << std::hex << (addr + i) << ": expected $"
          << (int)b << " got $" << (int)ram[addr + i];
      i++;
    }
  }
};
