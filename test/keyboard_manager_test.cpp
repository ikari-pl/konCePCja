...
static bool is_pressed(std::atomic<byte> keyboard_matrix[], CPCScancode sc) {
  ...
  return (keyboard_matrix[line].load(std::memory_order_relaxed) &
          bit_values[bit]) == 0;
}
...
class KeyboardManagerTest : public ::testing::Test {
 protected:
  ...
  void setMode(KeyboardSupportMode mode) { CPC.keyboard_support_mode = mode; }
...
TEST_F(KeyboardManagerTest, Buffered_UpdateWithoutScanDoesNotRelease) {
  ...
  EXPECT_TRUE(
      is_pressed(matrix, KEY_A));  // Still pressed regardless of frame count
}

TEST_F(KeyboardManagerTest, Buffered_KeyupAfterScanReleasesImmediately) {
  setMode(KeyboardSupportMode::BufferedUntilRead);

  // Press a key, scan its line, then release -- the key_needs_scan flag is
  // cleared
  km.handle_keydown(KEY_A, matrix);
  km.notify_scanned(scancode_line(KEY_A));
...
TEST_F(KeyboardManagerTest, Update_NoPendingReleasesIsNoop) {
  ...
  km.update(matrix, 0);
  for (int i = 0; i < 16; i++) {
    EXPECT_EQ(matrix[i], expected[i])
        << "Line " << i << " changed unexpectedly";
  }
}
...
TEST_F(KeyboardManagerTest, Buffered_KeyNeedsScanErasedAfterUpdate) {
  ...
  // Should be buffered (still pressed) because handle_keydown set
  // needs_scan=true
  EXPECT_TRUE(is_pressed(matrix, KEY_A));
...
TEST_F(KeyboardManagerTest, InvalidScancode0xFF_IsIgnored) {
  ...
  for (int i = 0; i < 16; i++) {
    EXPECT_EQ(matrix[i], 0xFF)
        << "Line " << i << " was modified by 0xFF scancode";
  }
}
...
TEST_F(KeyboardManagerTest, Direct_ReleaseModifiersFalse) {
  ...
  // When release_modifiers is false, SHIFT/CTRL should not be explicitly
  // released Press a key (applyKeypressDirect will set SHIFT/CTRL state based
  // on scancode)
  km.handle_keydown(KEY_A, matrix);
...
TEST_F(KeyboardManagerTest, KeydownOnlyAffectsTargetBit) {
  ...
  for (int i = 0; i < 16; i++) {
    if (i == key_line) {
      ...
      EXPECT_EQ(matrix[i],
                static_cast<byte>(0xFF & ~bit_values[scancode_bit(KEY_A)]));
    } else {
      EXPECT_EQ(matrix[i], 0xFF)
          << "Line " << i << " was unexpectedly modified";
    }
  }
}
