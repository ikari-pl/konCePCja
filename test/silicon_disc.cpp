...
class SiliconDiscTest : public ::testing::Test {
 protected:
  void SetUp() override { sd_ = SiliconDisc(); }
  void TearDown() override { silicon_disc_free(sd_); }
  SiliconDisc sd_;
};
...
TEST_F(SiliconDiscTest, SaveAndLoadRoundTrip) {
  ...
  std::string path =
      (std::filesystem::temp_directory_path() / "test_sdisc.ksdx").string();
...
TEST_F(SiliconDiscTest, LoadRejectsBadHeader) {
  std::string path =
      (std::filesystem::temp_directory_path() / "test_sdisc_bad.ksdx").string();
...
TEST_F(SiliconDiscTest, SaveFailsWhenNotAllocated) {
  std::string path =
      (std::filesystem::temp_directory_path() / "test_sdisc_null.ksdx")
          .string();
