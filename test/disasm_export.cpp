...
TEST_F(DisasmExportDataTest, BytesDataAreaFormatsAsDb) {
  ...
  std::string result =
      mgr_.format_at(0x4000, mem_ + 0x4000, sizeof(mem_) - 0x4000, &consumed);
...
TEST_F(DisasmExportDataTest, WordsDataAreaFormatsAsDw) {
  ...
  int consumed = 0;
  std::string result =
      mgr_.format_at(0x5000, mem_ + 0x5000, sizeof(mem_) - 0x5000, &consumed);
  EXPECT_EQ(consumed, 4);
...
TEST_F(DisasmExportDataTest, TextDataAreaFormatsAsDbWithQuotes) {
  ...
  int consumed = 0;
  std::string result =
      mgr_.format_at(0x6000, mem_ + 0x6000, sizeof(mem_) - 0x6000, &consumed);
...
TEST_F(DisasmExportDataTest, TextWithNonPrintableFormatsAsMixed) {
  ...
  int consumed = 0;
  std::string result =
      mgr_.format_at(0x7000, mem_ + 0x7000, sizeof(mem_) - 0x7000, &consumed);
  EXPECT_EQ(consumed, 4);
...
TEST_F(DisasmExportDataTest, FormatAtReturnsEmptyForNonDataArea) {
  int consumed = 0;
  std::string result =
      mgr_.format_at(0x8000, mem_ + 0x8000, sizeof(mem_) - 0x8000, &consumed);
...
TEST_F(DisasmExportDataTest, BytesAreaPartialFormat) {
  ...
  int consumed1 = 0;
  std::string line1 =
      mgr_.format_at(0x4000, mem_ + 0x4000, sizeof(mem_) - 0x4000, &consumed1);
  ...
  int consumed2 = 0;
  std::string line2 =
      mgr_.format_at(0x4008, mem_ + 0x4008, sizeof(mem_) - 0x4008, &consumed2);
