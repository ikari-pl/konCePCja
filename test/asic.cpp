...
class AsicTest : public testing::Test {
 public:
  AsicTest()
      : lockSeq({0x00, 0xff, 0x77, 0xb3, 0x51, 0xa8, 0xd4, 0x62, 0x39, 0x9c,
                 0x46, 0x2b, 0x15, 0x8a, 0xcd}) {
    asic.locked = true;
    asic.lockSeqPos = 0;
  }
...
// In all cases, the lock sequence is the same with the last byte having any
// value except the one it has in the unlock sequence All cartridges tested
// unlock with: FF 00 FF 77 B3 51 A8 D4 62 39 9C 46 2B 15 8A CD EE Klax
// alternates between: FF 00 FF 77 B3 51 A8 D4 62 39 9C 46 2B 15 8A CD EE And:
// FF 00 FF 77 B3 51 A8 D4 62 39 9C 46 2B 15 8A CD FF
// But making the latter unlock the asic make the game bug (loop on welcome
// screen)
...
TEST_F(AsicTest, UnlockWithRepetitionOfFirstTwoBytesOfSequence) {
  ...
  // (Crazy Cars II also but it checks that the unlock is properly done and
  // retries)
  asic_poke_lock_sequence(0x00);
