...
#table(
  ...
  [#cfg-key[vsync]], [1 = VSYNC on (default), 0 = off. Keep 1 on a variable-refresh-rate (VRR) display; 0 is only for the remote-desktop present stall],
)
...
#table(
  ...
  stroke: 0.4pt + rule-grey, inset: 4.5pt,
  [*Key*], [*Meaning*],
  [#cfg-key[m4board]], [Enable the M4 Board (0 = off, the default; 1 = on --- its ROM then owns the disc vectors until `|DISC`)],
