...
enum class SearchMode : std::uint8_t { HEX, TEXT, ASM };

struct SearchResult {
  uint16_t address;
  std::vector<uint8_t> matched_bytes;
  std::string
      context;  // disassembled instruction for ASM mode, hex dump otherwise
};
...
struct PatternElement {
  enum Kind : std::uint8_t { LITERAL, ANY_ONE, ANY_MANY };
  Kind kind;
