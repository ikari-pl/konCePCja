...
void asic_reset() {
  ...
  for (int i = 0; i < 16; i++) {
    asic.sprites_x[i] = asic.sprites_y[i] = asic.sprites_mag_x[i] =
...
void asic_poke_lock_sequence(byte val) {
  static constexpr byte lockSeq[] = {0x00, 0x00, 0xff, 0x77, 0xb3, 0x51,
                                     0xa8, 0xd4, 0x62, 0x39, 0x9c, 0x46,
                                     0x2b, 0x15, 0x8a, 0xcd};
  static constexpr int lockSeqLength = sizeof(lockSeq) / sizeof(lockSeq[0]);
  LOG_DEBUG("ASIC poked with val=" << std::hex << static_cast<int>(val)
                                   << std::dec
                                   << " lockPos = " << asic.lockSeqPos);
  ...
  if (asic.lockSeqPos == 0) {
    ...
    if (asic.lockSeqPos < lockSeqLength) {
      if (val == lockSeq[asic.lockSeqPos]) {
        ...
        asic.lockSeqPos++;
        // If the lock sequence is matched except for the last byte, it means
        // lock
        if (asic.lockSeqPos == lockSeqLength) {
      ...
      // Full sequence matched and an additional value was written, it means
      // unlock
      if (asic.lockSeqPos == lockSeqLength) {
...
void asic_dma_cycle() {
  ...
  // Read one 16bits instruction for each enabled channel X at each scan line
  // and execute it. More precisely: after leading edge of HSYNC, one dead cycle
  // followed by a fetch cycle for each active channel (enabled and not
  // executing a pause) followed by an execution cycle for each active channel.
  // All instructions last 1 cycle except LOAD that lasts 8 (up to 10 if CPU is
  // also accessing the PSG).
  //  - LOAD R,DD will write DD to PSG.RegisterAY.Index[R] - The ASIC should
  //  actually be blocking the CPU if it tries to access the PSG, to determine
  //  if it's important to emulate.
  //  - PAUSE N set a counter to wait N*(chX.prescaler+1) cycles
  //  - REPEAT NNN keep address of loop start (next instruction) and a counter
  //  of loops
  //  - NOP does nothing
  //  - LOOP jump to address of loop stat if counter of loops is >0 and
  //  decrement it (yes, code is actually executed NNN+1 times)
  //  - INT generates an interruption for chX by setting chX.interrupt to true
  //  (code for CPU to detect it must also be done !)
  //  - STOP set chX.enabled to false ? (still increment address for when
  //  processing will restart) The last 4 can be OR-ed to be combined
  ...
  for (int c = 0; c < NB_DMA_CHANNELS; c++) {
    ...
    if (opcode == 0) {  // LOAD
      ...
      SetAYRegister(R, val);
      LOG_DEBUG("DMA [" << c << "] load " << std::hex << static_cast<int>(val)
                        << " in register " << R << std::dec);
    } else {
      if (opcode & 0x01) {  // PAUSE
        channel.pause_ticks = (instruction) & 0x0FFF;
        channel.tick_cycles = 0;
        LOG_DEBUG("DMA [" << c << "] pause " << channel.pause_ticks << "*"
                          << static_cast<int>(channel.prescaler) << " cycles");
      }
    ...
    // TODO: cleaner way to modify back the register value here ...
    {
      ...
      g_memory_bus.write_raw(addr,
                             static_cast<byte>(channel.source_address & 0xFF));
      addr++;
      g_memory_bus.write_raw(
          addr, static_cast<byte>((channel.source_address & 0xFF00) >> 8));
      /* Useless ?
      addr++;
  ...
  // TODO: ... and here
  // Run RAM test of testplus.cpr when touching this (this is not a guarantee
  // that this is correct but at least it's a guarantee that it's less wrong !
  // cf issue #40)
  if (dcsr_changed) {
...
void asic_set_palette() {
  for (int colour = 0; colour < 32; colour++) {
    ...
    // TODO: deduplicate with code in video_set_palette + make it work in
    // monochrome
    dword red = static_cast<dword>(asic_colours[colour].r *
                                   (CPC.scr_intensity / 10.0) * 255);
    if (red > 255) {  // limit to the maximum
      red = 255;
    }
    dword green = static_cast<dword>(asic_colours[colour].g *
                                     (CPC.scr_intensity / 10.0) * 255);
    if (green > 255) {
      green = 255;
    }
    dword blue = static_cast<dword>(asic_colours[colour].b *
                                    (CPC.scr_intensity / 10.0) * 255);
    if (blue > 255) {
      blue = 255;
    }
    video_update_palette_entry(colour, static_cast<uint8_t>(red),
                               static_cast<uint8_t>(green),
                               static_cast<uint8_t>(blue));
    // TODO(cpitrat): Confirm whether we should update the mode 2
    // 'anti-aliasing' colour (cf. src/kon_cpc_ja.cpp where
    // GateArray.palette[33] is set).
  }
}
...
bool asic_register_page_write(word addr, byte val) {
  ...
  LOG_DEBUG("ASIC register page write: addr=" << std::hex << addr << ", val="
                                              << static_cast<int>(val)
                                              << std::dec);
  ...
  if (addr >= 0x4000 && addr < 0x5000) {
    ...
    // LOG_DEBUG("Received sprite data for sprite " << id << ": x=" << x << ",
    // y=" << y << ", color=" << static_cast<int>(val));
    return false;
    ...
    switch (type) {
      case 0:
        ...
        // LOG_DEBUG("Received sprite X for sprite " << id << " x=" <<
        // asic.sprites_x[id]);
      ...
      case 2:
        ...
        // LOG_DEBUG("Received sprite Y for sprite " << id << " y=" <<
        // asic.sprites_y[id]);
        //  Mirrored in RAM image 4 bytes after
        pbRegisterPage[(addr & 0x3FFF) + 4] = val;
        break;
      ...
      case 4:
        ...
        // LOG_DEBUG("Received sprite magnification for sprite " << id << " mx="
        // << asic.sprites_mag_x[id] << ", my=" << asic.sprites_mag_y[id]);
        //  Write-only: does not affect pbRegisterPage
        return false;
      default:
        LOG_DEBUG("Received sprite operation of unsupported type: "
                  << type << " addr=" << std::hex << addr
    ...
    if ((addr % 2) == 1) {
      ...
      // LOG_DEBUG("Received color operation: color " << colour << " has green =
      // " << green);
      asic_colours[colour].g = green;
      pbRegisterPage[(addr & 0x3FFF)] = (val & 0x0F);
      // TODO: find a cleaner way to do this - this is a copy paste from "Set
      // ink value" in kon_cpc_ja.cpp
    } else {
      double red = static_cast<double>((val & 0xF0) >> 4) / 16;
      double blue = static_cast<double>(val & 0x0F) / 16;
      // LOG_DEBUG("Received color operation: color " << colour << " has red = "
      // << red << " and blue = " << blue);
    ...
    if (addr == 0x6800) {
      LOG_DEBUG("Received programmable raster interrupt scan line: "
                << static_cast<int>(val));
      ...
      LOG_DEBUG("Received address for split: " << std::hex << CRTC.split_addr
                                               << std::dec);
    } else if (addr == 0x6803) {
      ...
      LOG_DEBUG("Received soft scroll control: "
                << static_cast<int>(val) << ": dx=" << asic.hscroll << ", dy="
                << asic.vscroll << ", border=" << asic.extend_border);
      update_skew();
    } else if (addr == 0x6805) {
      LOG_DEBUG(
          "[UNSUPPORTED] Received interrupt vector: " << static_cast<int>(val));
      asic.interrupt_vector = val & 0xF8;
      // TODO: Write this part !!! (Interrupt service part from
    ...
    int c = ((addr & 0xc) >> 2);
    LOG_DEBUG("Received DMA source address: "
              << std::hex << addr << " (channel " << c << ") "
    ...
    LOG_DEBUG("Received DMA control register: "
              << std::hex << static_cast<int>(val) << std::dec);
    for (int c = 0; c < NB_DMA_CHANNELS; c++) {
      asic.dma.ch[c].enabled = (val & (0x1 << c));
      LOG_DEBUG("DMA channel "
                << c << (asic.dma.ch[c].enabled ? " enabled" : " disabled"))
    }
  } else {
    LOG_DEBUG("Received unused write at " << std::hex << addr << " - val: "
                                          << static_cast<int>(val) << std::dec);
  }
  return true;
}
...
void asic_draw_sprites() {
  // TODO: this should be affected by other CRTC.registers (reg2 for borderWidth
  // ?, reg1 for screenWidth ? reg6 for screenHeight ?)
            ...
            if (p) {
              ...
              for (int dx = 0; dx < mx; dx++) {
                for (int dy = 0; dy < my * static_cast<int>(dwXScale); dy++) {
                  putpixel(back_surface, (sx + (x * mx) + dx) * dwXScale / 2,
                           (sy + (y * my)) * dwXScale + dy, pixel);
                }
              }
            }
          }
