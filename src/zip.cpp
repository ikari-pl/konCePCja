...
int dir(t_zip_info* zi) {
  ...
  do {
    ...
    pbPtr =
        pbGPBuffer +
        (256 -
         22);  // pointer to end of central directory (under ideal conditions)
    while (pbPtr != static_cast<byte*>(pbGPBuffer)) {
      if (*reinterpret_cast<dword*>(pbPtr) ==
          0x06054b50) {  // check for end of central directory signature
  ...
  for (n = wCentralDirEntries; n; n--) {
    ...
    dwNextEntry = wFilenameLength + *reinterpret_cast<word*>(pbPtr + 30) +
                  *reinterpret_cast<word*>(pbPtr + 32);
    pbPtr += 46;
    const char* pchThisExtension = zi->extensions.c_str();
    while (*pchThisExtension !=
           '\0') {  // loop for all extensions to be checked
      if (strncasecmp(reinterpret_cast<char*>(pbPtr) + (wFilenameLength - 4),
                      pchThisExtension, 4) == 0) {
...
int extract(const t_zip_info& zi, FILE** pfileOut) {
...
#ifdef WINDOWS
  // Windows version of tmpfile is broken by design as it tries to create the
  // temporary file in the root directory. Use mkstemp-style approach: _mktemp_s
  // + fopen on Windows.
  ...
  *pfileOut = tmpfile();
  ...
  memutils::scope_exit<decltype(closure)> cs(closure);
  if (fseek(pfileIn, dwOffset, SEEK_SET) !=
      0) {  // move file pointer to beginning of data block
    LOG_ERROR("Couldn't read zip file: " << zi.filename);
    fclose(*pfileOut);
    return ERR_FILE_UNZIP_FAILED;
  };
  ...
  dwSize =
      *reinterpret_cast<dword*>(pbGPBuffer + 18);  // length of compressed data
  dwOffset += 30 + *reinterpret_cast<word*>(pbGPBuffer + 26) +
              *reinterpret_cast<word*>(pbGPBuffer + 28);
  if (fseek(pfileIn, dwOffset, SEEK_SET) !=
      0) {  // move file pointer to start of compressed data
    LOG_ERROR("Couldn't read zip file: " << zi.filename);
    fclose(*pfileOut);
    return ERR_FILE_UNZIP_FAILED;
  }
  ...
  do {
    ...
    z.avail_in = fread(pbInputBuffer, 1, z.avail_in,
                       pfileIn);  // load compressed data chunck from ZIP file
    while ((z.avail_in) &&
           (iStatus == Z_OK)) {  // loop until all data has been processed
