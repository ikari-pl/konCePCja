...
namespace {
void write_le32_buf(uint8_t* buf, uint32_t val) {
...
void write_le16(FILE* f, uint16_t val) {
  uint8_t b[2] = {static_cast<uint8_t>(val), static_cast<uint8_t>(val >> 8)};
  fwrite(b, 1, 2, f);
}
}  // namespace
...
bool SessionRecorder::start_recording(const std::string& path,
  ...
  // Write embedded SNA
  if (fwrite(snap_data.data(), 1, snap_data.size(), rec_file_) !=
      snap_data.size()) {
    fclose(rec_file_);
    rec_file_ = nullptr;
    return false;
...
bool SessionRecorder::start_playback(const std::string& path,
                                     std::string& snap_path_out) {
  if (state_ != SessionState::IDLE) return false;

  FilePtr const f(fopen(path.c_str(), "rb"));
  if (!f) return false;
  ...
  if (header[0] != 'K' || header[1] != 'S' || header[2] != 'R' ||
      header[3] != 0x1A) {
    return false;
  }
  ...
  uint32_t const sna_size = read_le32(header + 8);
  uint32_t const evt_count = read_le32(header + 12);
...
bool SessionRecorder::stop_playback() {
  if (state_ != SessionState::PLAYING && state_ != SessionState::IDLE)
    return false;
  events_.clear();
  play_pos_ = 0;
  state_ = SessionState::IDLE;
