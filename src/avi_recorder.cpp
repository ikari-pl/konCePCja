...
void AviRecorder::write_fourcc(const char* cc, FILE* f) { fwrite(cc, 1, 4, f); }
...
uint32_t AviRecorder::stop() {
  ...
  uint32_t const result = video_frames_;
  video_frames_ = 0;
  audio_bytes_ = 0;
  total_bytes_ = 0;
  path_.clear();
...
void AviRecorder::capture_video_frame(const uint8_t* pixels, int width,
  ...
  uint32_t const chunk_offset =
      static_cast<uint32_t>(ftell(file_) - movi_start_ - 4);

  // Write video chunk: "00dc" + size + data (+ pad byte if odd size)
  write_le_u32(FOURCC_00dc, file_);
  uint32_t const data_size = static_cast<uint32_t>(jpeg_data.size());
  ...
  index_entries_.push_back(
      {FOURCC_00dc, AVIIF_KEYFRAME, chunk_offset, data_size});
  video_frames_++;
...
void AviRecorder::capture_audio_samples(const int16_t* samples, size_t count) {
  ...
  uint32_t const byte_count = static_cast<uint32_t>(count * sizeof(int16_t));
  uint32_t const chunk_offset =
      static_cast<uint32_t>(ftell(file_) - movi_start_ - 4);
...
void AviRecorder::write_headers() {
  ...
  uint32_t const hdrl_start = static_cast<uint32_t>(ftell(file_));
  write_fourcc("hdrl", file_);

  // Main AVI header (avih) - 56 bytes
  write_fourcc("avih", file_);
  write_le_u32(56, file_);  // avih chunk size
  write_le_u32(1000000 / CPC_FPS,
               file_);                 // dwMicroSecPerFrame (20000 for 50fps)
  ...
  uint32_t const vstrl_start = static_cast<uint32_t>(ftell(file_));
  ...
  write_le_u32(static_cast<uint32_t>(width_ * height_ * 3),
               file_);     // biSizeImage
  ...
  uint32_t const vstrl_end = static_cast<uint32_t>(ftell(file_));
  ...
  write_fourcc("LIST", file_);
  write_le_u32(0, file_);  // placeholder for strl size
  uint32_t const astrl_start = static_cast<uint32_t>(ftell(file_));
  write_fourcc("strl", file_);

  // strh (stream header) - 56 bytes
  uint16_t const block_align =
      static_cast<uint16_t>(channels_ * (bits_per_sample_ / 8));
  uint32_t const byte_rate = sample_rate_ * block_align;
  ...
  uint32_t const astrl_end = static_cast<uint32_t>(ftell(file_));
  fseek(file_, static_cast<long>(astrl_start - 4), SEEK_SET);
  write_le_u32(astrl_end - astrl_start, file_);
  fseek(file_, 0, SEEK_END);

  // Patch hdrl LIST size
  uint32_t const hdrl_end = static_cast<uint32_t>(ftell(file_));
...
void AviRecorder::finalize() {
  ...
  long const movi_end = ftell(file_);

  // Patch movi LIST size
  uint32_t const movi_size = static_cast<uint32_t>(movi_end) - movi_start_;
...
void AviRecorder::write_idx1() {
  ...
  uint32_t const idx_size = static_cast<uint32_t>(index_entries_.size() * 16);
...
void AviRecorder::patch_sizes() {
  long const file_end = ftell(file_);

  // Patch RIFF file size (total file size minus 8 bytes for RIFF header)
  uint32_t const riff_size = static_cast<uint32_t>(file_end - 8);
  ...
  uint16_t const block_align =
      static_cast<uint16_t>(channels_ * (bits_per_sample_ / 8));
  uint32_t const audio_samples =
      (block_align > 0) ? (audio_bytes_ / block_align) : 0;
...
#ifdef HAS_LIBJPEG
// Suppress old-style-cast warnings from libjpeg macros (jpeg_create_compress,
// etc.)
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wold-style-cast"

std::vector<uint8_t> AviRecorder::compress_jpeg(const uint8_t* rgba, int w,
