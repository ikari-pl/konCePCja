...
void TraceRecorder::record(uint16_t pc, uint8_t a, uint8_t f, uint16_t bc,
                           uint16_t de, uint16_t hl, uint16_t sp) {
...
int TraceRecorder::entry_count() const { return count; }

namespace {
void format_entry(const TraceEntry& e, char* buf, size_t bufsz) {
  char opcodes[16] = {};
  int pos = 0;
  ...
  snprintf(buf, bufsz,
           "%04X %-8s A=%02X F=%02X BC=%04X DE=%04X HL=%04X SP=%04X", e.pc,
           opcodes, e.a, e.f, e.bc, e.de, e.hl, e.sp);
}
}  // namespace
...
bool TraceRecorder::dump(const std::string& path) const {
  ...
  int const start = (count < static_cast<int>(buffer.size())) ? 0 : head;
  char line[128];
  for (int i = 0; i < count; i++) {
    int const idx = (start + i) % static_cast<int>(buffer.size());
...
std::string TraceRecorder::to_string(int max_lines) const {
  std::string result;
  int const start = (count < static_cast<int>(buffer.size())) ? 0 : head;
  int const n = (max_lines > 0 && max_lines < count) ? max_lines : count;
  // Show the most recent n entries
  int const skip = count - n;
  char line[128];
