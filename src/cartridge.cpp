...
int cartridge_init() {
  pbCartridgeImage.reset(
      new byte[CARTRIDGE_MAX_SIZE]);  // attempt to allocate the general purpose
                                      // buffer
...
int cpr_load(FILE* pfile) {
  ...
  while (offset < totalSize) {
    if (fread(pbTmpBuffer, CPR_CHUNK_HEADER_SIZE, 1, pfile) !=
        1) {  // read chunk header
    ...
    LOG_DEBUG("Chunk '" << chunkId << "' at offset " << offset << " of size "
                        << chunkSize);
    ...
    if (chunkKept > 0) {
      ...
      if (fread(&pbCartridgeImage[cartridgeOffset], chunkKept, 1, pfile) !=
          1) {  // read chunk content
        LOG_DEBUG("Failed reading chunk content");
        return ERR_CPR_INVALID;
      }
      if (chunkKept < CARTRIDGE_PAGE_SIZE) {
        // TODO: use the chunkId to identify the cartridge page to set (cbXX
        // with XX between 00 and 31) This would require intializing the whole
        // to 0 before instead of filling what remains at the end Not sure if
        // there are some CPR with unordered pages but this seems to be allowed
        // in theory
        memset(&pbCartridgeImage[cartridgeOffset + chunkKept], 0,
  ...
  if (cartridgeOffset < CARTRIDGE_MAX_SIZE) {
    memset(&pbCartridgeImage[cartridgeOffset], 0,
           CARTRIDGE_MAX_SIZE - cartridgeOffset);
  }
