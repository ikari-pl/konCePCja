...
bool write_be_u16(uint16_t val, FILE* f) {
  return fputc((val >> 8) & 0xff, f) != EOF && fputc(val & 0xff, f) != EOF;
}

bool write_be_u32(uint32_t val, FILE* f) {
  return fputc((val >> 24) & 0xff, f) != EOF &&
         fputc((val >> 16) & 0xff, f) != EOF &&
         fputc((val >> 8) & 0xff, f) != EOF && fputc(val & 0xff, f) != EOF;
}
...
uint32_t YmRecorder::stop() {
  ...
  recording_ = false;
  uint32_t const count = static_cast<uint32_t>(frames_.size());
...
bool YmRecorder::write_ym5_file() {
  ...
  bool ok = true;
  uint32_t const num_frames = static_cast<uint32_t>(frames_.size());
  ...
  size_t const name_len = strlen(song_name) + 1;
  ...
  // 14 blocks, each block is num_frames bytes (one byte per frame for that
  // register)
