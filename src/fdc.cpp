...
fdc_cmd_table_def fdc_cmd_table[MAX_CMD_COUNT] = {
    /* syntax is:
       command code, number of bytes for command, number of bytes for result,
       direction, pointer to command handler
    */
...
t_track* active_track;  // reference to the currently selected track, of the
                        // active_drive
...
#define LOAD_RESULT_WITH_STATUS                                               \
  ...
  if (FDC.command[CMD_CODE] !=                                                \
      0x42) { /* continue only if not a read track command */                 \
    if ((FDC.result[RES_ST1] & 0x7f) ||                                       \
        (FDC.result[RES_ST2] & 0x7f)) { /* any 'error bits' set? */           \
      FDC.result[RES_ST1] &= 0x7f;      /* mask out End of Cylinder */        \
      if ((FDC.result[RES_ST1] & 0x20) ||                                     \
          (FDC.result[RES_ST2] & 0x20)) {      /* DE and/or DD? */            \
...
int init_status_regs() {
  ...
  if ((active_drive->tracks == 0) ||
      (!FDC.motor)) {  // no DSK in the drive, or drive motor is turned off?
...
t_sector* find_sector(byte* requested_CHRN) {
  ...
  do {
    if (!(memcmp(&active_track->sector[idx].CHRN, requested_CHRN,
                 4))) {  // sector matches requested ID?
      sector = &active_track
                    ->sector[idx];  // return value points to sector information
      if ((sector->flags[0] & 0x20) ||
          (sector->flags[1] & 0x20)) {   // any Data Errors?
        if (active_drive->random_DEs) {  // simulate 'random' DEs?
          FDC.flags |= RNDDE_flag;
        }
      }
      FDC.result[RES_ST2] &=
          ~(0x02 | 0x10);  // remove possible Bad Cylinder + No Cylinder flags
      break;
    }
    ...
    if (idx >=
        active_track
            ->sectors) {  // index beyond number of sectors for this track?
      idx = 0;            // reset index
  ...
  if (loop_count &&
      active_drive->track_hook)              // track looped and hook available?
    active_drive->track_hook(active_drive);  // update flakey data

  active_drive->current_sector =
...
inline void cmd_write() {
  ...
  if (sector) {              // sector found
    ...
    if (FDC.command[CMD_N] == 0) {  // use DTL for length?
      sector_size =
          FDC.command[CMD_DTL];  // size of sector is defined by DTL value
      if (sector_size > 0x80) {
        sector_size = 0x80;  // max DTL value is 128
      }
    } else {
      sector_size =
          128 << FDC.command[CMD_N];  // determine number of bytes from N value
    }
    FDC.buffer_count = sector_size;  // init number of bytes to transfer
    // Note: do not handle writing to weak sectors (would need to write to all
    // of them ?)
    FDC.buffer_ptr = sector->getDataForWrite();  // pointer to sector data
    FDC.buffer_endptr = active_track->data +
                        active_track->size;  // pointer beyond end of track data
...
loop:
  ...
  if (sector) {              // sector found
    FDC.result[RES_ST1] =
        sector->flags[0] & 0x25;  // copy ST1 to result, ignoring unused bits
    FDC.result[RES_ST2] =
        sector->flags[1] & 0x61;  // copy ST2 to result, ignoring unused bits
    if (FDC.command[CMD_CODE] == 0x4c) {  // read deleted data command?
      FDC.result[RES_ST2] ^= 0x40;        // invert Control Mark
    }
    if ((FDC.flags & SKIP_flag) &&
        (FDC.result[RES_ST2] &= 0x40)) {                 // skip sector?
      ...
      if (FDC.result[RES_ST2] &=
          0x40) {  // does the sector have an AM opposite of what we want?
        FDC.command[CMD_EOT] =
            FDC.command[CMD_R];  // execution ends on this sector
      }
      if (FDC.command[CMD_N] == 0) {  // use DTL for length?
        sector_size =
            FDC.command[CMD_DTL];  // size of sector is defined by DTL value
        ...
        sector_size =
            128
            << FDC.command[CMD_N];  // determine number of bytes from N value
      }
      FDC.buffer_count = sector_size;  // init number of bytes to transfer
...
inline void cmd_readtrk() {
  ...
  if (memcmp(&sector->CHRN, &FDC.command[CMD_C],
             4)) {                // sector does not match requested ID?
    FDC.result[RES_ST1] |= 0x04;  // No Data
  }
  ...
  switch (FDC.phase) {
    case CMD_PHASE:                           // in command phase?
      if (FDC.byte_count) {                   // receiving command parameters?
        ...
        for (idx = 0; idx < MAX_CMD_COUNT;
             idx++) {  // loop through all known FDC commands
        ...
        if (idx != MAX_CMD_COUNT) {  // valid command received
          FDC.cmd_length =
              fdc_cmd_table[idx].cmd_length;  // command length in bytes
          FDC.res_length =
              fdc_cmd_table[idx].res_length;  // result length in bytes
          FDC.cmd_direction =
              fdc_cmd_table[idx]
                  .cmd_direction;  // direction is CPU to FDC, or FDC to CPU
          FDC.cmd_handler =
              fdc_cmd_table[idx].cmd_handler;  // pointer to command handler

          FDC.command[FDC.byte_count++] = val;  // copy command code to buffer
          if (FDC.byte_count ==
              FDC.cmd_length) {      // already received all command bytes?
            FDC.byte_count = 0;      // clear byte counter
            FDC.phase = EXEC_PHASE;  // switch to execution phase
            FDC.cmd_handler();
          }
    ...
    case EXEC_PHASE:                          // in execution phase?
      if (FDC.cmd_direction == CPU_TO_FDC) {  // proper direction?
        ...
        if (--FDC.buffer_count == 0) {  // processed all data?
          if ((FDC.flags &
               SCAN_flag)) {  // processing any of the scan commands?
            if ((FDC.flags & SCANFAILED_flag) &&
                (FDC.command[CMD_R] != FDC.command[CMD_EOT])) {
              FDC.command[CMD_R] +=
                  FDC.command[CMD_STP];  // advance to next sector
              cmd_scan();
            } else {
              if ((FDC.flags & SCANFAILED_flag)) {
            ...
            sector_size = 128 << FDC.command[CMD_C];  // determine number of
                                                      // bytes from N value
            if (((sector_size + 62 + FDC.command[CMD_R]) * FDC.command[CMD_H]) >
                CPC.max_tracksize) {      // track size exceeds maximum?
              ...
              active_track->data = new byte[track_size];  // attempt to allocate
                                                          // the required memory
              pbDataPtr = active_track->data;
              pbPtr = pbGPBuffer;
              for (sector = 0; sector < FDC.command[CMD_H]; sector++) {
                memcpy(active_track->sector[sector].CHRN, pbPtr,
                       4);  // copy CHRN
                memset(active_track->sector[sector].flags, 0,
                       2);  // clear ST1 & ST2
                active_track->sector[sector].setData(
                    pbDataPtr);  // store pointer to sector data
                pbDataPtr += sector_size;
                pbPtr += 4;
              }
              memset(active_track->data, FDC.command[CMD_N],
                     track_size);  // fill track data with specified byte value
            }
            pbPtr =
                pbGPBuffer + ((FDC.command[CMD_H] - 1) *
                              4);  // pointer to the last CHRN passed to writeID
            memcpy(&FDC.result[RES_C], pbPtr,
                   4);  // copy sector's CHRN to result buffer
            FDC.result[RES_N] =
                FDC.command[CMD_C];  // overwrite with the N value from the
                                     // writeID command

            active_drive->altered =
                true;  // indicate that the image has been modified
            FDC.phase = RESULT_PHASE;  // switch to result phase
          } else if (FDC.command[CMD_R] !=
                     FDC.command[CMD_EOT]) {  // haven't reached End of Track?
            FDC.command[CMD_R]++;             // advance to next sector
            cmd_write();
          } else {
            active_drive->altered =
          ...
          if (FDC.flags & OVERRUN_flag) {  // overrun condition detected?
            ...
            if (FDC.command[CMD_CODE] == 0x42) {  // read track command?
              if ((--FDC.command[CMD_EOT])) {     // continue reading sectors?
                if (active_drive->current_sector >=
                    active_track->sectors) {  // index beyond number of sectors
                                              // for this track?
                  active_drive->current_sector = 0;  // reset index
                  if (active_drive->track_hook)
                    active_drive->track_hook(
                        active_drive);  // update flakey data
                }
                FDC.command[CMD_R]++;  // advance to next sector
                cmd_readtrk();
              } else {
              ...
              if (!((FDC.result[RES_ST1] & 0x31) ||
                    (FDC.result[RES_ST2] & 0x21))) {  // no error bits set?
                if (FDC.command[CMD_R] !=
                    FDC.command[CMD_EOT]) {  // haven't reached End of Track?
                  FDC.command[CMD_R]++;      // advance to next sector
                  cmd_read();
                } else {
...
void fdc_specify() {
  FDC.phase = CMD_PHASE;  // switch back to command phase (fdc_specify has no
                          // result phase!)
}
...
void fdc_drvstat() {
  ...
  if ((active_drive->write_protected) ||
      (active_drive->tracks == 0)) {  // write protected, or disk missing?
...
void fdc_intstat() {
  ...
  if (FDC.flags & SEEKDRVA_flag) {   // seek completed on drive A?
    ...
    FDC.flags &= ~(SEEKDRVA_flag |
                   STATUSDRVA_flag);  // clear seek done and status change flags
    FDC.result[RES_ST0] = val;
    FDC.result[RES_ST1] = driveA.current_track;
  } else if (FDC.flags & SEEKDRVB_flag) {  // seek completed on drive B?
    val |= 0x21;                           // set Seek End
    FDC.flags &= ~(SEEKDRVB_flag |
                   STATUSDRVB_flag);  // clear seek done and status change flags
    FDC.result[RES_ST0] = val;
    FDC.result[RES_ST1] = driveB.current_track;
  } else if (FDC.flags &
             STATUSDRVA_flag) {  // has the status of drive A changed?
    ...
    FDC.result[RES_ST1] = driveA.current_track;
             ...
             STATUSDRVB_flag) {  // has the status of drive B changed?
...
void fdc_seek() {
  ...
  FDC.flags |= (FDC.command[CMD_UNIT] & 1)
                   ? SEEKDRVB_flag
                   : SEEKDRVA_flag;  // signal completion of seek operation
  FDC.phase = CMD_PHASE;  // switch back to command phase (fdc_seek has no
                          // result phase!)
}
...
void fdc_readtrk() {
  ...
  check_unit();                   // switch to target drive
  if (init_status_regs() == 0) {  // drive Ready?
    active_drive->current_side =
        (FDC.command[CMD_UNIT] & 4) >> 2;  // extract target side
    dword side = active_drive->sides
                     ? active_drive->current_side
                     : 0;  // single sided drives only acccess side 1
    if ((active_drive
             ->flipped)) {  // did the user request to access the "other" side?
...
void fdc_readID() {
  ...
  if (init_status_regs() == 0) {  // drive Ready?
    ...
    if (active_track->sectors != 0) {  // track is formatted?
      ...
      memcpy(&FDC.result[RES_C], &active_track->sector[idx].CHRN,
             4);  // copy sector's CHRN to result buffer
      active_drive->current_sector =
...
void fdc_writeID() {
  ...
  if (init_status_regs() == 0) {  // drive Ready?
    ...
    if (active_drive->write_protected) {  // is write protect tab set?
      ...
      FDC.buffer_count =
          FDC.command[CMD_H]
          << 2;  // number of sectors * 4 = number of bytes still outstanding
      FDC.buffer_ptr =
