...
namespace {
IDE_Device& active_ide() {
  return g_symbiface.active_drive ? g_symbiface.ide_slave
                                  : g_symbiface.ide_master;
}
}  // namespace

namespace {
uint32_t ide_lba(const IDE_Device& dev) {
  return (static_cast<uint32_t>(dev.drive_head & 0x0F) << 24) |
         (static_cast<uint32_t>(dev.lba_high) << 16) |
         (static_cast<uint32_t>(dev.lba_mid) << 8) | dev.lba_low;
}
}  // namespace
...
void ide_set_string(uint16_t* buf, int word_start, int word_count,
  ...
  for (int w = 0; w < word_count; w++) {
    int const si = w * 2;
    uint8_t const c0 = (si < slen) ? static_cast<uint8_t>(str[si]) : 0x20;
    uint8_t const c1 =
...
namespace {
void ide_do_identify(IDE_Device& dev) {
  ...
  id[55] = 16;
  id[56] = 63;
  uint32_t const chs_secs = cyls * 16 * 63;
...
namespace {
void ide_do_read(IDE_Device& dev) {
  if (!dev.image) {
    dev.status = ATA_SR_DRDY | ATA_SR_ERR;
    dev.error = 0x04;  // abort
    return;
  }
  uint32_t const lba = ide_lba(dev);
  ...
  size_t const read = fread(dev.sector_buf, 1, 512, dev.image);
...
namespace {
void ide_do_write_commit(IDE_Device& dev) {
  if (!dev.image) {
    dev.status = ATA_SR_DRDY | ATA_SR_ERR;
    dev.error = 0x04;
    return;
...
namespace {
void ide_execute_command(IDE_Device& dev) {
  switch (dev.command) {
    ...
    case ATA_CMD_INIT_PARAMS:
      ...
      break;
    default:
      LOG_DEBUG("Symbiface IDE: unknown command 0x"
                << std::hex << static_cast<int>(dev.command));
      dev.status = ATA_SR_DRDY | ATA_SR_ERR;
      dev.error = 0x04;  // abort
      break;
  }
...
void symbiface_reset() {
  ...
  g_symbiface.ide_master.status =
      g_symbiface.ide_master.present ? ATA_SR_DRDY : 0;
  ...
  g_symbiface.ide_slave.status =
      g_symbiface.ide_slave.present ? ATA_SR_DRDY : 0;
...
bool symbiface_ide_attach(int drive, const std::string& path) {
  ...
  long const size = ftell(dev.image);
  dev.total_sectors = static_cast<uint32_t>(size / 512);
  dev.status = ATA_SR_DRDY;

  LOG_INFO("Symbiface IDE " << drive << ": attached " << path << " ("
  ...
  switch (reg_offset & 0x07) {
    case 0:  // Data register
      if (dev.data_ready && dev.buf_pos < 512) {
        byte const val = dev.sector_buf[dev.buf_pos++];
        if (dev.buf_pos >= 512) {
          dev.data_ready = false;
          ...
          if (dev.command == ATA_CMD_READ_SECTORS) {
            dev.sector_count--;
            if (dev.sector_count > 0) {
              uint32_t const next = ide_lba(dev) + 1;
              dev.lba_low = next & 0xFF;
              dev.lba_mid = (next >> 8) & 0xFF;
              dev.lba_high = (next >> 16) & 0xFF;
              dev.drive_head = (dev.drive_head & 0xF0) | ((next >> 24) & 0x0F);
...
void symbiface_rtc_write_data(byte val) {
  uint8_t const reg = g_symbiface.rtc.address_reg;
  if (reg >= 14 && reg < 64) {
...
byte symbiface_rtc_read() {
  ...
  if (reg < 14) {
    ...
    switch (reg) {
      case 0:
        return to_bcd(t->tm_sec);  // seconds
      case 2:
        return to_bcd(t->tm_min);  // minutes
      case 4:
        return to_bcd(t->tm_hour);  // hours (24h)
      case 6:
        return static_cast<byte>(t->tm_wday == 0 ? 7
                                                 : t->tm_wday);  // day of week
      case 7:
        return to_bcd(t->tm_mday);  // day of month
      case 8:
        return to_bcd(t->tm_mon + 1);  // month
      case 9:
        return to_bcd(t->tm_year % 100);  // year
      case 10:
        return 0x26;  // Register A: UIP=0, DV=010, RS=0110
      case 11:
        return 0x02;  // Register B: 24h mode, BCD
      case 12:
        return 0x00;  // Register C: no interrupts
      case 13:
        return 0x80;  // Register D: VRT=1 (valid RAM)
      default:
        return 0;
    }
  } else if (reg < 64) {
...
namespace {
void mouse_fifo_push(SF2_Mouse& m, uint8_t val) {
  int const next = (m.head + 1) % SF2_Mouse::FIFO_SIZE;
  if (next == m.tail)
    return;  // full — drop oldest would complicate, just drop new
  m.fifo[m.head] = val;
  m.head = next;
}
}  // namespace
...
namespace {
uint8_t mouse_fifo_pop(SF2_Mouse& m) {
  if (m.head == m.tail) return 0x00;  // empty → mode 00 = no data
  uint8_t const val = m.fifo[m.tail];
...
void symbiface_mouse_update(float dx, float dy, uint32_t sdl_buttons) {
  ...
  int const whole_y = static_cast<int>(m.accum_y);
  if (whole_y != 0) {
    m.accum_y -= whole_y;
    int iy = -whole_y;
    iy = std::min(iy, 31);
...
namespace {
bool symbiface_in_handler_fd(reg_pair port, byte& ret_val) {
  byte const lo = port.b.l;
...
namespace {
bool symbiface_out_handler_fd(reg_pair port, byte val) {
  byte const lo = port.b.l;
...
void symbiface_register_io() {
  io_register_in(0xFD, symbiface_in_handler_fd, &g_symbiface.enabled,
                 "Symbiface II");
  io_register_out(0xFD, symbiface_out_handler_fd, &g_symbiface.enabled,
                  "Symbiface II");
}
