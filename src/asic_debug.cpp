...
std::string asic_dump_sprite(int index) {
  ...
  std::ostringstream out;
  char buf[128];
  int const mx = static_cast<int>(asic.sprites_mag_x[index]);
  int const my = static_cast<int>(asic.sprites_mag_y[index]);
  bool const enabled = (mx > 0 && my > 0);
  ...
  for (int y = 0; y < 16; y++) {
    for (int x = 0; x < 16; x++) {
      byte const pix = asic.sprites[index][x][y];
      int const val = (pix == 0) ? 0 : (pix - 16);
...
std::string asic_dump_palette() {
  ...
  for (int i = 0; i < 32; i++) {
    int const offset = kPaletteRegisterOffset + (i * 2);
    byte const rb = pbRegisterPage ? pbRegisterPage[offset] : 0;
    byte const g = pbRegisterPage ? pbRegisterPage[offset + 1] : 0;
    int const r_val = (rb >> 4) & 0x0F;
    int const g_val = g & 0x0F;
    int const b_val = rb & 0x0F;
    char buf[16];
