...
static void png_write_SDL(png_structp png_ptr, png_bytep data,
                          png_size_t length) {
...
int SDL_SavePNG(SDL_Surface* src, const std::string& file) {
  ...
  if (!src) {
    SDL_SetError(
        "Argument 1 to SDL_SavePNG can't be NULL, expecting SDL_Surface*\n");
    return (ERROR);
  }
  ...
  png_structp png_ptr =
      png_create_write_struct(PNG_LIBPNG_VER_STRING, nullptr, png_error_SDL,
                              nullptr); /* err_ptr, err_fn, warn_fn */
  if (!png_ptr) {
  ...
  if (setjmp(
          png_jmpbuf(png_ptr))) /* All other errors, see also "png_error_SDL" */
  {
  ...
  png_set_write_fn(png_ptr, dst, png_write_SDL,
                   nullptr); /* w_ptr, write_fn, flush_fn */
  ...
  if (bpp > 0 && bpp <= 8 && (pal = pal)) {
    colortype |= PNG_COLOR_MASK_PALETTE;
    std::unique_ptr<png_color[]> pal_ptr =
        std::make_unique<png_color[]>(pal->ncolors);
  ...
  std::unique_ptr<png_bytep[]> row_pointers =
      std::make_unique<png_bytep[]>(surface->h);
  for (i = 0; i < surface->h; i++)
    row_pointers[i] = static_cast<png_bytep>(static_cast<void*>(
        static_cast<char*>(surface->pixels) + i * surface->pitch));
  png_write_image(png_ptr, row_pointers.get());
#else
  for (i = 0; i < surface->h; i++)
    png_write_row(png_ptr,
                  (png_bytep)(Uint8*)surface->pixels + i * surface->pitch);
#endif
