...
void io_register_in(byte port_high, PeriphInHandler fn, const bool* enabled,
                    const char* name) {
  PortSlot& slot = g_io_dispatch.in_slots[port_high];
  if (slot.count >= MAX_PORT_HANDLERS) {
    LOG_ERROR("Expansion '"
              << name << "' cannot be enabled for I/O IN on port high-byte 0x"
              << std::hex << (int)port_high << std::dec
              << ": the slot is full (max 4 handlers). "
...
void io_register_out(byte port_high, PeriphOutHandler fn, const bool* enabled,
                     const char* name) {
  PortSlot& slot = g_io_dispatch.out_slots[port_high];
  if (slot.count >= MAX_PORT_HANDLERS) {
    LOG_ERROR("Expansion '"
              << name << "' cannot be enabled for I/O OUT on port high-byte 0x"
              << std::hex << (int)port_high << std::dec
              << ": the slot is full (max 4 handlers). "
              << "Try disabling other expansions sharing this port range.");
    return;
...
void io_register_kbd_read_hook(KeyboardReadHook fn, const bool* enabled) {
  ...
  if (slot.count >= MAX_HOOKS) {
    LOG_ERROR(
        "Failed to register keyboard read hook: maximum capacity reached.");
    return;
  }
...
void io_register_kbd_line_hook(NotifyHookInt fn, const bool* enabled) {
  ...
  if (slot.count >= MAX_HOOKS) {
    LOG_ERROR(
        "Failed to register keyboard line hook: maximum capacity reached.");
    return;
  }
  slot.entries[slot.count++] = {fn, enabled};
}
...
void io_dispatch_clear() { g_io_dispatch = IODispatch{}; }
