...
class t_sector {
 public:
  ...
  unsigned char
      flags[4];  // ST1 and ST2 - reflects any possible error conditions
 ...
 private:
  ...
  unsigned int weak_versions_;  // number of versions of this sector (should be
                                // 1 except for weak/random sectors)
  unsigned int
      weak_read_version_;  // version of the sector to return when reading
};
...
struct t_drive {
  ...
  t_track track[DSK_TRACKMAX]
               [DSK_SIDEMAX];  // array of track information structures
};
