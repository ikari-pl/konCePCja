...
inline byte io_fire_kbd_read_hooks(int line) {
  ...
  for (int i = 0; i < slot.count; i++) {
    if (*slot.entries[i].enabled) mask &= slot.entries[i].fn(line);
  }
  return mask;
}
...
inline void io_fire_kbd_line_hooks(int line) {
  ...
  for (int i = 0; i < slot.count; i++) {
    if (*slot.entries[i].enabled) slot.entries[i].fn(line);
  }
}
...
inline void io_fire_tape_motor_hooks(bool on) {
  ...
  for (int i = 0; i < slot.count; i++) {
    if (*slot.entries[i].enabled) slot.entries[i].fn(on);
  }
}
