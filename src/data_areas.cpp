...
void DataAreaManager::mark(uint16_t start, uint16_t end, DataType type,
...
void DataAreaManager::clear(uint16_t start) { areas_.erase(start); }

void DataAreaManager::clear_all() { areas_.clear(); }
...
std::string DataAreaManager::format_at(uint16_t addr, const uint8_t* mem,
                                       size_t mem_size,
  ...
  switch (area->type) {
    case DataType::BYTES: {
      oss << "db ";
      int const remaining =
          static_cast<int>(area->end) - static_cast<int>(addr) + 1;
      int const count = std::min(remaining, 8);
    ...
    case DataType::WORDS: {
      oss << "dw ";
      int const remaining_bytes =
          static_cast<int>(area->end) - static_cast<int>(addr) + 1;
      int const word_count = std::min(remaining_bytes / 2, 4);
      for (int i = 0; i < word_count; i++) {
        size_t const off = static_cast<size_t>(i * 2);
        if (off + 1 >= mem_size) break;
        if (i > 0) oss << ",";
        uint16_t const w =
            static_cast<uint16_t>(mem[off] | (mem[off + 1] << 8));
        oss << '$' << std::setw(4) << w;
        consumed += 2;
      }
      break;
    ...
    case DataType::TEXT: {
      ...
      int const remaining =
          static_cast<int>(area->end) - static_cast<int>(addr) + 1;
      int const count = std::min(remaining, 64);
      bool in_string = false;
      bool first = true;
      for (int i = 0; i < count; i++) {
        if (static_cast<size_t>(i) >= mem_size) break;
        uint8_t const c = mem[i];
        if (c >= 0x20 && c < 0x7F) {
          if (!in_string) {
            if (!first) oss << ",";
            oss << "\"";
