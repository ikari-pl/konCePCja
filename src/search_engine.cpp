  ...
  while (iss >> token) {
    if (token == "*") {
      ...
      for (size_t i = 0; i + 1 < token.size(); i += 2) {
        std::string const pair = token.substr(i, 2);
        if (pair == "??" || pair == "?") {
          result.push_back({PatternElement::ANY_ONE, 0});
        } else {
          unsigned int val = 0;
...
bool match_recursive(const std::vector<PatternElement>& compiled,
  ...
  while (pat_idx < compiled.size()) {
    ...
    if (elem.kind == PatternElement::LITERAL) {
      ...
      uint8_t const pat_val = elem.value;
      if (case_insensitive) {
      ...
      size_t const max_skip =
          std::min(mem_size - mem_pos, static_cast<size_t>(256));
...
namespace {
std::string hex_context(const uint8_t* mem, size_t offset, size_t len) {
    ...
    for (size_t addr = 0; addr < mem_size && results.size() < max_results;
      ...
      if (search_detail::match_pattern(compiled, mem, mem_size, addr, match_len,
        ...
        for (size_t i = 0; i < match_len && i < 32; i++) {
          char const c = static_cast<char>(mem[addr + i]);
