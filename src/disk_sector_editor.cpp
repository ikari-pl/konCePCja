...
namespace {
unsigned int sector_size_from_N(uint8_t N) { return 128u << N; }
}  // namespace
...
std::vector<uint8_t> disk_sector_read(t_drive* drive, unsigned int track,
  ...
  if (!sec) {
    ...
    oss << "sector " << std::hex << std::uppercase
        << static_cast<unsigned>(sector_id) << " not found on track "
        << std::dec << track << " side " << side;
    err = oss.str();
    return {};
  }

  unsigned int const size = sector_size_from_N(sec->CHRN[3]);
...
std::vector<SectorInfo> disk_sector_info(t_drive* drive, unsigned int track,
  ...
  t_track const& trk = drive->track[track][side];
  std::vector<SectorInfo> result;
  result.reserve(trk.sectors);

  for (unsigned int s = 0; s < trk.sectors; s++) {
    t_sector const& sec = trk.sector[s];
    SectorInfo info;
    info.C = sec.CHRN[0];
    info.H = sec.CHRN[1];
    info.R = sec.CHRN[2];
