...
int Tape_ReadSampleDataBit() {
  if (dwTapeDataCount) {
    ...
    iTapeCycleCount += static_cast<int>(
        dwTapePulseCycles);  // set cycle count for current level
#ifdef DEBUG_TAPE
    fprintf(pfoDebug, "%c %d\r\n", (bTapeLevel == TAPE_LEVEL_HIGH ? 'H' : 'L'),
            iTapeCycleCount);
#endif
    return 1;
  }
  ...
  while (pbTapeBlock < pbTapeImageEnd) {  // loop until a valid block is found
    ...
    switch (*pbTapeBlock) {
...
#endif
        dwTapePulseCycles =
            CYCLE_ADJUST(*reinterpret_cast<word*>(pbTapeBlock + 0x01));
        iTapeCycleCount += static_cast<int>(
            dwTapePulseCycles);  // set cycle count for current level
      ...
      case 0x13:  // sequence of pulses of different length
        ...
        pwTapePulseTable = pwTapePulseTablePtr =
            reinterpret_cast<word*>(pbTapeBlock + 0x01 + 0x01);
      ...
      case 0x14:  // pure data block
        ...
        fputs("--- DATA\r\n", pfoDebug);
#endif
        dwTapeZeroPulseCycles = CYCLE_ADJUST(*reinterpret_cast<word*>(
            pbTapeBlock + 0x01));  // pulse length for a zero bit
        dwTapeOnePulseCycles = CYCLE_ADJUST(*reinterpret_cast<word*>(
            pbTapeBlock + 0x01 + 0x02));  // pulse length for a one bit
        dwTapeDataCount =
            ((*reinterpret_cast<dword*>(pbTapeBlock + 0x01 + 0x07) &
              0x00ffffff) -
             1)
            << 3;  // (byte count - 1) * 8 bits
        dwTapeDataCount +=
            *(pbTapeBlock + 0x01 +
              0x04);  // add the number of bits in the last data byte
        pbTapeBlockData =
            pbTapeBlock + 0x01 + 0x0a;  // pointer to the tape data
        dwTapeBitsToShift = 0;
      ...
      case 0x15:  // direct recording
        ...
        fputs("--- SAMPLE DATA\r\n", pfoDebug);
#endif
        dwTapePulseCycles = CYCLE_ADJUST(*reinterpret_cast<word*>(
            pbTapeBlock + 0x01));  // number of T states per sample
        dwTapeDataCount =
            ((*reinterpret_cast<dword*>(pbTapeBlock + 0x01 + 0x05) &
              0x00ffffff) -
             1)
            << 3;  // (byte count - 1) * 8 bits
        dwTapeDataCount +=
            *(pbTapeBlock + 0x01 +
              0x04);  // add the number of bits in the last data byte
        pbTapeBlockData =
            pbTapeBlock + 0x01 + 0x08;  // pointer to the tape data
        dwTapeBitsToShift = 0;
        Tape_ReadSampleDataBit();  // get the first bit of the first data byte
        return 1;

      case 0x20:  // pause
        if (*reinterpret_cast<word*>(pbTapeBlock +
                                     0x01)) {  // was a pause requested?
          dwTapeStage = TAPE_PAUSE_STAGE;
#ifdef DEBUG_TAPE
          fputs("--- PAUSE\r\n", pfoDebug);
#endif
          dwTapePulseCycles = MS_TO_CYCLES(
              1);  // start with a 1ms level opposite to the one last played
          iTapeCycleCount += static_cast<int>(
              dwTapePulseCycles);  // set cycle count for current level
          dwTapePulseCycles =
              MS_TO_CYCLES(*reinterpret_cast<word*>(pbTapeBlock + 0x01) -
                           1);   // get the actual pause length
      ...
      case 0x21:  // group start
        pbTapeBlock +=
            *(pbTapeBlock + 0x01) + 1 + 1;  // nothing to do, skip the block
        break;
      ...
      case 0x31:  // message block
        pbTapeBlock += *(pbTapeBlock + 0x01 + 0x01) + 2 +
                       1;  // nothing to do, skip the block
        break;

      case 0x32:  // archive info
        pbTapeBlock += *reinterpret_cast<word*>(pbTapeBlock + 0x01) + 2 +
                       1;  // nothing to do, skip the block
        break;

      case 0x33:  // hardware type
        pbTapeBlock += (*(pbTapeBlock + 0x01) * 3) + 1 +
                       1;  // nothing to do, skip the block
        break;
      ...
      case 0x35:  // custom info block
        pbTapeBlock += *reinterpret_cast<dword*>(pbTapeBlock + 0x01 + 0x10) +
                       0x14 + 1;  // nothing to do, skip the block
        break;

      case 0x40:  // snapshot block
        pbTapeBlock += (*reinterpret_cast<dword*>(pbTapeBlock + 0x01 + 0x01) &
                        0x00ffffff) +
                       0x04 + 1;  // nothing to do, skip the block
        break;
      ...
      default:  // "extension rule"
        pbTapeBlock += *reinterpret_cast<dword*>(pbTapeBlock + 0x01) + 4 +
                       1;  // nothing to do, skip the block
    }
  }
...
void Tape_BlockDone() {
  if (pbTapeBlock < pbTapeImageEnd) {
    switch (*pbTapeBlock) {
      case 0x10:  // standard speed data block
        pbTapeBlock +=
            *reinterpret_cast<word*>(pbTapeBlock + 0x01 + 0x02) + 0x04 + 1;
        break;

      case 0x11:  // turbo loading data block
        pbTapeBlock += (*reinterpret_cast<dword*>(pbTapeBlock + 0x01 + 0x0f) &
                        0x00ffffff) +
                       0x12 + 1;
        break;
      ...
      case 0x14:  // pure data block
        pbTapeBlock += (*reinterpret_cast<dword*>(pbTapeBlock + 0x01 + 0x07) &
                        0x00ffffff) +
                       0x0a + 1;
        break;

      case 0x15:  // direct recording
        pbTapeBlock += (*reinterpret_cast<dword*>(pbTapeBlock + 0x01 + 0x05) &
                        0x00ffffff) +
                       0x08 + 1;
        break;
      ...
      if (dwTapePulseCount > 0) {  // is the pilot tone still playing?
        ...
        switch (*pbTapeBlock) {
          case 0x10:  // standard speed data block
            ...
            wCycleTable[0] = 667;
            wCycleTable[1] = 735;
            pwTapePulseTable = pwTapePulseTablePtr = &wCycleTable[0];
...
#endif
            pwTapePulseTable = pwTapePulseTablePtr =
                reinterpret_cast<word*>(pbTapeBlock + 0x01 + 0x02);
            pwTapePulseTableEnd =
                reinterpret_cast<word*>(pbTapeBlock + 0x01 + 0x06);
            Tape_GetCycleCount();
            iTapeCycleCount += static_cast<int>(
                dwTapePulseCycles);  // set cycle count for current level
#ifdef DEBUG_TAPE
      ...
      if (dwTapePulseCount > 0) {
        ...
        switch (*pbTapeBlock) {
          case 0x10:  // standard speed data block
            ...
            fputs("--- DATA\r\n", pfoDebug);
#endif
            dwTapeZeroPulseCycles =
                CYCLE_ADJUST(855);  // pulse length for a zero bit
            dwTapeOnePulseCycles =
                CYCLE_ADJUST(1710);  // pulse length for a one bit
            dwTapeDataCount =
                *reinterpret_cast<word*>(pbTapeBlock + 0x01 + 0x02)
                << 3;  // byte count * 8 bits;
            pbTapeBlockData =
                pbTapeBlock + 0x01 + 0x04;  // pointer to the tape data
            dwTapeBitsToShift = 0;
            Tape_ReadDataBit();
            iTapeCycleCount += static_cast<int>(
                dwTapePulseCycles);  // set cycle count for current level
...
#endif
            dwTapeZeroPulseCycles = CYCLE_ADJUST(*reinterpret_cast<word*>(
                pbTapeBlock + 0x01 + 0x06));  // pulse length for a zero bit
            dwTapeOnePulseCycles = CYCLE_ADJUST(*reinterpret_cast<word*>(
                pbTapeBlock + 0x01 + 0x08));  // pulse length for a one bit
            dwTapeDataCount =
                ((*reinterpret_cast<dword*>(pbTapeBlock + 0x01 + 0x0f) &
                  0x00ffffff) -
                 1)
                << 3;  // (byte count - 1) * 8 bits;
            dwTapeDataCount +=
                *(pbTapeBlock + 0x01 +
                  0x0c);  // add the number of bits in the last data byte
            pbTapeBlockData =
                pbTapeBlock + 0x01 + 0x12;  // pointer to the tape data
            dwTapeBitsToShift = 0;
            Tape_ReadDataBit();
            iTapeCycleCount += static_cast<int>(
                dwTapePulseCycles);  // set cycle count for current level
      ...
      if (dwTapePulseCount > 0) {
        ...
        if (Tape_ReadDataBit()) {
          ...
          switch (*pbTapeBlock) {
...
#endif
                dwTapePulseCycles =
                    MS_TO_CYCLES(1);  // start with a 1ms level opposite to the
                                      // one last played
                iTapeCycleCount += static_cast<int>(
                    dwTapePulseCycles);  // set cycle count for current level
#ifdef DEBUG_TAPE
            ...
            case 0x11:  // turbo loading data block
              if (*reinterpret_cast<word*>(pbTapeBlock + 0x01 +
                                           0x0d)) {  // was a pause requested?
                dwTapeStage = TAPE_PAUSE_STAGE;
#ifdef DEBUG_TAPE
                fputs("--- PAUSE\r\n", pfoDebug);
#endif
...
#endif
                dwTapePulseCycles = MS_TO_CYCLES(
                    *reinterpret_cast<word*>(pbTapeBlock + 0x01 + 0x0d) -
                    1);                // pause in ms
                dwTapePulseCount = 2;  // just one pulse
              } else {
                Tape_BlockDone();
...
#endif
                dwTapePulseCycles = MS_TO_CYCLES(
                    *reinterpret_cast<word*>(pbTapeBlock + 0x01 + 0x05) -
                    1);                // pause in ms
                dwTapePulseCount = 2;  // just one pulse
              } else {
                Tape_BlockDone();
...
#endif
          dwTapePulseCycles =
              MS_TO_CYCLES(*reinterpret_cast<word*>(pbTapeBlock + 0x01 + 0x02) -
                           1);   // pause in ms
          dwTapePulseCount = 2;  // just one pulse
        } else {
          Tape_BlockDone();
