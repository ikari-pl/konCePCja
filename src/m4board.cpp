...
// Upper-ROM bank table owned by the memory subsystem (kon_cpc_ja.cpp).
// Declared once at file scope so every M4 command handler can index it.
...
std::string resolve_path(const std::string& rel_path) {
  ...
  std::string const base = g_m4board.sd_root_path;
...
namespace {
void respond_error(const char* msg = nullptr) {
  ...
  if (msg) {
    ...
    size_t const max = static_cast<size_t>(M4Board::RESPONSE_SIZE - 5);
    len = std::min(len, max);
...
namespace {
void respond_error_code(uint8_t code) {
  g_m4board.response[0] = M4_ERROR;
  g_m4board.response[1] = 0;
  g_m4board.response[2] = 0;
...
namespace {
void cmd_cd() {
  std::string const path = extract_string(g_m4board.cmd_buf, 3);
  ...
  std::string const resolved = resolve_path(path);
  if (resolved.empty()) {
  ...
  try {
    if (std::filesystem::is_directory(resolved)) {
      auto root_canonical =
          std::filesystem::weakly_canonical(g_m4board.sd_root_path);
      auto rel =
          std::filesystem::path(resolved).lexically_normal().lexically_relative(
              root_canonical.lexically_normal());
      std::string rel_str;
      if (rel == ".") {
        rel_str = "/";
...
// Populate the directory entry cache (called by C_DIRSETARGS or on first
// C_READDIR)
namespace {
void dir_populate() {
  ...
  std::string const resolved = resolve_path(g_m4board.current_dir);
  ...
  try {
    for (auto& entry : std::filesystem::directory_iterator(resolved)) {
      ...
      bool const is_dir = entry.is_directory();
      uint32_t fsize = 0;
      if (!is_dir) {
        try {
          fsize = static_cast<uint32_t>(entry.file_size());
...
void format_amsdos_83(const std::string& name, bool is_dir,
  ...
  for (int i = 0; i < 3; i++) {
    if (i < static_cast<int>(ext.size()))
      out[9 + i] =
          static_cast<char>(toupper(static_cast<unsigned char>(ext[i])));
    else
      out[9 + i] = ' ';
  }
}
...
void cmd_readdir() {
  ...
  bool const ls_mode = (g_m4board.cmd_buf.size() > 3);
  ...
  if (ls_mode) {
    ...
    int const max_name = M4Board::RESPONSE_SIZE - pos - 1;
    int const len = std::min(static_cast<int>(entry.name.size()), max_name);
    ...
    // ASCII size field (5 chars): "1234B", " 100K", "  15M", "   2G", or
    // "<DIR>"
    char sizebuf[12];
    if (entry.is_dir) {
      ...
      snprintf(sizebuf, sizeof(sizebuf), "%4uM",
               (entry.size + (1024 * 1024) - 1) / (1024 * 1024));
    } else {
      snprintf(
          sizebuf, sizeof(sizebuf), "%4uG",
          (entry.size + (1024u * 1024 * 1024) - 1) / (1024u * 1024 * 1024));
    }
    ...
    uint16_t const kb = static_cast<uint16_t>(
        std::min(entry.size / 1024, static_cast<uint32_t>(0xFFFF)));
...
FILE* try_open_cpc(const std::string& dir_path,
  ...
  try {
    for (const auto& entry : std::filesystem::directory_iterator(dir_path)) {
      std::string const entry_name = entry.path().filename().string();
      std::string entry_upper = entry_name;
      for (auto& c : entry_upper)
      ...
      for (const auto& cand : candidates) {
        ...
        if (entry_upper == cand_upper) {
          ...
          if (fp) {
            LOG_DEBUG("M4: C_OPEN matched '" << entry_name << "' for '"
                                             << filename << "'");
            return fp;
          }
        }
...
// Open a file from inside a DSK container: extract to a temp file, return
// FILE*.
namespace {
FILE* container_open_file(const std::string& filename) {
...
void cmd_open() {
  ...
  // mode = FatFs access flags: FA_READ=0x01, FA_WRITE=0x02,
  // FA_CREATE_ALWAYS=0x08 Read (RUN/LOAD):  mode = FA_READ (0x01) Write (SAVE):
  // mode = FA_WRITE|FA_CREATE_ALWAYS (0x0A)
  uint8_t const mode =
      (g_m4board.cmd_buf.size() > 3) ? g_m4board.cmd_buf[3] : 0x01;
  bool const is_write = (mode & 0x02) != 0;   // FA_WRITE
  bool const is_create = (mode & 0x08) != 0;  // FA_CREATE_ALWAYS
  std::string raw_name = extract_string(g_m4board.cmd_buf, 4);
  if (raw_name.empty()) {
    raw_name = extract_string(g_m4board.cmd_buf, 3);
  }
  LOG_DEBUG("M4: C_OPEN name='" << raw_name << "' mode=0x" << std::hex
                                << (int)mode << std::dec);
  ...
  if (in_container()) {
    ...
    int const handle = 1;  // FA_READ → handle 1
  ...
  std::string const dir = resolve_path(g_m4board.current_dir);
  if (dir.empty()) {
  ...
  if (is_write && is_create) {
    ...
    std::string const resolved = resolve_path(raw_name);
    if (resolved.empty()) {
    ...
    if (!resolved.empty()) {
      g_m4board.open_files[handle] =
          fopen(resolved.c_str(), is_write ? "r+b" : "rb");
    }
...
void cmd_close() {
  ...
  int const handle = g_m4board.cmd_buf[3];
...
// C_READ: Used by fread() in M4 ROM for bulk data loading (_cas_in_direct,
// etc.) Protocol: cmd[3]=fd, cmd[4-5]=count (16-bit LE) Response:
// response[3]=status (0=OK, non-zero=error), response[4+]=data bytes ROM copies
// data from rom_response+4 with LDIR, bc=requested chunk size.
namespace {
void cmd_read() {
  ...
  int const max_read = M4Board::RESPONSE_SIZE - 4;
  if (count > max_read) count = static_cast<uint16_t>(max_read);

  size_t const nread =
      fread(g_m4board.response + 4, 1, count, g_m4board.open_files[handle]);
...
// Response: response[3]=status (0=OK, 0x14=EOF), response[4-5]=bytes_read
// (16-bit LE),
...
void cmd_read2() {
  ...
  int const max_read = M4Board::RESPONSE_SIZE - 8;
  if (count > max_read) count = static_cast<uint16_t>(max_read);

  size_t const nread =
      fread(g_m4board.response + 8, 1, count, g_m4board.open_files[handle]);
  bool const at_eof = feof(g_m4board.open_files[handle]);
...
// On the real M4, this reads from .dsk images. For our virtual FS (loose
// files), we map track/sector to a linear file offset via AMSDOS convention:
...
void cmd_readsector() {
  ...
  uint8_t const track = g_m4board.cmd_buf[3];
  uint8_t const sector = g_m4board.cmd_buf[4];
  ...
  int const linear_sector = (sector >= 0xC1) ? (sector - 0xC1) : sector;
  long const offset = ((track * 9) + linear_sector) * 512L;

  fseek(f, offset, SEEK_SET);
  size_t const nread = fread(g_m4board.response + 4, 1, 512, f);
  // Pad remainder with 0xE5 (empty disc filler)
  if (nread < 512) memset(g_m4board.response + 4 + nread, 0xE5, 512 - nread);
  ...
  if (in_container() && g_m4board.container_drive) {
    ...
    for (const auto& f : files) {
      ...
      for (auto& c : fu)
        c = static_cast<char>(toupper(static_cast<unsigned char>(c)));
      if (fu == upper) {
        uint32_t const sz = f.size_bytes;
        g_m4board.response[0] = M4_OK;
        g_m4board.response[1] = 0;
        g_m4board.response[2] = 0;
...
void cmd_erasefile() {
  if (in_container()) {
    respond_error("Read-only container");
    return;
  }
  std::string const path = extract_string(g_m4board.cmd_buf, 3);
  std::string const resolved = resolve_path(path);
...
void cmd_write() {
  ...
  int const handle = g_m4board.cmd_buf[3];
  if (handle < 0 || handle >= 4 || !g_m4board.open_files[handle]) {
    LOG_DEBUG("M4: C_WRITE bad handle="
              << handle << " file="
              << (g_m4board.open_files[handle & 3] ? "open" : "null"));
    respond_error("Bad handle");
    return;
  }

  size_t const data_len = g_m4board.cmd_buf.size() - 4;
  LOG_DEBUG("M4: C_WRITE fd=" << handle << " data_len=" << data_len
                              << " buf_size=" << g_m4board.cmd_buf.size());
  size_t const written = fwrite(g_m4board.cmd_buf.data() + 4, 1, data_len,
                                g_m4board.open_files[handle]);
...
void cmd_seek() {
  ...
  uint32_t const offset = static_cast<uint32_t>(g_m4board.cmd_buf[4]) |
  ...
  if (fseek(g_m4board.open_files[handle], static_cast<long>(offset),
            SEEK_SET) == 0) {
    respond_ok();
  } else {
...
void cmd_rename() {
  ...
  std::string const newname = extract_string(g_m4board.cmd_buf, 3);
  // Find the second string after the first null terminator
  size_t const old_offset = 3 + newname.size() + 1;
  if (old_offset >= g_m4board.cmd_buf.size()) {
    respond_error("Missing old name");
    return;
  }
  std::string const oldname = extract_string(g_m4board.cmd_buf, old_offset);

  std::string const resolved_old = resolve_path(oldname);
  std::string const resolved_new = resolve_path(newname);
...
void cmd_httpget() {
  if (!g_m4board.network_enabled) {
    respond_error("WiFi disabled");
    return;
  }
  // Protocol: [size, cmd_lo, cmd_hi, "url:port/file"]
  // URL format: [@ prefix]host[:port]/path[>outfile]
  std::string const raw_url = extract_string(g_m4board.cmd_buf, 3);
  if (raw_url.empty()) {
  ...
  std::string const full_url = "http://" + url;

  // Resolve output path within the virtual SD
  std::string const dest = resolve_path(out_filename);
  if (dest.empty()) {
  ...
  g_m4board.response[1] = 0;
  g_m4board.response[2] = 0;
  size_t const max_ok = static_cast<size_t>(M4Board::RESPONSE_SIZE - 4);
...
size_t curl_write_mem(void* ptr, size_t size, size_t nmemb,
  ...
  size_t const to_copy = std::min(total, remain);
  if (to_copy > 0) {
...
void cmd_httpgetmem() {
  ...
  uint16_t const max_dl = (static_cast<uint16_t>(g_m4board.cmd_buf[3]) << 8) |
                          static_cast<uint16_t>(g_m4board.cmd_buf[4]);
  std::string const raw_url = extract_string(g_m4board.cmd_buf, 5);
  if (raw_url.empty()) {
    respond_error("No URL given");
    return;
  }
  ...
  size_t const max_size =
      std::min(static_cast<size_t>(max_dl),
  ...
  uint16_t const dl_size = static_cast<uint16_t>(membuf.data.size());
  g_m4board.response[0] = M4_OK;
  g_m4board.response[1] = 0;
  g_m4board.response[2] = 0;
  ...
  LOG_INFO("M4 HTTPGETMEM: downloaded " << dl_size << " bytes from "
                                        << full_url);
}
}  // namespace
...
void cmd_writesector() {
  ...
  fseek(f, offset, SEEK_SET);
  size_t const written = fwrite(g_m4board.cmd_buf.data() + 6, 1, 512, f);
  fflush(f);
...
void cmd_sdread() {
  ...
  uint32_t const lba = static_cast<uint32_t>(g_m4board.cmd_buf[3]) |
  ...
  uint8_t const num = g_m4board.cmd_buf[7];
  ...
  g_m4board.response_len = 4;
  LOG_DEBUG("M4: C_SDREAD lba=" << lba << " n=" << (int)num
                                << " (not supported)");
}
}  // namespace
...
void cmd_sdwrite() {
  // Protocol: [size, cmd_lo, cmd_hi, lba0, lba1, lba2, lba3, num_sectors,
  // data...] Raw SD not supported in emulation
  g_m4board.response[0] = M4_OK;
  g_m4board.response[1] = 0;
  g_m4board.response[2] = 0;
  g_m4board.response[3] = 1;  // error flag
...
std::string get_host_ip() {
  ...
  std::string result;
  int best_score = -1;
  for (struct ifaddrs const* ifa = ifap; ifa; ifa = ifa->ifa_next) {
    if (!ifa->ifa_addr || ifa->ifa_addr->sa_family != AF_INET) continue;
    auto* sa = reinterpret_cast<struct sockaddr_in*>(ifa->ifa_addr);
    char buf[INET_ADDRSTRLEN];
    inet_ntop(AF_INET, &sa->sin_addr, buf, sizeof(buf));
    int const s = score_interface(ifa->ifa_name, buf);
    if (s > best_score) {
      best_score = s;
      result = buf;
    }
...
namespace {
std::string get_host_ssid_uncached();
}  // namespace

namespace {
std::string get_host_ssid() {
  ...
  if (std::chrono::duration_cast<std::chrono::seconds>(now - cached_at)
              .count() < 30 &&
      !cached_ssid.empty()) {
    return cached_ssid;
  }
...
void cmd_netstat() {
  ...
  std::string const ip = get_host_ip();
  std::string const ssid = get_host_ssid();
  ...
  size_t const max_len =
      static_cast<size_t>(M4Board::RESPONSE_SIZE - 6);  // room for \0 + status
...
void cmd_time() {
  ...
  char buf[64];
  snprintf(buf, sizeof(buf), "%02d:%02d:%02d %04d-%02d-%02d", t->tm_hour,
           t->tm_min, t->tm_sec, t->tm_year + 1900, t->tm_mon + 1, t->tm_mday);
  g_m4board.response[0] = M4_OK;
  g_m4board.response[1] = 0;
  g_m4board.response[2] = 0;
...
namespace {
void sock_close(M4Board::socket_t s) {
#ifdef _WIN32
  closesocket(s);
#else
  close(s);
...
namespace {
void sock_set_nonblocking(M4Board::socket_t s) {
#ifdef _WIN32
  u_long mode = 1;
...
void cmd_netsocket() {
  ...
  M4Board::socket_t const s = socket(AF_INET, SOCK_STREAM, 0);
...
void cmd_netconnect() {
  ...
  // Protocol: [size, cmd_lo, cmd_hi, socket, ip0, ip1, ip2, ip3, port_hi,
  // port_lo]
  if (g_m4board.cmd_buf.size() < 10) {
    respond_error();
    return;
  }
  int const slot = g_m4board.cmd_buf[3];
  ...
  uint32_t const ip = (static_cast<uint32_t>(g_m4board.cmd_buf[4]) << 24) |
                      (static_cast<uint32_t>(g_m4board.cmd_buf[5]) << 16) |
                      (static_cast<uint32_t>(g_m4board.cmd_buf[6]) << 8) |
                      static_cast<uint32_t>(g_m4board.cmd_buf[7]);
  addr.sin_addr.s_addr = htonl(ip);
  uint16_t const port = (static_cast<uint16_t>(g_m4board.cmd_buf[8]) << 8) |
  ...
  int const result =
      connect(g_m4board.sockets[slot],
  ...
  if (ok) {
    ...
    LOG_INFO("M4: C_NETCONNECT slot "
             << slot << " -> " << (int)g_m4board.cmd_buf[4] << "."
             << (int)g_m4board.cmd_buf[5] << "." << (int)g_m4board.cmd_buf[6]
             << "." << (int)g_m4board.cmd_buf[7] << ":" << port
...
void cmd_netclose() {
  ...
  if (slot >= 0 && slot < M4Board::MAX_SOCKETS &&
    ...
    if (getsockname(g_m4board.sockets[slot],
                    ...
                    &addr_len) == 0) {
      uint16_t const bound_port = ntohs(local_addr.sin_port);
      if (bound_port > 0) {
        for (auto& m : g_m4_http.port_mappings()) {
          if (m.host_port == bound_port && m.active) {
            m.active = false;
            LOG_DEBUG("M4: port mapping CPC " << m.cpc_port << " -> host "
                                              << m.host_port
                                              << " now inactive");
            break;
          }
...
void cmd_netsend() {
  ...
  int const slot = g_m4board.cmd_buf[3];
  uint16_t const data_size = static_cast<uint16_t>(g_m4board.cmd_buf[4]) |
  ...
  size_t const avail = g_m4board.cmd_buf.size() - 6;
  size_t const to_send = std::min(static_cast<size_t>(data_size), avail);
  if (to_send > 0) {
    int const sent =
        send(g_m4board.sockets[slot],
...
void cmd_netrecv() {
  ...
  int const slot = g_m4board.cmd_buf[3];
  uint16_t const max_recv = static_cast<uint16_t>(g_m4board.cmd_buf[4]) |
                            (static_cast<uint16_t>(g_m4board.cmd_buf[5]) << 8);
  ...
  size_t const buf_space = static_cast<size_t>(M4Board::RESPONSE_SIZE - 6);
  size_t const to_recv = std::min(static_cast<size_t>(max_recv), buf_space);

  // Non-blocking recv — returns immediately if no data available
  int const received = recv(g_m4board.sockets[slot],
  ...
  if (received > 0) {
    ...
    g_m4board.response_len = (6 + received);
  } else {
...
void cmd_nethostip() {
  ...
  std::string const hostname = extract_string(g_m4board.cmd_buf, 3);
  if (hostname.empty()) {
    g_m4board.response[0] = M4_OK;
    g_m4board.response[1] = 0;
    g_m4board.response[2] = 0;
  ...
  int const err = getaddrinfo(hostname.c_str(), nullptr, &hints, &result);
  if (err != 0 || !result) {
    // Lookup failed — return "in progress" (will never resolve, as on offline
    // M4)
    g_m4board.response[0] = M4_OK;
    g_m4board.response[1] = 0;
    g_m4board.response[2] = 0;
  ...
  uint32_t const ip = ntohl(addr->sin_addr.s_addr);
  freeaddrinfo(result);
  ...
  LOG_DEBUG("M4: C_NETHOSTIP resolved "
            << hostname << " -> " << (int)g_m4board.response[4] << "."
            << (int)g_m4board.response[5] << "." << (int)g_m4board.response[6]
            << "." << (int)g_m4board.response[7]);
}
}  // namespace
...
void cmd_netbind() {
  ...
  uint16_t const cpc_port = (static_cast<uint16_t>(g_m4board.cmd_buf[8]) << 8) |
                            static_cast<uint16_t>(g_m4board.cmd_buf[9]);
  ...
  uint16_t const host_port = g_m4_http.resolve_host_port(cpc_port);
  ...
  for (int p = host_port; p < host_port + 10; p++) {
    ...
    if (bind(g_m4board.sockets[slot], reinterpret_cast<struct sockaddr*>(&addr),
             sizeof(addr)) == 0) {
      bound_port = p;
      break;
    }
  ...
  if (bound_port == 0) {
    LOG_ERROR("M4: C_NETBIND failed — no port available near "
              << host_port << " (CPC requested " << cpc_port << ")");
    g_m4board.response[0] = M4_OK;
    g_m4board.response[1] = 0;
    g_m4board.response[2] = 0;
    g_m4board.response[3] = 0xFF;
  ...
  bool const was_user = (g_m4_http.resolve_host_port(cpc_port) != cpc_port);
  g_m4_http.set_port_mapping(cpc_port, static_cast<uint16_t>(bound_port),
                             was_user);
  // Mark active
  for (auto& m : g_m4_http.port_mappings()) {
  ...
  LOG_INFO("M4: C_NETBIND slot "
           << slot << " CPC port " << cpc_port << " -> host port " << bound_port
...
void cmd_netlisten() {
  ...
  int const result = listen(g_m4board.sockets[slot], 1);
  g_m4board.response[0] = M4_OK;
  g_m4board.response[1] = 0;
  g_m4board.response[2] = 0;
...
void cmd_netaccept() {
  ...
  // If a client connected, it replaces the listening socket with the accepted
  // one and returns response[3] = 0 (success).
  if (g_m4board.cmd_buf.size() < 4) {
    respond_error();
    return;
  }
...
#endif
  M4Board::socket_t const client =
      accept(g_m4board.sockets[slot], reinterpret_cast<struct sockaddr*>(&peer),
             &plen);
  ...
  uint32_t const peer_ip = ntohl(peer.sin_addr.s_addr);
  LOG_INFO("M4: C_NETACCEPT slot "
           << slot << " accepted from " << ((peer_ip >> 24) & 0xFF) << "."
           << ((peer_ip >> 16) & 0xFF) << "." << ((peer_ip >> 8) & 0xFF) << "."
...
void cmd_wifipow() {
  ...
  bool const on = (g_m4board.cmd_buf[3] != 0);
...
void cmd_copybuf() {
  ...
  // Copies from the internal buffer (last C_HTTPGETMEM response data at offset
  // 5+) into the response buffer. This lets CPC software read large HTTP
  // downloads in chunks.
  if (g_m4board.cmd_buf.size() < 7) {
    respond_error();
    return;
  }
  uint16_t const offset = (static_cast<uint16_t>(g_m4board.cmd_buf[3]) << 8) |
                          static_cast<uint16_t>(g_m4board.cmd_buf[4]);
  uint16_t const size = (static_cast<uint16_t>(g_m4board.cmd_buf[5]) << 8) |
  ...
  int const data_start = 5 + static_cast<int>(offset);
  ...
  g_m4board.response[1] = 0;
  g_m4board.response[2] = 0;
  // Data copied in-place (it's already in the response buffer at offset
  // 5+offset) Move it to response[3]+ for the CPC to read
...
void cmd_copyfile() {
  ...
  if (g_m4board.sd_root_path.empty()) {
    respond_error("No SD path");
    return;
  }

  // Extract two null-terminated strings
  std::string const src = extract_string(g_m4board.cmd_buf, 3);
  size_t const src_end = 3 + src.size() + 1;  // past the null terminator
  std::string dst;
  ...
  try {
    ...
    // Traversal check
    auto src_rel =
        src_full.lexically_normal().lexically_relative(root.lexically_normal());
    auto dst_rel =
        dst_full.lexically_normal().lexically_relative(root.lexically_normal());
    ...
    std::filesystem::copy_file(
        src_full, dst_full, std::filesystem::copy_options::overwrite_existing);
...
void cmd_romlow() {
  ...
  int const mode = (g_m4board.cmd_buf.size() >= 4) ? g_m4board.cmd_buf[3] : 0;
...
namespace {
void cmd_romcp() {
  // Protocol: [size, cmd_lo, cmd_hi, dest_hi, dest_lo, src_bank, src_hi,
  // src_lo,
  ...
  if (g_m4board.cmd_buf.size() < 11) {
    respond_error();
    return;
  }

  uint16_t const dest_off = (static_cast<uint16_t>(g_m4board.cmd_buf[3]) << 8) |
                            static_cast<uint16_t>(g_m4board.cmd_buf[4]);
  // src_bank = cmd_buf[5] (0 = M4 ROM)
  uint16_t const src_off = (static_cast<uint16_t>(g_m4board.cmd_buf[6]) << 8) |
                           static_cast<uint16_t>(g_m4board.cmd_buf[7]);
  // dest_bank = cmd_buf[8]
  uint16_t const size = (static_cast<uint16_t>(g_m4board.cmd_buf[9]) << 8) |
  ...
  if (src_off + size > 0x4000 || dest_off + size > 0x4000) {
    LOG_ERROR("M4: C_ROMCP out of bounds (src=0x"
              << std::hex << src_off << " dst=0x" << dest_off
              << " size=" << std::dec << size << ")");
    respond_error();
    return;
  }
...
void cmd_config() {
  ...
  // (jump vectors, ROM slot number, AMSDOS version, etc.) at ROM offset
  // 0x3400+.
  if (g_m4board.cmd_buf.size() < 4) {
    respond_error();
    return;
  }
  uint8_t const config_offset = g_m4board.cmd_buf[3];
  size_t const data_len = g_m4board.cmd_buf.size() - 4;

  // Store in local config buffer
  size_t const max_len = std::min(
      data_len, static_cast<size_t>(M4Board::CONFIG_SIZE - config_offset));
  if (max_len > 0) {
    memcpy(g_m4board.config_buf + config_offset, &g_m4board.cmd_buf[4],
           max_len);
  }
...
void cmd_ftell() {
  ...
  long const pos = ftell(g_m4board.open_files[handle]);
  if (pos < 0) {
...
void cmd_getpath() {
  ...
  size_t const max_len = static_cast<size_t>(M4Board::RESPONSE_SIZE - 4);
  size_t const len = std::min(dir.size(), max_len);
...
void cmd_free() {
  ...
  std::string const resolved = resolve_path("/");
  uint64_t free_kb = 0;
  if (!resolved.empty()) {
    try {
  ...
  char buf[32];
  if (free_kb >= 1048576)
    snprintf(buf, sizeof(buf), "%lluG free",
             static_cast<unsigned long long>(free_kb / 1048576));
  else if (free_kb >= 1024)
    snprintf(buf, sizeof(buf), "%lluM free",
             static_cast<unsigned long long>(free_kb / 1024));
  else
    snprintf(buf, sizeof(buf), "%lluK free",
             static_cast<unsigned long long>(free_kb));
...
void m4board_data_out(byte val) { g_m4board.cmd_buf.push_back(val); }
...
void m4board_execute() {
  ...
  uint16_t const cmd =
      g_m4board.cmd_buf[1] | (static_cast<uint16_t>(g_m4board.cmd_buf[2]) << 8);
  ...
  switch (cmd) {
    ...
    case C_READDIR:
      cmd_readdir();
      g_m4board.last_op = M4Board::LastOp::DIR;
      break;
    case C_OPEN:
      cmd_open();
      break;
    ...
    case C_READ:
      cmd_read();
      g_m4board.last_op = M4Board::LastOp::READ;
      break;
    case C_READ2:
      cmd_read2();
      g_m4board.last_op = M4Board::LastOp::READ;
    ...
    case C_WRITESECTOR:
      cmd_writesector();
      g_m4board.last_op = M4Board::LastOp::WRITE;
      break;
    case C_FORMATTRACK:
      cmd_formattrack();
      break;
...
void m4board_write_response(byte* rom_base) {
  ...
  // Write response at offset &2800 within the ROM (maps to &E800 in CPC address
  // space) The M4 ROM link table at &FF02 points to &E800 = ROM base + &2800
  int const offset = 0x2800;
  int const len = std::min(g_m4board.response_len, M4Board::RESPONSE_SIZE);
...
void m4board_load_rom(byte** rom_map, const std::string& rom_path,
  ...
  int const slot = g_m4board.rom_slot;
  if (slot < 0 || slot >= 256) return;

  // Override any existing ROM in this slot (like real hardware on the expansion
  // bus)
  if (rom_map[slot] != nullptr) {
    LOG_INFO("M4: overriding ROM in slot " << slot
                                           << " (expansion bus priority)");
    delete[] rom_map[slot];
  ...
  size_t const read = fread(rom_data, 1, 16384, fp);
  fclose(fp);
  ...
  // On real hardware, the M4 firmware (ESP8266) pre-fills runtime data in the
  // ROM image before the CPC boots. The ROM's init sends C_CONFIG commands to
  // populate the data area at offset 0x3400+ (jump vectors, ROM slot number,
  // AMSDOS version). Then set_SDdrive (offset 0x798) patches the CAS firmware
  // jumpblock (&BC77+) so CAT/LOAD/SAVE go through the M4. This works now
  // because cmd_config() fills the ROM data area — no need to disable
  // set_SDdrive.
  ...
  // becomes inaccessible when TXT OUTPUT pages out the ROM to write screen
  // memory. Stage 1 (ROM &F800): saves DE, copies stage 2 + string to RAM
  // &8000, jumps there. Stage 2 (RAM &8000): prints string via CALL &BB5A,
  // restores DE, SCF/RET. DE must be preserved — the firmware uses it to track
  // free memory after ROM init.
  ...
  memcpy(rom_data + 0x380F + sizeof(stage2), boot_msg,
         sizeof(boot_msg));  // 21 bytes
...
namespace {
bool m4board_out_handler_fe(reg_pair port, byte val) {
...
namespace {
bool m4board_out_handler_fc(reg_pair /*port*/, byte /*val*/) {
  m4board_execute();
...
void m4board_register_io() {
  io_register_out(0xFE, m4board_out_handler_fe, &g_m4board.enabled,
                  "M4 Board Data");
  io_register_out(0xFC, m4board_out_handler_fc, &g_m4board.enabled,
                  "M4 Board Kick");
}
