...
static std::string extract_string(const std::vector<uint8_t>& buf,
                                  size_t offset) {
  std::string s;
...
static bool container_enter_dsk(const std::string& host_path,
...
static void format_amsdos_83(const std::string& name, bool is_dir,
                             char out[12]) {
...
static void cmd_readdir() {
  ...
  if (ls_mode) {
    ...
    if (entry.is_dir) {
      ...
      snprintf(sizebuf, sizeof(sizebuf), "%4uG",
    ...
    uint16_t kb =
        static_cast<uint16_t>(std::min(entry.size / 1024, uint32_t(0xFFFF)));
...
static FILE* try_open_cpc(const std::string& dir_path,
...
static size_t curl_write_file(void* ptr, size_t size, size_t nmemb,
                              void* userdata) {
...
static size_t curl_write_mem(void* ptr, size_t size, size_t nmemb,
                             void* userdata) {
...
static void cmd_free() {
  ...
  if (free_kb >= 1048576)
    snprintf(buf, sizeof(buf), "%lluG free",
             (unsigned long long)(free_kb / 1048576));
