...
bool write_le_u16(uint16_t val, FILE* f) {
  return fputc(val & 0xff, f) != EOF && fputc((val >> 8) & 0xff, f) != EOF;
}
...
uint32_t WavRecorder::stop() {
  ...
  uint32_t const result = data_bytes_;
  data_bytes_ = 0;
  path_.clear();
  return result;
}
...
void WavRecorder::write_samples(const uint8_t* data, uint32_t len) {
  ...
  size_t const written = fwrite(data, 1, len, file_);
...
void WavRecorder::write_header() {
  uint32_t const byte_rate = sample_rate_ * channels_ * (bits_per_sample_ / 8);
...
void WavRecorder::finalize_header() {
  ...
  uint32_t const riff_size =
      data_bytes_ + 36;  // 44 - 8 = 36 bytes of header after RIFF size
