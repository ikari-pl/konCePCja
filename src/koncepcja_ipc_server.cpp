...
T parse_num_impl(const std::string& s) {
...
namespace {
unsigned long parse_number(const std::string& s) {
...
bool is_safe_path(const std::string& path_str) {
  ...
  std::filesystem::path const p(path_str);
  // Normalize the path to collapse "." and ".." components.
  std::filesystem::path const normalized = p.lexically_normal();
...
namespace {
std::string build_debug_context() {
  char buf[256];
  // ROM config: bit2=lower ROM disable, bit3=upper ROM disable
  bool const lo_rom = !(GateArray.ROM_config & 0x04);
  bool const hi_rom = !(GateArray.ROM_config & 0x08);
  std::string rom_str;
  if (lo_rom && hi_rom)
  ...
  // error
  size_t const len =
      (n < 0) ? 0 : std::min(static_cast<size_t>(n), sizeof(buf) - 1);
...
void ipc_apply_keypress(CPCScancode cpc_key,
  ...
  if (pressed) {
    ...
    keyboard_matrix[static_cast<byte>(cpc_key) >> 4].fetch_or(
        bit_values[static_cast<byte>(cpc_key) & 7], std::memory_order_relaxed);
    keyboard_matrix[0x25 >> 4].fetch_or(bit_values[0x25 & 7],
                                        std::memory_order_relaxed);
    keyboard_matrix[0x27 >> 4].fetch_or(bit_values[0x27 & 7],
...
namespace {
bool has_path_traversal(const std::string& path) {
...
void init_command_registry() {
  ...
  register_command("ping", "CORE", "ping", "Check if server is alive",
                   "Pings the IPC server. The server will respond with 'OK "
                   "pong' if it is operational.",
                   [](const auto&, const auto&) { return "OK pong\n"; });

  register_command(
      "version", "CORE", "version", "Get emulator and protocol version",
      "Returns the emulator version string and the active IPC port number.",
      [](const auto&, const auto&) {
        int const p = g_ipc_instance ? g_ipc_instance->port() : 0;
  ...
  register_command(
      "metrics", "CORE", "metrics", "Get current performance / timing metrics",
      "Returns the timing counters the render thread updates once per second:\n"
      "  frame_time_avg_us   — full frame wall-clock (Z80 + render)\n"
      "  display_time_avg_us — Phase A wall-clock (framebuffer upload + ImGui "
      "render)\n"
  ...
  register_command("quit", "CORE", "quit [code]",
                   "Exit the emulator with optional exit code",
                   "Terminates the emulator process immediately. An optional "
                   "integer exit code can be provided.");

  register_command("disconnect", "CORE", "disconnect",
                   "Close the current IPC connection",
                   "Closes the current persistent IPC connection. The client "
                   "socket is closed and the server "
                   "returns to listening for new connections. Note: 'quit' "
                   "always terminates the emulator "
                   "(with optional exit code); use 'disconnect' to close only "
                   "the connection.");

  register_command("pause", "CORE", "pause", "Pause emulation",
                   "Stops the Z80 CPU and machine timers. The UI remains "
  ...
  register_command("reset", "CORE", "reset [--no-resume]",
                   ...
                   "Performs a hardware reset of the CPC. By default, "
                   "emulation resumes immediately after reset. "
                   "Use --no-resume to keep the machine paused (useful for "
                   "setting breakpoints before BIOS startup).");

  register_command(
      "tape", "MEDIA",
      "tape play|stop|rewind|eject|status | tape seek <block> | "
      "tape volume [0-100] | tape linein on [left|right|mix] | tape linein off",
      "Cassette deck control",
      "play/stop drive the PLAY button (the firmware owns the motor relay). "
      "status reports deck state (sub-cycle engine adds block/pos/level). "
      "linein routes the microphone through the host Schmitt stage into the "
      "deck; 'tape lineout on [left|right] [rdata|wdata]' renders the deck "
  ...
  register_command("bp", "DEBUG",
                   ...
                   "  list:  Shows all active breakpoints with their addresses "
                   "and conditions.\n"
  ...
  register_command(
      "wp", "DEBUG",
      "wp add <addr> [len] [r|w|rw] [if <expr>] [pass <N>] | wp del <index> | "
      "wp list | wp clear",
      ...
      "  list:  Shows all watchpoints with index, address, range, type and "
      "conditions.\n"
  ...
  register_command(
      "iobp", "DEBUG",
      "iobp add <port> [mask] [in|out|both] [if <expr>] | iobp del <index> | "
      "iobp list | iobp clear",
      "Manage IO breakpoints",
      "Controls I/O port breakpoints (break on IN/OUT instructions).\n"
      "  add:   Adds an IO breakpoint. Port can use BCXX shorthand (X=wildcard "
      "nibble).\n"
  ...
  register_command(
      "wait", "DEBUG",
      "wait pc <addr> [timeout] | wait mem <addr> <val> [mask] [timeout] | "
  ...
  register_command(
      ...
      "Force screen render from current RAM state (no CPU advancement)",
      "Re-renders the CPC display from the current CRTC registers and video "
      "RAM "
      "contents without advancing the Z80 CPU. Useful when the CPU is paused "
      "and "
      "the display surface may not reflect the latest writes to screen "
      "memory.\n"
  ...
  register_command(
      "snapshot", "MEDIA", "snapshot save <path> | snapshot load <path>",
      "Manage machine snapshots",
      "Saves or loads the entire state of the emulated CPC into a .SNA file.");

  register_command("record", "MEDIA", "record wav|ym|avi <start|stop> [path]",
  ...
  register_command(
      "frames", "MEDIA", "frames dump <pattern> <count>",
      "Dump series of frames",
      "Dumps a sequence of frames to an animated GIF (if pattern ends in .gif) "
      "or a series of PNG files. Pattern can include %d or %04d for frame "
      "numbering.");

  register_command(
      "devtools", "TOOLS", "devtools <on|off|show|hide> [name]",
  ...
  register_command(
      ...
      "  The buffer drains on each read — reconnecting returns only new text "
      "since the previous connection.\n"
      "  Input bytes sent to the telnet port are fed into the CPC keyboard "
      "buffer (AutoTypeQueue).\n"
  ...
  register_command(
      "disasm", "DEBUG",
      "disasm <addr> <count> [--symbols] | disasm follow <addr> | disasm refs "
      "<addr> | disasm export <start> <end> [path] [--symbols]",
      "Disassemble Z80 code",
      "Disassemble instructions, follow code flow, find cross-references, or "
      "export to .asm source.\n"
  ...
  register_command("sym", "DEBUG",
                   "sym load <path> | sym add <addr> <name> | sym del <name> | "
                   "sym list [filter] | sym lookup <addr_or_name>",
                   "Manage symbol table",
                   "Load .sym files, add/delete symbols, list with optional "
                   "filter, or look up by address or name.");

  register_command("data", "DEBUG",
                   "data mark <start> <end> <bytes|words|text> [label] | data "
                   "clear <addr|all> | data list",
                   "Mark memory regions as data for disassembly",
                   "Marks address ranges so the disassembler formats them as "
                   "data (bytes, words or text) instead of code.");

  register_command("stack", "DEBUG", "stack [depth]", "Walk call stack",
                   "Heuristic stack walk from SP upward. Shows return "
                   "addresses with [call] markers and symbol names.\n"
                   "  Default depth: 16, max: 128.");

  register_command(
      "trace", "DEBUG",
      "trace on [buffer_size] | trace off | trace dump <path> | trace on_crash "
      "<path> | trace status",
      "Z80 instruction trace",
      "Ring-buffer execution trace (default 65536 entries). Records every\n"
      "retired instruction (PC + A/F/BC/DE/HL/SP + opcode) under BOTH the\n"
      "legacy core and the sub-cycle engine (engine=1).\n"
      "  on:       Enable tracing.\n"
      "  off:      Disable tracing.\n"
  ...
  register_command("timer", "DEBUG", "timer list | timer clear", "Debug timers",
                   "List debug timers with hit count, last/min/max/average "
                   "elapsed times. Clear all timers.");

  register_command("event", "DEBUG",
                   "event on <trigger> <command> | event once <trigger> "
                   "<command> | event off <id> | event list",
                   "Fire IPC commands on triggers",
                   "Register commands to auto-execute on PC match (pc=ADDR), "
                   "memory write (mem=ADDR[:VAL]), or VBL interval (vbl=N).\n"
  ...
  register_command("hash", "DEBUG",
                   ...
                   "Compute CRC32 of screen surface, memory range, or packed "
                   "register state.\n"
  ...
  register_command(
      "gfx", "DEBUG",
      "gfx view <addr> <w> <h> <mode> [path] | gfx decode <hex> <mode> | gfx "
      "paint <addr> <w> <h> <mode> <x> <y> <color> | gfx palette",
  ...
  register_command(
      ...
      "Manage the 256 KB battery-backed Silicon Disc in banks 4-7 (6128+ "
      "only).\n"
  ...
  register_command(
      "m4", "HARDWARE",
      "m4 status | m4 ls | m4 cd <path> | m4 reset | m4 wifi [0|1] | m4 http "
      "[start|stop|status] | m4 ports | m4 port set/del <cpc> [host]",
  ...
  register_command(
      "poke", "TOOLS",
      "poke load <path> | poke list | poke apply <game> <poke|all> | poke "
      "unapply <game> <poke> | poke write <addr> <val>",
  ...
  register_command(
      "session", "TOOLS",
      "session record <path> | session play <path> | session stop | session "
      "status",
...
// Split a line on semicolons for command chaining (e.g. "pause; bp add $4000;
// run"). Trims leading/trailing whitespace from each segment. Skips empty
// segments.
  ...
  try {
    ...
    if (cmd == "help") {
      if (parts.size() > 1) {
        ...
        return "ERR 404 no specific help for '" + parts[1] +
      ...
      oss << "OK available commands (usage: help <command>):\n"
          << "  Protocol: persistent connections, ';' chains commands, "
             "'disconnect' closes.\n"
          << "  Numbers: 0x, $, &, # hex prefixes accepted (e.g. $C000, &4000, "
             "#BB5A).\n";
      static const std::vector<std::string> order = {
          "CORE", "DEBUG", "HARDWARE", "INPUT", "MEDIA", "TOOLS"};
    ...
    if (cmd == "quit") {
      ...
      if (parts.size() >= 2) {
        try {
          code = parse_int(parts[1]);
        } catch (const std::exception&) {
          return "ERR 400 bad-exit-code\n";
        }
      }
    ...
    if (cmd == "hash" && parts.size() >= 2) {
      ...
      if (parts[1] == "mem" && parts.size() >= 4) {
        ...
        len =
            std::min<unsigned int>(len, 65536);  // Clamp to full address space
        uLong crc = crc32(0L, nullptr, 0);
      ...
      if (parts[1] == "regs") {
        ...
        packed.AF = z80.AF.w.l;
        packed.BC = z80.BC.w.l;
        packed.DE = z80.DE.w.l;
        packed.HL = z80.HL.w.l;
        packed.IX = z80.IX.w.l;
        packed.IY = z80.IY.w.l;
        packed.SP = z80.SP.w.l;
        packed.PC = z80.PC.w.l;
        packed.AFx = z80.AFx.w.l;
        packed.BCx = z80.BCx.w.l;
        packed.DEx = z80.DEx.w.l;
        packed.HLx = z80.HLx.w.l;
        packed.I = z80.I;
        packed.R = z80.R;
        packed.IM = z80.IM;
        packed.IFF1 = z80.IFF1;
        packed.IFF2 = z80.IFF2;
        uLong crc = crc32(0L, nullptr, 0);
        crc =
            crc32(crc, reinterpret_cast<const Bytef*>(&packed), sizeof(packed));
        snprintf(buf, sizeof(buf), "OK crc32=%08lX\n", crc);
        return {buf};
      }
    ...
    if (cmd == "repaint") {
      ...
      for (size_t i = 1; i < parts.size(); i++) {
        if (parts[i] == "--screenshot" && i + 1 < parts.size()) {
          shot_path = parts[i + 1];
          if (!is_safe_path(shot_path))
            return "ERR 403 path-traversal-blocked\n";
          break;
        }
      }
    ...
    if ((cmd == "reg" || cmd == "regs") && parts.size() >= 2 &&
      ...
      resp << "OK";
      char buf[64];
      snprintf(buf, sizeof(buf), " MODE=%u PEN=%02X", GateArray.scr_mode,
               GateArray.pen);
      resp << buf;
      for (int i = 0; i < 17; i++) {
      ...
      snprintf(buf, sizeof(buf),
               " ROM_CFG=%02X RAM_CFG=%02X SL=%02X INT_DELAY=%02X",
               GateArray.ROM_config, GateArray.RAM_config, GateArray.sl_count,
               GateArray.int_delay);
      resp << buf << "\n";
      return resp.str();
    }
    if ((cmd == "reg" || cmd == "regs") && parts.size() >= 2 &&
      ...
      snprintf(buf, sizeof(buf), " SELECT=%02X CONTROL=%02X", PSG.reg_select,
               PSG.control);
      resp << buf << "\n";
      return resp.str();
    }
    if ((cmd == "reg" || cmd == "regs") && parts.size() >= 2 &&
        parts[1] == "asic") {
      subcycle_bridge_refresh_asic_view();  // Device truth into the view
    ...
    if (cmd == "reg")
      return "ERR 400 usage: reg (get|set|crtc|ga|psg|asic) ...\n";
    ...
    if (cmd == "asic" && parts.size() >= 2) {
      ...
      if (parts[1] == "sprite") {
        ...
        try {
          ...
          return "ERR 400 bad-args (asic sprite <0-15>)\n";
        }
        if (idx < 0 || idx > 15)
          return "ERR 400 sprite index out of range (0-15)\n";
      ...
      if (parts[1] == "dma") {
        if (parts.size() >= 3) {
          int ch = 0;
          try {
            ch = parse_int(parts[2]);
          } catch (const std::exception&) {
            return "ERR 400 bad-args (asic dma <0-2>)\n";
          }
          if (ch < 0 || ch > 2)
            return "ERR 400 DMA channel out of range (0-2)\n";
    ...
    if (cmd == "regs") {
      ...
      snprintf(out, sizeof(out),
               ...
               z80.AF.b.h, z80.AF.b.l, z80.BC.b.h, z80.BC.b.l, z80.DE.b.h,
               z80.DE.b.l, z80.HL.b.h, z80.HL.b.l, z80.IX.w.l, z80.IY.w.l,
               z80.SP.w.l, z80.PC.w.l, z80.IM, z80.HALT);
      return {out};
    }
    if (cmd == "screenshot") {
      if (parts.size() >= 3 && parts[1] == "window") {
        ...
        // Wait for the main loop to render a frame and capture (up to 5
        // seconds)
        for (int i = 0; i < 500; i++) {
          std::this_thread::sleep_for(std::chrono::milliseconds(10));
    ...
    if (cmd == "tape") {
      ...
      if (parts[1] == "rewind") {
        if (mach) mach->tape_rewind_deck();
        return ok_with_context();
      }
      if (parts[1] == "eject") {
        if (mach) mach->eject_tape();
        CPC.tape.file = "";
        return ok_with_context();
      }
      ...
      if (parts[1] == "status") {
        std::ostringstream os;
        os << "OK play=" << (CPC.tape_play_button ? 1 : 0);
        if (mach) {
          TapeRegs tr{};
          tape_peek(mach->tape(), &tr);
          os << " attached=" << static_cast<int>(tr.attached)
             << " motor=" << static_cast<int>(tr.motor)
             << " level=" << static_cast<int>(tr.level) << " block=" << tr.block
             << " pos=" << tr.pos
             << " linein=" << static_cast<int>(tr.line_mode)
             << " lineout=" << (tape_line_out_active() ? 1 : 0)
             << " error=" << static_cast<int>(tr.error);
        }
        // Loaded cassette path (CPC.tape.file). Printed outside the `mach`
        // gate so it reports under either engine. Trailing field: existing
        // parsers keyed to the front of the line are unaffected.
        os << " file=\"" << CPC.tape.file << "\"";
        os << "\n";
        return os.str();
      }
      if (parts[1] == "volume") {
        // tape volume [<0-100>] — level of the tape data monitor ("screech");
        // no arg reports. Persists via [sound] tape_data_volume.
        if (parts.size() >= 3) {
          try {
      ...
      if (parts[1] == "lineout" && parts.size() >= 3) {
        if (!mach) return "ERR 409 subcycle-engine-only\n";
        if (parts[2] == "off") {
          tape_line_out_disarm(*mach);
          return "OK\n";
        }
        if (parts[2] == "on") {
          // tape lineout on [left|right] [rdata|wdata] — opt-in, soft-ramped,
          // auto-disarmed on any audio device change (cable-swap guard).
          int ch = 0;
          bool src_rdata = true;
          for (size_t i = 3; i < parts.size(); i++) {
            if (parts[i] == "right")
              ...
              ch = 0;
            else if (parts[i] == "wdata")
              src_rdata = false;
            else if (parts[i] == "rdata")
              src_rdata = true;
          }
          if (!tape_line_out_arm(*mach, ch, src_rdata))
            return "ERR 503 no-playback-device\n";
          return "OK ramping\n";
        }
      }
      if (parts[1] == "linein" && parts.size() >= 3) {
        if (!mach) return "ERR 409 subcycle-engine-only\n";
        if (parts[2] == "off") {
          tape_line_in_stop();
          mach->tape_line_in(false);
          return "OK\n";
        }
        if (parts[2] == "on") {
          int ch = 0;
    ...
    if (cmd == "snapshot")
      return "ERR 400 usage: snapshot (save|load) <path>\n";
    if (cmd == "mem" && parts.size() >= 4 && parts[1] == "read") {
      ...
      unsigned int const len = parse_number(parts[3]);
      ...
      for (unsigned int i = 0; i < len; i++) {
        ...
        resp << std::hex << std::uppercase << std::setfill('0') << std::setw(2)
             << static_cast<int>(v);
        if (with_ascii) {
          char const c = (v >= 32 && v <= 126) ? static_cast<char>(v) : '.';
    ...
    if (cmd == "mem" && parts.size() >= 5 && parts[1] == "compare") {
      ...
      for (unsigned int i = 0; i < len; i++) {
        ...
        if (v1 != v2) {
          ...
          if (diff_count <= 64) {
            char buf[32];
            snprintf(buf, sizeof(buf), " %04X:%02X:%02X", (addr1 + i), v1, v2);
            diffs += buf;
          }
        }
      }
    ...
    if (cmd == "mem" && parts.size() >= 4 && parts[1] == "cpu-read") {
      // mem cpu-read <addr> <len> — reads through CPU memory map
      // (SmartWatch/ROM banking) but does NOT trigger watchpoints or IPC
      // events.
      try {
        unsigned int const addr = parse_number(parts[2]);
        unsigned int const len = parse_number(parts[3]);
        ...
        resp << "OK ";
        for (unsigned int i = 0; i < len; i++) {
          byte const v = z80_cpu_read_mem(static_cast<word>(addr + i));
    ...
    if (cmd == "disasm" && parts.size() >= 2) {
      ...
      if (parts[1] == "follow" && parts.size() >= 3) {
        ...
        for (const auto& line : code.lines) {
          std::string const sym = g_symfile.lookupAddr(line.address_);
      ...
      if (parts[1] == "refs" && parts.size() >= 3) {
        unsigned int const target = parse_number(parts[2]);
        std::ostringstream resp;
        resp << "OK";
        int found = 0;
        // NOLINTNEXTLINE(misc-const-correctness): clang-tidy FP — variable is
      ...
      if (parts[1] == "export" && parts.size() >= 4) {
        ...
        word const end_pos = static_cast<word>(end_addr);

        while (pos <= end_pos) {
          // Emit symbol label if present
          if (with_symbols) {
            std::string const sym = g_symfile.lookupAddr(pos);
          ...
          if (da) {
            int const remaining =
                static_cast<int>(da->end) - static_cast<int>(pos) + 1;
            int const max_bytes = (da->type == DataType::TEXT) ? 64 : 8;
            ...
            int line_bytes = 0;
            std::string const formatted = g_data_areas.format_at(
                pos, membuf.data(), membuf.size(), &line_bytes);
            oss << "  " << formatted;
            // Add hex comment
            oss << "  ; ";
            ...
            if (with_symbols && !line.ref_address_string_.empty()) {
              std::string const sym = g_symfile.lookupAddr(line.ref_address_);
              if (!sym.empty()) {
            ...
            snprintf(buf, sizeof(buf), "%04X:", static_cast<unsigned>(pos));
            oss << buf;
            int const sz = line.Size();
            for (int bi = 0; bi < sz; bi++) {
              snprintf(buf, sizeof(buf), " %02X",
                       z80_read_mem(static_cast<word>(pos + bi)));
              oss << buf;
            }
            oss << "\n";
            unsigned int const next =
                static_cast<unsigned int>(pos) + line.Size();
            if (next > 0xFFFF || next > end_addr + 1u) break;
            pos = static_cast<word>(next);
          }
        }
        ...
        for (int i = 0; i < count; i++) {
          ...
          if (da) {
            ...
            int const remaining =
                static_cast<int>(da->end) - static_cast<int>(pos) + 1;
            int const max_bytes = (da->type == DataType::TEXT) ? 64 : 8;
            int const buf_len = std::min(remaining, max_bytes);
            std::vector<uint8_t> membuf(buf_len);
            for (int mi = 0; mi < buf_len; mi++) {
              membuf[mi] = z80_read_mem(static_cast<word>(pos + mi));
            }
            ...
            std::string const formatted = g_data_areas.format_at(
                pos, membuf.data(), membuf.size(), &line_bytes);
            resp << std::setfill('0') << std::setw(4) << std::hex << pos
                 << ":          " << formatted << "\n";
            if (line_bytes == 0) line_bytes = 1;
            ...
            if (with_symbols && !line.ref_address_string_.empty()) {
              ...
              if (!sym.empty()) {
                ...
                resp << std::setfill('0') << std::setw(4) << std::hex
                     << line.address_ << ": ";
                resp << std::setfill(' ') << std::setw(8) << line.opcode_ << " "
                     << instr << "\n";
              } else {
                resp << line << "\n";
              }
    ...
    if (cmd == "bp" && parts.size() >= 2) {
      if (parts[1] == "add" && parts.size() >= 3) {
        ...
        if (!cond_str.empty()) {
          ...
          z80_add_breakpoint_cond(static_cast<word>(addr), std::move(ast),
                                  cond_str, pass_count);
        } else {
      ...
      if (parts[1] == "list") {
        ...
        for (const auto& b : bps) {
          resp << " " << std::hex << std::uppercase << std::setfill('0')
               << std::setw(4) << static_cast<unsigned int>(b.address);
      ...
      return "ERR 400 usage: bp (add <addr> [if <expr>] [pass <N>] | del "
             "<addr> | list | clear)\n";
    }
    if (cmd == "bp")
      return "ERR 400 usage: bp (add <addr> [if <expr>] [pass <N>] | del "
    ...
    if (cmd == "iobp" && parts.size() >= 2) {
      if (parts[1] == "add" && parts.size() >= 3) {
        ...
        if (port_str.size() == 4 && (port_str.find('X') != std::string::npos ||
                                     port_str.find('x') != std::string::npos)) {
          // Shorthand: BCXX → port=0xBC00, mask=0xFF00
          port_val = 0;
          mask_val = 0;
          for (int ni = 0; ni < 4; ni++) {
            char const ch = port_str[ni];
            int const shift = (3 - ni) * 4;
            if (ch == 'X' || ch == 'x') {
        ...
        if (!cond_str.empty()) {
          ...
          auto ast = expr_parse(cond_str, err);
          if (!ast) return "ERR 400 bad-expr: " + err + "\n";
          z80_add_io_breakpoint_cond(port_val, mask_val, dir, std::move(ast),
                                     cond_str);
        } else {
      ...
      if (parts[1] == "del" && parts.size() >= 3) {
        int const idx = parse_int(parts[2]);
      ...
      if (parts[1] == "list") {
        ...
        for (size_t i = 0; i < bps.size(); i++) {
          ...
          if (bps[i].dir == IO_IN)
            dir_str = "in";
          else if (bps[i].dir == IO_OUT)
            dir_str = "out";
          resp << " " << i << ":" << std::hex << std::uppercase
               << std::setfill('0') << std::setw(4)
               << static_cast<unsigned>(bps[i].port) << "/" << std::setw(4)
    ...
    if (cmd == "step") {
      ...
      if (parts.size() == 1 ||
          (parts.size() >= 2 &&
           (parts[1] == "in" ||
            std::isdigit(static_cast<unsigned char>(parts[1][0]))))) {
        int count = 1;
        if (parts.size() >= 2) {
          if (parts[1] == "in") {
    ...
    // Trace commands: trace on [size], trace off, trace dump <path>, trace
    // on_crash <path>
    if (cmd == "trace" && parts.size() >= 2) {
      if (parts[1] == "on") {
        ...
        g_trace.enable(size);
        subcycle_bridge_set_instr_trace(true);  // engine=1 feed (no-op legacy)
        return "OK\n";
      }
      if (parts[1] == "off") {
        subcycle_bridge_set_instr_trace(false);
        g_trace.disable();
        return "OK\n";
      }
    ...
    if (cmd == "trace")
      return "ERR 400 usage: trace (on|off|dump|on_crash|status)\n";
    ...
    if (cmd == "frames" && parts.size() >= 4 && parts[1] == "dump") {
      const std::string& pattern = parts[2];
      if (!is_safe_path(pattern)) return "ERR 403 path-traversal-blocked\n";
      int const frame_count = parse_int(parts[3]);
    ...
    if (cmd == "input" && parts.size() >= 2) {
      ...
      if (parts[1] == "joy" && parts.size() >= 4) {
        int const joy_num = parse_int(parts[2]);
        std::string dir = parts[3];
        for (auto& c : dir)
          c = static_cast<char>(std::toupper(static_cast<unsigned char>(c)));
        bool const release = (dir[0] == '-');
        if (release) dir = dir.substr(1);
        CPC_KEYS key;
        if (dir == "U" || dir == "UP") {
          key = (joy_num == 0) ? CPC_J0_UP : CPC_J1_UP;
        } else if (dir == "D" || dir == "DOWN") {
          key = (joy_num == 0) ? CPC_J0_DOWN : CPC_J1_DOWN;
        } else if (dir == "L" || dir == "LEFT") {
          key = (joy_num == 0) ? CPC_J0_LEFT : CPC_J1_LEFT;
        } else if (dir == "R" || dir == "RIGHT") {
          key = (joy_num == 0) ? CPC_J0_RIGHT : CPC_J1_RIGHT;
        } else if (dir == "F" || dir == "F1" || dir == "FIRE1") {
          key = (joy_num == 0) ? CPC_J0_FIRE1 : CPC_J1_FIRE1;
        } else if (dir == "F2" || dir == "FIRE2") {
          key = (joy_num == 0) ? CPC_J0_FIRE2 : CPC_J1_FIRE2;
        } else if (dir == "0") {
          // Release all directions
          for (auto k : {CPC_J0_UP, CPC_J0_DOWN, CPC_J0_LEFT, CPC_J0_RIGHT,
                         CPC_J0_FIRE1, CPC_J0_FIRE2}) {
            CPC_KEYS const jk =
                (joy_num == 0)
                    ? k
                    : static_cast<CPC_KEYS>(k + (CPC_J1_UP - CPC_J0_UP));
            ipc_apply_keypress(CPC.InputMapper->CPCscancodeFromCPCkey(jk),
                               keyboard_matrix, false);
          }
          return "OK\n";
        } else {
          return "ERR 400 bad-dir\n";
        }
        CPCScancode const scancode =
            CPC.InputMapper->CPCscancodeFromCPCkey(key);
    ...
    if (cmd == "wait" && parts.size() >= 2) {
      ...
      if (parts[1] == "mem" && parts.size() >= 4) {
        unsigned int const addr = parse_number(parts[2]);
        unsigned int const val = parse_number(parts[3]);
        ...
        if (parts.size() >= 5) {
          if (parts[4].rfind("mask=", 0) == 0) {
            ...
            if (parts.size() >= 6) {
              deadline = std::chrono::steady_clock::now() +
                         std::chrono::milliseconds(parse_int(parts[5]));
            }
            ...
            deadline = std::chrono::steady_clock::now() +
                       std::chrono::milliseconds(parse_int(parts[4]));
          }
        }
        cpc_resume();
        while (true) {
          byte const memv = z80_read_mem(static_cast<word>(addr));
      ...
      if (parts[1] == "bp") {
        if (parts.size() >= 3)
          deadline = std::chrono::steady_clock::now() +
                     std::chrono::milliseconds(parse_int(parts[2]));
        while (true) {
          if (g_ipc_instance) {
            uint16_t pc = 0;
            bool watch = false;
            if (g_ipc_instance->consume_breakpoint_hit(pc, watch)) {
              ...
              if (watch) {
                snprintf(
                    resp, sizeof(resp),
                    "OK PC=%04X WATCH=1 WP_ADDR=%04X WP_VAL=%02X WP_OLD=%02X\n",
                    pc, z80.watchpoint_addr, z80.watchpoint_value,
                    z80.watchpoint_old);
              } else {
    ...
    if (cmd == "event" && parts.size() >= 2) {
      if (parts[1] == "on" && parts.size() >= 4) {
        ...
        const std::string& trigger_str = parts[2];
        ...
        std::string const event_cmd = line.substr(cmd_start);
        ...
        if (trigger_str.rfind("pc=", 0) == 0) {
          ...
          ev.address =
              static_cast<uint16_t>(parse_number(trigger_str.substr(3)));
        } else if (trigger_str.rfind("mem=", 0) == 0) {
          ev.trigger = EventTrigger::MEM_WRITE;
          std::string const addr_val = trigger_str.substr(4);
          auto colon = addr_val.find(':');
          if (colon != std::string::npos) {
            ev.address =
                static_cast<uint16_t>(parse_number(addr_val.substr(0, colon)));
            ev.value =
                static_cast<uint8_t>(parse_number(addr_val.substr(colon + 1)));
            ev.match_value = true;
          } else {
        ...
        int const id = g_ipc_instance->add_event(ev);
        char buf[32];
      ...
      if (parts[1] == "off" && parts.size() >= 3) {
        int const id = parse_int(parts[2]);
      ...
      if (parts[1] == "list") {
        ...
        for (const auto& e : evts) {
          ...
          if (e.trigger == EventTrigger::PC)
            trig_name = "pc";
          else if (e.trigger == EventTrigger::MEM_WRITE)
            trig_name = "mem";
          else if (e.trigger == EventTrigger::VBL)
            trig_name = "vbl";
    ...
    if (cmd == "timer" && parts.size() >= 2) {
      if (parts[1] == "list") {
        ...
        for (const auto& [id, t] : timers) {
          uint32_t const avg =
              t.count > 0 ? static_cast<uint32_t>(t.total_us / t.count) : 0;
          resp << " id=" << id << " count=" << t.count << " last=" << t.last_us
    ...
    if (cmd == "wp" && parts.size() >= 2) {
      if (parts[1] == "add" && parts.size() >= 3) {
        ...
        for (size_t pi = 3; pi < parts.size(); pi++) {
          ...
          if (kwl == "r") {
            ...
            // Try as length
            try {
              unsigned int const v = parse_number(kw);
      ...
      if (parts[1] == "list") {
        ...
        for (size_t i = 0; i < wps.size(); i++) {
          ...
          resp << " " << i << ":" << std::hex << std::uppercase
               << std::setfill('0') << std::setw(4)
               << static_cast<unsigned>(w.address) << "+" << std::dec
    ...
    if (cmd == "sym" && parts.size() >= 2) {
      ...
      if (parts[1] == "list") {
        ...
        for (const auto& [addr, name] : syms) {
          resp << "  " << std::hex << std::uppercase << std::setfill('0')
               << std::setw(4) << static_cast<unsigned>(addr) << " " << name
               << "\n";
        }
        return resp.str();
      }
      if (parts[1] == "lookup" && parts.size() >= 3) {
        ...
        try {
          ...
          std::string const name =
              g_symfile.lookupAddr(static_cast<word>(addr));
          if (!name.empty()) {
            return "OK " + name + "\n";
          }
        } catch (const std::exception&) {
      ...
      if (parts[2] == "hex" && parts.size() >= 6) {
        ...
        for (size_t i = 0; i + 1 < hex.size(); i += 2) {
          if (hex[i] == '?' && hex[i + 1] == '?') {
            ...
            pattern.push_back(
                static_cast<int>(std::stoul(hex.substr(i, 2), nullptr, 16)));
          }
        }
        ...
        resp << "OK";
        int found = 0;
        for (unsigned int addr = start;
             addr + pattern.size() - 1 <= end && found < 32; addr++) {
          bool match = true;
      ...
      if (parts[2] == "asm" && parts.size() >= 6) {
        ...
        for (unsigned int addr = start; addr <= end && found < 32;) {
          ...
          if (lower_pattern.find('*') != std::string::npos) {
            ...
            std::string const prefix = lower_pattern.substr(0, star);
            std::string const suffix = lower_pattern.substr(star + 1);
            match = (lower_instr.rfind(prefix, 0) == 0);
            if (match && !suffix.empty()) {
              match = (lower_instr.size() >= suffix.size() &&
                       lower_instr.substr(lower_instr.size() - suffix.size()) ==
                           suffix);
            }
          } else {
    ...
    if (cmd == "mem")
      return "ERR 400 usage: mem "
             "(read|write|fill|compare|cpu-read|cpu-write|find) ...\n";
    ...
    if (cmd == "stack") {
      ...
      for (int i = 0; i < depth; i++) {
        word const addr = static_cast<word>(sp + (i * 2));
        byte const lo = z80_read_mem(addr);
        byte const hi = z80_read_mem(static_cast<word>(addr + 1));
        word const val = static_cast<word>((hi << 8) | lo);
        char buf[64];
        snprintf(buf, sizeof(buf), "  SP+%d: %04X", i * 2,
        ...
        std::string const sym = g_symfile.lookupAddr(val);
        if (!sym.empty()) resp << " " << sym;
        resp << "\n";
      }
      return resp.str();
    ...
    if (cmd == "autotype") {
      if (parts.size() >= 2 && parts[1] == "status") {
        if (g_autotype_queue.is_active()) {
          return "OK active: " + std::to_string(g_autotype_queue.remaining()) +
                 " actions remaining\n";
        }
        return "OK idle\n";
      }
    ...
    if (cmd == "record" && parts.size() >= 2) {
      if (parts[1] == "wav") {
        if (parts.size() < 3)
          return "ERR 400 missing-action (start|stop|status)\n";
        if (parts[2] == "start") {
          if (parts.size() < 4) return "ERR 400 missing-path\n";
          uint32_t const rate = SAMPLE_RATES[CPC.snd_playback_rate];
          uint16_t const bits = CPC.snd_bits ? 16 : 8;
          uint16_t const channels = CPC.snd_stereo ? 2 : 1;
        ...
        if (parts[2] == "stop") {
          ...
          std::string const path = g_wav_recorder.current_path();
          uint32_t const bytes = g_wav_recorder.stop();
      ...
      if (parts[1] == "ym") {
        ...
        if (parts[2] == "stop") {
          ...
          std::string const path = g_ym_recorder.current_path();
          uint32_t const frames = g_ym_recorder.stop();
      ...
      if (parts[1] == "avi") {
        ...
        if (parts[2] == "start") {
          ...
          uint16_t const channels = CPC.snd_stereo ? 2 : 1;
          auto err =
              g_avi_recorder.start(parts[3], quality, rate, channels, bits);
          if (err.empty()) return "OK\n";
          return "ERR " + err + "\n";
        }
        if (parts[2] == "stop") {
          if (!g_avi_recorder.is_recording()) return "ERR not-recording\n";
          std::string const path = g_avi_recorder.current_path();
          uint32_t const frames = g_avi_recorder.stop();
          return "OK " + path + " " + std::to_string(frames) + "\n";
        }
        if (parts[2] == "status") {
          if (g_avi_recorder.is_recording()) {
            return "OK recording " + g_avi_recorder.current_path() +
                   " frames=" + std::to_string(g_avi_recorder.frame_count()) +
                   " bytes=" + std::to_string(g_avi_recorder.bytes_written()) +
                   "\n";
          }
          return "OK idle\n";
        }
    ...
    if (cmd == "poke" && parts.size() >= 2) {
      if (parts[1] == "load" && parts.size() >= 3) {
        // Reconstruct path (may contain spaces if quoted, but split_ws breaks
        // on space) For simplicity, take everything after "poke load "
        size_t const pos = line.find("load ");
        if (pos == std::string::npos) return "ERR 400 bad-args\n";
        ...
        if (err.empty()) {
          return "OK loaded " + std::to_string(g_poke_manager.games().size()) +
                 " games\n";
        }
        return "ERR " + err + "\n";
      }
      if (parts[1] == "list") {
        ...
        for (const auto& game : games) {
          resp << game.title << "\n";
          for (const auto& poke : game.pokes) {
            resp << "  Poke: " << poke.description;
            resp << " [" << poke.values.size() << " value";
            if (poke.values.size() != 1) resp << "s";
            resp << "]";
            if (poke.applied) resp << " [applied]";
            resp << "\n";
          }
        }
        return resp.str();
      ...
      if (parts[1] == "apply" && parts.size() >= 3) {
        ...
        if (parts.size() >= 4 && parts[3] == "all") {
          int total_vals = 0;
          int const n = g_poke_manager.apply_all(
              game_idx,
              [](uint16_t a, uint8_t v) {
                z80_write_mem(static_cast<word>(a), v);
              },
              [](uint16_t a) -> uint8_t {
                return z80_read_mem(static_cast<word>(a));
              },
              &total_vals);
          if (n < 0) return "ERR invalid game index\n";
          return "OK applied " + std::to_string(n) + " pokes (" +
                 std::to_string(total_vals) + " values total)\n";
        }
        if (parts.size() >= 4) {
          size_t poke_idx;
          try {
          ...
          int const n = g_poke_manager.apply(
              game_idx, poke_idx,
              [](uint16_t a, uint8_t v) {
                z80_write_mem(static_cast<word>(a), v);
              },
      ...
      if (parts[1] == "unapply" && parts.size() >= 4) {
        ...
        int const n = g_poke_manager.unapply(
            game_idx, poke_idx, [](uint16_t a, uint8_t v) {
              z80_write_mem(static_cast<word>(a), v);
            });
    ...
    if (cmd == "profile") {
      if (parts.size() < 2)
        return "ERR 400 missing subcommand (list|current|load|save|delete)\n";
      if (parts[1] == "list") {
        auto names = g_profile_manager.list();
        std::string const cur = g_profile_manager.current();
        std::ostringstream resp;
        resp << "OK\n";
      ...
      return "ERR 400 unknown profile subcommand "
             "(list|current|load|save|delete)\n";
    }
      ...
      if (parts[1] == "get" && parts.size() >= 3) {
        ...
        if (parts[2] == "crtc_info") {
          ...
          snprintf(buf, sizeof(buf), "OK type=%d chip=%s manufacturer=%s",
                   CRTC.crtc_type, crtc_type_chip_name(CRTC.crtc_type),
      ...
      if (parts[1] == "set" && parts.size() >= 4) {
        if (parts[2] == "crtc_type") {
          ...
          if (subcycle::Machine* m = subcycle_bridge_machine())
            m->set_crtc_type(static_cast<uint8_t>(t));
          return "OK\n";
        }
        if (parts[2] == "ram_size") {
          int const sz = parse_int(parts[3]);
        ...
        if (parts[2] == "m4_sd_path") {
          ...
          size_t const pos = line.find("m4_sd_path ");
          if (pos == std::string::npos) return "ERR 400 bad-args\n";
    ...
    if (cmd == "sdisc" && parts.size() >= 2) {
      if (parts[1] == "status") {
        std::string const status =
            g_silicon_disc.enabled ? "enabled" : "disabled";
        std::string const allocated =
            g_silicon_disc.data ? "allocated" : "not-allocated";
    ...
    if (cmd == "serial") {
      ...
      if (parts[1] == "config") {
        if (parts.size() == 3 && parts[2] == "get") {
          ...
          ss << "OK enabled=" << (cfg.enabled ? 1 : 0)
             << " backend=" << static_cast<int>(cfg.backend_type)
             << " device=" << cfg.device_path << " tcp_host=" << cfg.tcp_host
             << " tcp_port=" << cfg.tcp_port << " baud=" << cfg.baud_rate
             << "\n";
          return ss.str();
        }
        if (parts.size() >= 5 && parts[2] == "set") {
          ...
          if (parts[3] == "backend") {
            auto cfg = g_serial_interface.get_config();
            cfg.backend_type =
                static_cast<SerialBackendType>(parse_int(parts[4]));
            g_serial_interface.set_config(cfg);
            g_serial_interface.apply_config();
    ...
    if (cmd == "plotter") {
      ...
      if (parts[1] == "status") {
        ...
        ss << "OK pen=" << pv.pen << " pen_down=" << (pv.pen_down ? 1 : 0)
           << " x=" << pv.x << " y=" << pv.y
           << " segments=" << plotter_view_segments().size() << "\n";
        return ss.str();
      }
      if (parts[1] == "export") {
        std::string const path =
            (parts.size() >= 3) ? parts[2] : "plotter_output.svg";
    ...
    if (cmd == "search" && parts.size() >= 3) {
      ...
      if (submode == "hex")
        ...
        return "ERR 400 bad-search-mode (hex|text|asm)\n";
      ...
      if (mode == SearchMode::ASM) {
        ...
        for (unsigned int addr = 0; addr <= 0xFFFF && found < 256;) {
          ...
          size_t gpi = 0, gti = 0;
          size_t const plen = lower_pattern.size();
          size_t const tlen = lower_instr.size();
          size_t star_p = std::string::npos, star_t = 0;
          while (gti < tlen) {
            if (gpi < plen && (lower_pattern[gpi] == lower_instr[gti] ||
                               lower_pattern[gpi] == '?')) {
              gpi++;
              gti++;
    ...
    if (cmd == "rom" && parts.size() >= 2) {
      ...
      if (parts[1] == "load" && parts.size() >= 4) {
        ...
        bool const has_amsdos =
            (checksum == ((header[0x43] << 8) + header[0x44]));
        if (has_amsdos) {
          if (fread(header, 128, 1, f) != 1) {
            fclose(f);
        ...
        // Validate ROM type byte (0=foreground, 1=background, 2=extension) or
        // Graduate 'G'
        bool const valid_rom = !(header[0] & 0xfc);
        bool const graduate_rom = (header[0] == 0x47);
        ...
        size_t const remaining = 16384 - 128;
        ...
        fclose(f);
        // Free old ROM data if present (but not for slots 0/1 which are system
        // ROMs)
      ...
      if (parts[1] == "info" && parts.size() >= 3) {
        ...
        char buf[256];
        snprintf(buf, sizeof(buf),
                 "OK slot=%d loaded=true size=16384 crc=%08lX path=%s\n", slot,
                 static_cast<unsigned long>(crc), CPC.rom_file[slot].c_str());
        return {buf};
      }
    ...
    if (cmd == "data" && parts.size() >= 2) {
      if (parts[1] == "mark" && parts.size() >= 5) {
        unsigned int const start = parse_number(parts[2]);
        unsigned int const end = parse_number(parts[3]);
        if (start > 0xFFFF || end > 0xFFFF || start > end)
          return "ERR 400 bad-range\n";
        DataType dtype;
        if (parts[4] == "bytes")
          dtype = DataType::BYTES;
        else if (parts[4] == "words")
          dtype = DataType::WORDS;
        else if (parts[4] == "text")
          dtype = DataType::TEXT;
        else
          return "ERR 400 bad-type (bytes|words|text)\n";
        std::string label;
        if (parts.size() >= 6) {
        ...
        g_data_areas.mark(static_cast<uint16_t>(start),
                          static_cast<uint16_t>(end), dtype, label);
        return "OK\n";
      }
      if (parts[1] == "clear") {
      ...
      if (parts[1] == "list") {
        ...
        for (const auto& a : areas) {
          ...
          if (a.type == DataType::WORDS)
            type_str = "words";
          else if (a.type == DataType::TEXT)
            type_str = "text";
    ...
    if (cmd == "gfx" && parts.size() >= 2) {
      if (parts[1] == "view" && parts.size() >= 6) {
        ...
        try {
          unsigned int const addr = parse_number(parts[2]);
          int const w = parse_int(parts[3]);
          int const h = parse_int(parts[4]);
          int const mode = parse_int(parts[5]);
          ...
          int const pixel_width =
              gfx_decode(pbRAM, CPC.ram_size * 1024, params, palette, pixels);
      ...
      if (parts[1] == "decode" && parts.size() >= 4) {
        ...
        try {
          unsigned int const byte_val = std::stoul(parts[2], nullptr, 16);
          int const mode = parse_int(parts[3]);
          uint8_t indices[8];
          int const count =
              gfx_decode_byte(static_cast<uint8_t>(byte_val), mode, indices);
      ...
      if (parts[1] == "paint" && parts.size() >= 8) {
        ...
        // Note: parts indices are paint=1, addr=2, w=3, h=4, mode=5, x=6, y=7,
        // color=8 We need 9 parts total for the paint command
        if (parts.size() < 9)
          return "ERR 400 usage: gfx paint <addr> <w> <h> <mode> <x> <y> "
                 "<color>\n";
        try {
          unsigned int const addr = parse_number(parts[2]);
          int const w = parse_int(parts[3]);
          int const h = parse_int(parts[4]);
          int const mode = parse_int(parts[5]);
          int const x = parse_int(parts[6]);
          int const y = parse_int(parts[7]);
          int const color = parse_int(parts[8]);
      ...
      if (parts[1] == "palette") {
        ...
        // surface used by `gfx decode`. Printing that uint32 with %08X,
        // however, emits it big-end-first (#AABBGGRR), transposing the red and
        // blue channels in the text. Unpack the bytes and print them in true
        ...
        resp << "OK\n";
        for (int i = 0; i < 16; i++) {
          uint32_t const c = palette[i];
          unsigned const r = c & 0xFF;
          unsigned const g = (c >> 8) & 0xFF;
          unsigned const b = (c >> 16) & 0xFF;
          unsigned const a = (c >> 24) & 0xFF;
          char buf[32];
    ...
    if (cmd == "session" && parts.size() >= 2) {
      if (parts[1] == "record" && parts.size() >= 3) {
        ...
        std::string const snap_path = parts[2] + ".sna";
      ...
      if (parts[1] == "status") {
        ...
        if (g_session.state() == SessionState::RECORDING)
          state_str = "recording";
        else if (g_session.state() == SessionState::PLAYING)
          state_str = "playing";
        char buf[256];
        snprintf(
            buf, sizeof(buf), "OK state=%s frames=%u events=%u path=%s",
    ...
    if (cmd == "asm" && parts.size() >= 2) {
      if (parts[1] == "text" && parts.size() >= 3) {
        // "asm text <source>" — set assembler source (rest of line after "asm
        // text ")
      ...
      if (parts[1] == "assemble") {
        ...
        if (r.success) {
          ...
          // Export symbols
          for (auto& [name, addr] : r.symbols) g_symfile.addSymbol(addr, name);
          return {buf};
        }
        char buf[64];
    ...
    if (cmd == "m4" && parts.size() >= 2) {
      if (parts[1] == "status") {
        ...
        for (auto& open_file : g_m4board.open_files) {
          if (open_file) open_files++;
        }
        std::ostringstream oss;
        oss << "OK enabled=" << (g_m4board.enabled ? 1 : 0) << " sd_path="
            << (g_m4board.sd_root_path.empty() ? "(none)"
                                               : g_m4board.sd_root_path)
            << " dir=" << g_m4board.current_dir << " files=" << open_files
            << "/4"
      ...
      if (parts[1] == "ls") {
        ...
        // Inside a DSK container — list files from the disk image, not the host
        // FS
      ...
      if (parts[1] == "cd" && parts.size() >= 3) {
        ...
        if ((path == ".." || path == "../") &&
            g_m4board.container_type != M4Board::ContainerType::NONE) {
          std::string const parent = g_m4board.container_parent_dir;
          if (g_m4board.container_drive) {
            dsk_eject(g_m4board.container_drive);
            delete g_m4board.container_drive;
            g_m4board.container_drive = nullptr;
        ...
        try {
          ...
          auto rel = canonical.lexically_normal().lexically_relative(
              root.lexically_normal());
          if (rel.empty() || (!rel.empty() && *rel.begin() == ".."))
            return "ERR 403 path-traversal\n";
          if (!std::filesystem::is_directory(canonical))
            return "ERR 404 not-a-directory\n";
          std::string rel_str;
          if (rel == ".") {
            rel_str = "/";
          } else {
      ...
      if (parts[1] == "http") {
        if (parts.size() < 3 || parts[2] == "status") {
          ...
          oss << "OK running=" << (g_m4_http.is_running() ? 1 : 0)
              << " port=" << g_m4_http.port() << " bind=" << g_m4_http.bind_ip()
              << "\n";
          return oss.str();
        }
        if (parts[2] == "start") {
      ...
      if (parts[1] == "port" && parts.size() >= 3) {
        if (parts[2] == "set" && parts.size() >= 5) {
          try {
            unsigned long const cpc_val = std::stoul(parts[3]);
            unsigned long const host_val = std::stoul(parts[4]);
            ...
            g_m4_http.set_port_mapping(static_cast<uint16_t>(cpc_val),
    ...
    // Unknown command — find closest match for "did you mean?" suggestion
    {
      ...
      for (const auto& kv : g_ipc_commands) {
        ...
        size_t const m = name.size();
        if (n > MAX_CMD_LEN || m > MAX_CMD_LEN) continue;
        for (size_t j = 0; j <= m; j++) prev[j] = j;
        for (size_t i = 1; i <= n; i++) {
          curr[0] = i;
          for (size_t j = 1; j <= m; j++) {
            size_t const cost = (cmd[i - 1] == name[j - 1]) ? 0 : 1;
    ...
    return err_with_context(
        400, "number-out-of-range (value too large for target type)");
  }
}
}  // namespace

KoncepcjaIpcServer::~KoncepcjaIpcServer() { stop(); }
...
void KoncepcjaIpcServer::run() {
  ...
  if (bound_port == 0) {
    LOG_ERROR("IPC: could not bind to any port in range "
              << kBasePort << "-" << (kBasePort + kMaxPortAttempts - 1));
  ...
  while (running.load()) {
    ...
    // Persistent connection: read lines until client disconnects, sends
    ...
    // Dispatch any trailing data without newline (for single-shot clients like
    // echo|nc)
...
void KoncepcjaIpcServer::run() {
  ...
  while (running.load()) {
    ...
    int const ready = select(server_fd + 1, &readfds, nullptr, nullptr, &tv);
    if (ready <= 0) continue;
    ...
    while (running.load() && !client_quit) {
      ...
      int const cready = select(client_fd + 1, &cfds, nullptr, nullptr, &ctv);
      if (cready <= 0) break;  // timeout or error

      ssize_t const n = read(client_fd, buf, sizeof(buf) - 1);
      if (n <= 0) break;  // client disconnected or error
      buf[n] = 0;
      buffer.append(buf);
