...
static void ipc_apply_keypress(CPCScancode cpc_key,
      ...
      if (parts[1] == "export" && parts.size() >= 4) {
        ...
        while (pos <= end_pos) {
          ...
          if (da) {
            ...
            int line_bytes = 0;
            std::string formatted = g_data_areas.format_at(
    ...
    if (cmd == "poke" && parts.size() >= 2) {
      ...
      if (parts[1] == "apply" && parts.size() >= 3) {
        ...
        if (parts.size() >= 4 && parts[3] == "all") {
          int total_vals = 0;
          int n = g_poke_manager.apply_all(
              game_idx,
    ...
    if (cmd == "plotter") {
      ...
      if (parts[1] == "status") {
        ...
        ss << "OK pen=" << g_plotter.selected_pen()
           ...
           << " x=" << g_plotter.pen_x() << " y=" << g_plotter.pen_y()
      ...
      for (const auto& kv : g_ipc_commands) {
        ...
        for (size_t i = 1; i <= n; i++) {
          ...
          for (size_t j = 1; j <= m; j++) {
            ...
            size_t del = prev[j] + 1, ins = curr[j - 1] + 1,
                   sub = prev[j - 1] + cost;
            curr[j] =
