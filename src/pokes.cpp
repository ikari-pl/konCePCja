...
const std::vector<PokeGame>& PokeManager::games() const { return games_; }

void PokeManager::clear() { games_.clear(); }
...
std::string PokeManager::parse_pok(const std::string& content) {
  ...
  while (std::getline(stream, line)) {
    ...
    std::string const rest = line.substr(1);
...
int PokeManager::apply(size_t game_idx, size_t poke_idx, WriteFn write_fn,
                       ReadFn read_fn) {
...
int PokeManager::apply_all(size_t game_idx, WriteFn write_fn, ReadFn read_fn,
                           int* total_values) {
  if (game_idx >= games_.size()) return -1;
  auto& game = games_[game_idx];
  int pokes_applied = 0;
  ...
  for (size_t i = 0; i < game.pokes.size(); i++) {
    int const n = apply(game_idx, i, write_fn, read_fn);
    if (n > 0) {
      pokes_applied++;
      values_total += n;
    }
