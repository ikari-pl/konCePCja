...
static void generate_motor_samples(std::vector<int16_t>& out) {
  ...
  for (int i = 0; i < len; i++) {
    ...
    double v = sin(2 * M_PI * 50 * t) * 0.4 + sin(2 * M_PI * 100 * t) * 0.2 +
...
static void generate_seek_samples(std::vector<int16_t>& out) {
  ...
  for (int i = 0; i < len; i++) {
    ...
    double v =
        sin(2 * M_PI * 800 * t) * env;  // click with high-frequency component
    v += (rand() / static_cast<double>(RAND_MAX) - 0.5) * 0.3 *
         env;  // mechanical noise
