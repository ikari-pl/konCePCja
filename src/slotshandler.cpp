...
inline bool fillSlot(t_slot& slot, bool& processedvar,
                     const std::string& fullpath, const std::string& extension,
                     const std::string& type_ext,
...
void fillSlots(std::vector<std::string> slot_list, t_CPC& CPC) {
  ...
  for (const auto& slot : slot_list) {  // loop for all command line arguments
    ...
    std::string fullpath =
        stringutils::trim(slot, '"');  // remove quotes if arguments quoted
    if (fullpath.length() > 5) {       // minumum for a valid filename
      ...
      if (extension == ".zip") {  // are we dealing with a zip archive?
        ...
        extension =
            stringutils::lower(filename.substr(pos));  // grab the extension
      }

      if (fillSlot(CPC.driveA, have_DSKA, fullpath, extension, ".dsk",
                   "drive A disk"))
        continue;
      if (fillSlot(CPC.driveA, have_DSKA, fullpath, extension, ".ipf",
                   "drive A disk (IPF)"))
        continue;
      if (fillSlot(CPC.driveA, have_DSKA, fullpath, extension, ".raw",
                   "drive A disk (CT-RAW)"))
        continue;
      if (fillSlot(CPC.driveB, have_DSKB, fullpath, extension, ".dsk",
                   "drive B disk"))
        continue;
      if (fillSlot(CPC.driveB, have_DSKB, fullpath, extension, ".ipf",
                   "drive B disk (IPF)"))
        continue;
      if (fillSlot(CPC.driveB, have_DSKB, fullpath, extension, ".raw",
                   "drive B disk (CT-IPF)"))
        continue;
      if (fillSlot(CPC.snapshot, have_SNA, fullpath, extension, ".sna",
                   "CPC state snapshot"))
        continue;
      if (fillSlot(CPC.tape, have_TAP, fullpath, extension, ".cdt",
                   "tape (CDT)"))
        continue;
      if (fillSlot(CPC.tape, have_TAP, fullpath, extension, ".voc",
                   "tape (VOC)"))
        continue;
      if (fillSlot(CPC.cartridge, have_CPR, fullpath, extension, ".cpr",
                   "cartridge"))
        continue;
    }
  }
...
t_disk_format parseDiskFormat(const std::string& format) {
  ...
  for (int iSide = 0; iSide < static_cast<int>(result.sides); iSide++) {
    for (int iSector = 0; iSector < static_cast<int>(result.sectors);
         iSector++) {
...
int snapshot_save(const std::string& filename) {
  ...
  sh.I = z80.I;
  if (z80.IFF1) sh.IFF0 = 1;
  if (z80.IFF2) sh.IFF1 = 1;
  sh.IX[1] = z80.IX.b.h;
  sh.IX[0] = z80.IX.b.l;
  sh.IY[1] = z80.IY.b.h;
  sh.IY[0] = z80.IY.b.l;
  ...
  sh.psg_env_step =
      PSG.AmplitudeEnv >> 1;  // divide by 2 to bring it into the 0 - 15 range
  if (PSG.FirstPeriod) {
  ...
  if ((pfileObject = fopen(filename.c_str(), "wb")) != nullptr) {
    if (fwrite(&sh, sizeof(sh), 1, pfileObject) !=
        1) {  // write snapshot header
      fclose(pfileObject);
      return ERR_SNA_WRITE;
    }
    if (fwrite(pbRAM, CPC.ram_size * 1024, 1, pfileObject) !=
        1) {  // write memory contents to snapshot file
      fclose(pfileObject);
      return ERR_SNA_WRITE;
    }
    fclose(pfileObject);
...
void dsk_eject(t_drive* drive) {
  ...
  for (track = 0; track < DSK_TRACKMAX; track++) {  // loop for all tracks
    for (side = 0; side < DSK_SIDEMAX; side++) {    // loop for all sides
      delete[] drive->track[track][side]
          .data;  // release memory allocated for this track
    }
  }
...
int dsk_load(FILE* pfile, t_drive* drive) {
  ...
  if (memcmp(pbPtr, "MV - CPC", 8) == 0) {  // normal DSK image?
    ...
    dwTrackSize = (*(pbPtr + 0x32) + (*(pbPtr + 0x33) << 8)) -
                  0x100;  // determine track size in bytes, minus track header
    ...
    for (track = 0; track < drive->tracks; track++) {  // loop for all tracks
      for (side = 0; side <= drive->sides; side++) {   // loop for all sides
        ...
        pbPtr = track_header;
        if (memcmp(pbPtr, "Track-Info", 10) !=
            0) {  // abort if ID does not match
          LOG_ERROR("Corrupted DSK track header for track " << track << " side "
                                                            << side);
          dsk_eject(drive);
          return ERR_DSK_INVALID;
        }
        dwSectorSize = 0x80
                       << *(pbPtr + 0x14);  // determine sector size in bytes
        dwSectors = *(pbPtr + 0x15);        // grab number of sectors
        if (dwSectors >
            DSK_SECTORMAX) {  // abort if sector count greater than maximum
          LOG_ERROR("DSK track with " << dwSectors << " sectors, expected "
        ...
        drive->track[track][side].data =
            new byte[dwTrackSize];  // attempt to allocate the required memory
        pbDataPtr = drive->track[track][side]
                        .data;  // pointer to start of memory buffer
        pbTempPtr = pbDataPtr;  // keep a pointer to the beginning of the buffer
                                // for the current track
        for (sector = 0; sector < dwSectors;
             sector++) {  // loop for all sectors
          memcpy(drive->track[track][side].sector[sector].CHRN, (pbPtr + 0x18),
                 4);  // copy CHRN
          memcpy(drive->track[track][side].sector[sector].flags, (pbPtr + 0x1c),
                 2);  // copy ST1 & ST2
          drive->track[track][side].sector[sector].setSizes(dwSectorSize,
                                                            dwSectorSize);
          drive->track[track][side].sector[sector].setData(
              pbDataPtr);  // store pointer to sector data
          pbDataPtr += dwSectorSize;
          pbPtr += 8;
        }
        if (dwTrackSize > 0 &&
            !fread(pbTempPtr, dwTrackSize, 1,
                   pfile)) {  // read entire track data in one go
          LOG_ERROR("Couldn't read track data for track " << track << " side "
                                                          << side);
          dsk_eject(drive);
          return ERR_DSK_INVALID;
        }
    ...
    if (memcmp(pbPtr, "EXTENDED", 8) == 0) {  // extended DSK image?
      ...
      drive->random_DEs =
          *(pbPtr + 0x31) & 0x80;          // simulate random Data Errors?
      ...
      for (track = 0; track < drive->tracks; track++) {  // loop for all tracks
        for (side = 0; side <= drive->sides; side++) {   // loop for all sides
          ...
          if (dwTrackSize != 0) {  // only process if track contains data
            ...
            byte track_header[0x100];
            if (fread(track_header, 0x100, 1, pfile) !=
                1) {  // read track header
              LOG_ERROR("Couldn't read DSK track header for track "
                        << track << " side " << side);
              dsk_eject(drive);
            ...
            if (memcmp(pbPtr, "Track-Info", 10) != 0) {  // valid track header?
              LOG_ERROR("Corrupted DSK track header for track "
                        << track << " side " << side);
              dsk_eject(drive);
              return ERR_DSK_INVALID;
            }
            ...
            drive->track[track][side].sectors =
                dwSectors;                                 // store sector count
            drive->track[track][side].size = dwTrackSize;  // store track size
            drive->track[track][side].data =
                new byte[dwTrackSize];  // attempt to allocate the required
                                        // memory
            pbDataPtr = drive->track[track][side]
                            .data;  // pointer to start of memory buffer
            pbTempPtr = pbDataPtr;  // keep a pointer to the beginning of the
                                    // buffer for the current track
            pbPtr += 0x18;
            for (sector = 0; sector < dwSectors;
                 sector++) {  // loop for all sectors
              memcpy(drive->track[track][side].sector[sector].CHRN, pbPtr,
                     4);  // copy CHRN
              memcpy(drive->track[track][side].sector[sector].flags,
                     (pbPtr + 0x04), 2);  // copy ST1 & ST2
              dword dwRealSize = 0x80 << *(pbPtr + 0x03);
              dwSectorSize = *(pbPtr + 0x6) +
                             (*(pbPtr + 0x7) << 8);  // sector size in bytes
              drive->track[track][side].sector[sector].setSizes(dwRealSize,
                                                                dwSectorSize);
              drive->track[track][side].sector[sector].setData(
                  pbDataPtr);  // store pointer to sector data
              pbDataPtr += dwSectorSize;
              pbPtr += 8;
              LOG_DEBUG("Sector "
                        << sector << " size: " << dwSectorSize
                        << " real size: " << dwRealSize << " CHRN: "
                        << chrn_to_string(
            ...
            if (dwTrackSize > 0 &&
                !fread(pbTempPtr, dwTrackSize, 1,
                       pfile)) {  // read entire track data in one go
              LOG_ERROR("Couldn't read track data for track "
                        << track << " side " << side);
              dsk_eject(drive);
              return ERR_DSK_INVALID;
            }
...
int dsk_save(const std::string& filename, t_drive* drive) {
  ...
  if ((pfileObject = fopen(filename.c_str(), "wb")) != nullptr) {
    ...
    dh.sides = (drive->sides + 1) |
               (drive->random_DEs);  // correct side count and indicate random
                                     // DEs, if necessary
    pos = 0;
    for (track = 0; track < drive->tracks; track++) {  // loop for all tracks
      for (side = 0; side <= drive->sides; side++) {   // loop for all sides
        ...
        if (drive->track[track][side].size) {          // track is formatted?
          ...
          for (sector = 0; sector < th.sectors; sector++) {
            ...
            memcpy(&th.sector[sector][4],
                   ...
                   2);  // copy ST1 & ST2
            th.sector[sector][6] =
                drive->track[track][side].sector[sector].getTotalSize() & 0xff;
            th.sector[sector][7] =
                (drive->track[track][side].sector[sector].getTotalSize() >> 8) &
                0xff;  // sector size in bytes
          }
          ...
          if (!fwrite(drive->track[track][side].data,
                      drive->track[track][side].size, 1,
                      pfileObject)) {  // write track data
            fclose(pfileObject);
            LOG_ERROR("Error while writing to the file '" << filename << "'");
            return ERR_DSK_WRITE;
          }
...
int dsk_format(t_drive* drive, int iFormat) {
  ...
  drive->sides--;  // zero base number of sides
  for (dword track = 0; track < drive->tracks;
       track++) {                                         // loop for all tracks
    for (dword side = 0; side <= drive->sides; side++) {  // loop for all sides
      dword dwSectorSize =
          0x80 << disk_format[iFormat]
                      .sector_size;  // determine sector size in bytes
      ...
      dword dwTrackSize =
          dwSectorSize *
          dwSectors;  // determine track size in bytes, minus track header
      drive->track[track][side].sectors = dwSectors;  // store sector count
      drive->track[track][side].size = dwTrackSize;   // store track size
      drive->track[track][side].data =
          new byte[dwTrackSize];  // attempt to allocate the required memory
      byte* pbDataPtr =
          drive->track[track][side].data;  // pointer to start of memory buffer
      ...
      for (dword sector = 0; sector < dwSectors;
           sector++) {  // loop for all sectors
        CHRN[2] = disk_format[iFormat].sector_ids[side][sector];
        memcpy(drive->track[track][side].sector[sector].CHRN, CHRN,
               4);  // copy CHRN
        drive->track[track][side].sector[sector].setSizes(dwSectorSize,
                                                          dwSectorSize);
        drive->track[track][side].sector[sector].setData(
...
void tape_eject() { pbTapeImage.clear(); }
...
int snapshot_load(FILE* pfile) {
  ...
  if (dwSnapSize >
      CPC.ram_size) {  // memory dump size differs from current RAM size?
    byte* pbTemp;
    ...
    pbRAMbuffer = pbTemp;
    // Ensure 1 byte is available before pbRAM as prerender_normal*_plus can
    // read it
    pbRAM = pbRAMbuffer + 1;
  }
  emulator_reset();
  n = fread(pbRAM, dwSnapSize * 1024, 1,
            pfile);  // read memory dump into CPC RAM
  if (!n) {
    emulator_reset();
  ...
  z80.I = sh.I;
  if (sh.IFF0) z80.IFF1 = Pflag;
  if (sh.IFF1) z80.IFF2 = Pflag;
  z80.IX.b.h = sh.IX[1];
  z80.IX.b.l = sh.IX[0];
  z80.IY.b.h = sh.IY[1];
  z80.IY.b.l = sh.IY[0];
  ...
  if (sh.version > 2) {  // does the snapshot have version 3 data?
    ...
    PSG.AmplitudeEnv = sh.psg_env_step
                       << 1;  // multiply by 2 to bring it into the 0 - 30 range
...
int tape_insert_cdt(FILE* pfile) {
  ...
  *reinterpret_cast<word*>(&pbTapeImage[1]) =
      2000;  // set the length to 2 seconds
  if (fread(&pbTapeImage[3], lFileSize, 1, pfile) !=
      1) {  // append the entire CDT file
    LOG_ERROR("Couldn't read CDT file");
    return ERR_TAP_INVALID;
  }
  *(&pbTapeImage[lFileSize + 3]) = 0x20;  // end with a pause block
  *reinterpret_cast<word*>(&pbTapeImage[lFileSize + 3 + 1]) =
      2000;  // set the length to 2 seconds
  ...
  while (pbBlock < pbTapeImageEnd) {
    ...
    switch (bID) {
      ...
      case 0x11:  // turbo loading data block
        ...
        iBlockLength =
            (*reinterpret_cast<dword*>(pbBlock + 0x0f) & 0x00ffffff) + 0x12;
        bolGotDataBlock = true;
        break;
      case 0x12:  // pure tone
        if (remaining() < 4) {
      ...
      case 0x14:  // pure data block
        ...
        iBlockLength =
            (*reinterpret_cast<dword*>(pbBlock + 0x07) & 0x00ffffff) + 0x0a;
        bolGotDataBlock = true;
        break;
      case 0x15:  // direct recording
        ...
        iBlockLength =
            (*reinterpret_cast<dword*>(pbBlock + 0x05) & 0x00ffffff) + 0x08;
        bolGotDataBlock = true;
        break;
      case 0x20:  // pause
        if (remaining() < 2) {
        ...
        if ((!bolGotDataBlock) && (pbBlock != &pbTapeImage[1])) {
          *reinterpret_cast<word*>(pbBlock) =
              0;  // remove any pauses (execept ours) before the data starts
        }
        iBlockLength = 2;
        break;
      ...
      case 0x40:  // snapshot block
        ...
        iBlockLength =
            (*reinterpret_cast<dword*>(pbBlock + 0x01) & 0x00ffffff) + 0x04;
        break;
      ...
      default:  // "extension rule"
        if (remaining() < 4) {
          LOG_ERROR("Truncated CDT block 0x" << std::hex
                                             << static_cast<int>(bID));
          return ERR_TAP_INVALID;
        }
    ...
    if (iBlockLength < 0 || pbBlock + iBlockLength > pbTapeImageEnd) {
      LOG_ERROR("CDT block 0x" << std::hex << static_cast<int>(bID)
...
int tape_insert_voc(FILE* pfile) {
  ...
  lOffset = lInitialOffset = *reinterpret_cast<word*>(pbPtr + 0x14);
  ...
  while ((!bolDone) && (lOffset < lFileSize)) {
    ...
    if (fread(pbPtr, 16, 1, pfile) != 1) {  // read block ID + size
      LOG_ERROR("Reading VOC file: Invalid VOC file: couldn't read block at "
                << lOffset);
      return ERR_TAP_BAD_VOC;
    }
#ifdef DEBUG_TAPE
    fprintf(pfoDebug, "%02x %d\r\n", *pbPtr,
            *(dword*)(pbPtr + 0x01) & 0x00ffffff);
#endif
    switch (*pbPtr) {
      case 0x0:  // terminator
        bolDone = true;
      ...
      case 0x1:  // sound data
        iBlockLength =
            (*reinterpret_cast<dword*>(pbPtr + 0x01) & 0x00ffffff) + 4;
        lSampleLength += iBlockLength - 6;
        if ((bSampleRate) &&
            (bSampleRate !=
             *(pbPtr + 0x04))) {  // no change in sample rate allowed
      ...
      case 0x3:  // silence
        ...
        if ((bSampleRate) &&
            (bSampleRate !=
             *(pbPtr + 0x03))) {  // no change in sample rate allowed
          LOG_ERROR("Reading VOC file: unsupported change in sample rate");
          return ERR_TAP_BAD_VOC;
        }
      ...
      case 0x9:  // sound data - new format
        ...
        return ERR_TAP_BAD_VOC;
      default:
        LOG_ERROR("Reading VOC file: unsupported unknown block type: "
                  << static_cast<int>(*pbPtr));
        return ERR_TAP_BAD_VOC;
    }
  ...
  dword dwTapePulseCycles =
      3500000L /
      (1000000L /
       (256 - bSampleRate));  // length of one pulse in ZX Spectrum T states
  dword dwCompressedSize = lSampleLength >> 3;  // 8x data reduction
  if (dwCompressedSize >
      0x00ffffff) {  // we only support one direct recording block right now
  ...
  *reinterpret_cast<word*>(&pbTapeImage[4]) =
      static_cast<word>(dwTapePulseCycles);  // number of T states per sample
  *reinterpret_cast<word*>(&pbTapeImage[6]) = 0;  // pause after block
  pbTapeImage[8] =
      lSampleLength & 7 ? lSampleLength & 7 : 8;  // bits used in last byte
  *reinterpret_cast<dword*>(&pbTapeImage[9]) =
      dwCompressedSize & 0x00ffffff;  // data length
  ...
  while ((!bolDone) && (lOffset < lFileSize)) {
    ...
    switch (*pbPtr) {
      case 0x0:  // terminator
        ...
        break;
      case 0x1:  // sound data
        if (fread(pbPtr, 3 + 2, 1, pfile) !=
            1) {  // get block size and sound info
  ...
  *reinterpret_cast<word*>(pbTapeImagePtr + 1) =
      2000;  // set the length to 2 seconds
