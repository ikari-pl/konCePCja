...
std::vector<const CommandEntry*> CommandPalette::filter_commands(
  ...
  for (const auto& cmd : commands_) {
    int const name_score = search_detail::fuzzy_score(query, cmd.name);
    int const desc_score = search_detail::fuzzy_score(query, cmd.description);
    int const best = std::max(name_score, desc_score);
    if (best > 0) {
      scored.emplace_back(best, &cmd);
    }
  }
...
void CommandPalette::render() {
  ...
  ImVec2 const display_size = io.DisplaySize;

  float const palette_w = std::min(600.0f, display_size.x * 0.8f);
  float const palette_h = std::min(400.0f, display_size.y * 0.7f);
  ImVec2 const pos((display_size.x - palette_w) * 0.5f, display_size.y * 0.15f);
  ...
  if (mode_ == 0) {
    ...
    for (size_t i = 0; i < filtered.size(); i++) {
      ...
      if (!cmd->shortcut.empty()) {
        float const shortcut_w = ImGui::CalcTextSize(cmd->shortcut.c_str()).x;
    ...
    ImGuiInputTextFlags const ipc_flags = ImGuiInputTextFlags_EnterReturnsTrue;
    bool const ipc_enter = ImGui::InputText("##IpcInput", ipc_input_buf_,
    ...
    if (ipc_enter && ipc_input_buf_[0] != '\0') {
      std::string const cmd(ipc_input_buf_);
