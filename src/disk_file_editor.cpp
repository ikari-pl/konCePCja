...
static uint8_t* find_sector_data(t_drive* drive, unsigned int track,
...
static bool read_directory(t_drive* drive,
...
static uint32_t compute_file_size(const uint8_t dir[],
...
static std::vector<uint8_t> get_file_blocks(const uint8_t dir[],
                                            const uint8_t* first_entry) {
  struct ExtentData {
    unsigned int extent_num;
