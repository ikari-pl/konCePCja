...
static constexpr unsigned int CPM_BLOCK_SIZE =
    1024;  // 1K blocks for DATA format
...
static constexpr unsigned int CPM_RECORDS_PER_EXTENT =
    128;  // 128 records of 128 bytes = 16K
static constexpr unsigned int CPM_RECORD_SIZE = 128;
static constexpr unsigned int CPM_TOTAL_BLOCKS =
    180;  // DATA format: 180 blocks total
static constexpr unsigned int CPM_EXTENT_SIZE = 16384;  // 16K per extent
namespace {
constexpr uint8_t CPM_DELETED_ENTRY = 0xE5;
}  // namespace
...
//   sector_offset = (N * 2) % 9
namespace {
bool read_block(t_drive* drive, unsigned int block, uint8_t* out) {
  if (block >= CPM_TOTAL_BLOCKS) return false;

  // Each block is 2 sectors (2 * 512 = 1024)
  unsigned int const first_sector = block * 2;

  // DATA format sector IDs: C1..C9 (0xC1..0xC9)
  for (int half = 0; half < 2; half++) {
    unsigned int const abs_sector = first_sector + half;
    unsigned int const track = abs_sector / CPM_SECTORS_PER_TRACK;
    unsigned int const sec_in_track = abs_sector % CPM_SECTORS_PER_TRACK;

    // Map logical sector index to sector ID
    uint8_t const sector_id = static_cast<uint8_t>(0xC1 + sec_in_track);

    uint8_t const* data = find_sector_data(drive, track, 0, sector_id);
...
namespace {
bool write_block(t_drive* drive, unsigned int block, const uint8_t* in) {
  if (block >= CPM_TOTAL_BLOCKS) return false;
...
namespace {
std::string format_cpm_name(const uint8_t* entry) {
  char name[9], ext[4];
...
namespace {
bool parse_cpm_name(const std::string& display, uint8_t out[11]) {
  std::memset(out, ' ', 11);

  std::string upper;
  for (char const c : display)
  ...
  for (size_t i = 0; i < name_part.size(); i++)
    out[i] = static_cast<uint8_t>(name_part[i]);
  for (size_t i = 0; i < ext_part.size(); i++)
    out[8 + i] = static_cast<uint8_t>(ext_part[i]);
  return true;
}
}  // namespace
...
namespace {
bool same_file(const uint8_t* a, const uint8_t* b) {
...
uint32_t compute_file_size(const uint8_t dir[],
  ...
  for (unsigned int i = 0; i < CPM_DIR_ENTRIES; i++) {
    ...
    unsigned int const ext_lo = entry[12];  // Extent low byte
    unsigned int const ext_hi = entry[14];  // Extent high byte (S2)
    unsigned int const extent_num = ext_lo + (ext_hi * 32);
    uint8_t const rc = entry[15];  // Record Count
...
std::vector<DiskFileEntry> disk_list_files(t_drive* drive, std::string& err) {
  ...
  for (unsigned int i = 0; i < CPM_DIR_ENTRIES; i++) {
    ...
    unsigned int const ext_lo = entry[12];
    unsigned int const ext_hi = entry[14];
    unsigned int const extent_num = ext_lo + (ext_hi * 32);

    std::string const display = format_cpm_name(entry);
    std::string const key = std::to_string(entry[0]) + ":" + display;
    ...
    for (unsigned int j = 0; j < CPM_DIR_ENTRIES; j++) {
      ...
      unsigned int const other_ext =
          other[12] + (static_cast<unsigned int>(other[14]) * 32);
      if (other_ext < extent_num) {
        has_lower = true;
        break;
      }
    }
...
std::vector<uint8_t> get_file_blocks(const uint8_t dir[],
  ...
  for (unsigned int i = 0; i < CPM_DIR_ENTRIES; i++) {
    ...
    // Block numbers are in bytes 16-31 (16 single-byte block numbers for DATA
    // format)
    for (int b = 16; b < 32; b++) {
      if (entry[b] != 0) {
...
std::vector<uint8_t> disk_read_file(t_drive* drive, const std::string& filename,
  ...
  for (unsigned int i = 0; i < CPM_DIR_ENTRIES; i++) {
    ...
    bool match = true;
    for (int j = 0; j < 11; j++) {
      if ((entry[1 + j] & 0x7F) != search_name[j]) {
        match = false;
        break;
      }
    }
  ...
  uint32_t const file_size = compute_file_size(dir, found);
  auto blocks = get_file_blocks(dir, found);

  // Read all blocks
  uint8_t block_buf[CPM_BLOCK_SIZE];
  uint32_t bytes_read = 0;
  for (uint8_t const block_num : blocks) {
...
AmsdosHeaderInfo disk_parse_amsdos_header(const std::vector<uint8_t>& data) {
  ...
  uint16_t const stored_checksum =
      static_cast<uint16_t>(data[67]) | (static_cast<uint16_t>(data[68]) << 8);
  ...
  info.load_addr =
      static_cast<uint16_t>(data[21]) | (static_cast<uint16_t>(data[22]) << 8);
  info.file_length = static_cast<uint32_t>(data[64]) |
                     (static_cast<uint32_t>(data[65]) << 8) |
                     (static_cast<uint32_t>(data[66]) << 16);
  info.exec_addr =
      static_cast<uint16_t>(data[26]) | (static_cast<uint16_t>(data[27]) << 8);
...
namespace {
std::vector<uint8_t> find_free_blocks(const uint8_t dir[]) {
...
namespace {
int find_free_dir_entry(const uint8_t dir[]) {
  for (unsigned int i = 0; i < CPM_DIR_ENTRIES; i++) {
...
std::string disk_write_file(t_drive* drive, const std::string& cpc_filename,
  ...
  uint32_t const total_bytes = static_cast<uint32_t>(full_data.size());
  uint32_t const blocks_needed =
      (total_bytes + CPM_BLOCK_SIZE - 1) / CPM_BLOCK_SIZE;
  ...
  // Calculate extents needed (16 blocks per extent for DATA format with 1K
  // blocks)
  uint32_t const blocks_per_extent =
      16;  // 16 block pointers per directory entry
  uint32_t const extents_needed =
      (blocks_needed + blocks_per_extent - 1) / blocks_per_extent;
  ...
  for (uint32_t b = 0; b < blocks_needed; b++) {
    std::memset(block_buf, 0xE5,
                CPM_BLOCK_SIZE);  // Fill with E5 (unused area marker)
    uint32_t to_copy = CPM_BLOCK_SIZE;
  ...
  for (uint32_t ext = 0; ext < extents_needed; ext++) {
    int const entry_idx = find_free_dir_entry(dir);
    ...
    for (uint32_t b = 0;
         b < blocks_per_extent && blocks_assigned + b < blocks_needed; b++) {
      entry[16 + b] = free_blocks[blocks_assigned + b];
      blocks_in_extent++;
      uint32_t block_bytes = CPM_BLOCK_SIZE;
      block_bytes = std::min(block_bytes, bytes_remaining);
    ...
    records = std::min(records, CPM_RECORDS_PER_EXTENT);
