...
enum class DataType : std::uint8_t { BYTES, WORDS, TEXT };
...
class DataAreaManager {
 public:
  void mark(uint16_t start, uint16_t end, DataType type,
  ...
  // mem points to data starting at addr (relative indexing: mem[0] is the byte
  // at addr). Returns empty string if addr is not in a data area. If
  // bytes_consumed is non-null, stores the number of bytes this line covers.
