...
namespace {
const uint64_t DS1216_PATTERN = 0x5CA33AC55CA33AC5ULL;
}  // namespace
...
void smartwatch_snapshot_time() {
  ...
  g_smartwatch.rtc_data[4] =
      (t->tm_wday == 0 ? 7 : t->tm_wday);               // day of week 1-7
...
byte smartwatch_rom_read(word addr, byte rom_byte) {
  bool const a0 = addr & 0x01;  // data bit (for pattern matching)
  bool const a2 = addr & 0x04;  // mode: 0=write(pattern), 1=read(data)
  ...
  switch (g_smartwatch.state) {
    case SmartWatch::IDLE:
      ...
      if (!a2) {
        ...
        g_smartwatch.shift_reg |=
            (static_cast<uint64_t>(a0) << g_smartwatch.bit_index);
    ...
    case SmartWatch::READING:
      if (a2) {
        ...
        int const byte_idx = g_smartwatch.bit_index / 8;
        int const bit_idx = g_smartwatch.bit_index % 8;
        byte const rtc_bit = (g_smartwatch.rtc_data[byte_idx] >> bit_idx) & 1;
        g_smartwatch.bit_index++;
        if (g_smartwatch.bit_index >= 64) {
          g_smartwatch.state = SmartWatch::IDLE;
          g_smartwatch.bit_index = 0;
