 ...
 private:
  ...
  std::unique_ptr<ExprNode> parse_atom() {
    ...
    if (cur.type == TokenType::IDENT) {
      std::string const name = cur.str_value;
      advance();
...
int32_t expr_eval(const ExprNode* node, const ExprContext& ctx) {
  ...
  switch (node->type) {
    ...
    case ExprNodeType::BINARY_OP: {
      int32_t const lv = expr_eval(node->left.get(), ctx);
      int32_t const rv = expr_eval(node->right.get(), ctx);
      switch (node->op) {
        case BinaryOp::ADD:
          return lv + rv;
        case BinaryOp::SUB:
...
namespace {
const char* binop_str(BinaryOp op) {
  switch (op) {
    case BinaryOp::ADD:
      return "+";
    case BinaryOp::SUB:
