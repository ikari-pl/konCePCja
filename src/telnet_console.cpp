...
namespace {
void txt_output_hook(uint8_t ch) { g_telnet.on_txt_output(ch); }
}  // namespace

// ── Ring buffer (SPSC) ─────────────────────────────────

void TelnetConsole::on_txt_output(uint8_t ch) {
  int const head = output_head.load(std::memory_order_relaxed);
  int const next = (head + 1) % OUTPUT_BUF_SIZE;
...
void TelnetConsole::drain_input() {
  std::string buf;
  {
    std::scoped_lock const lock(input_mutex);
...
TelnetConsole::~TelnetConsole() { stop(); }
...
void TelnetConsole::start(int base_port) {
  ...
  z80_set_bdos_output_hook(
      &txt_output_hook);  // CP/M: BDOS C_WRITE (C=2, char in E)
...
void TelnetConsole::run() {
  ...
  while (running.load()) {
    ...
    if (client_fd != INVALID_SOCKET) FD_SET(client_fd, &readfds);
    ...
    if (client_fd != INVALID_SOCKET && ready > 0 &&
        FD_ISSET(client_fd, &readfds)) {
      char buf[256];
      ...
      if (n <= 0) {
        ...
        size_t space = (pending_input.size() < MAX_PENDING_INPUT)
                           ? MAX_PENDING_INPUT - pending_input.size()
                           : 0;
        if (space > 0)
...
void TelnetConsole::run() {
  ...
  while (running.load()) {
    ...
    int const ready = select(maxfd + 1, &readfds, nullptr, nullptr, &tv);
    ...
    if (client_fd >= 0 && ready > 0 && FD_ISSET(client_fd, &readfds)) {
      char buf[256];
      ssize_t const n = ::read(client_fd, buf, sizeof(buf));
      if (n <= 0) {
        ::close(client_fd);
        client_fd = -1;
        client_connected.store(false);
        ...
        std::scoped_lock const lock(input_mutex);
        static constexpr size_t MAX_PENDING_INPUT = 4096;
        size_t const space = (pending_input.size() < MAX_PENDING_INPUT)
                                 ? MAX_PENDING_INPUT - pending_input.size()
                                 : 0;
        if (space > 0)
          pending_input.append(buf, std::min(static_cast<size_t>(n), space));
    ...
    if (client_fd >= 0) {
      int tail = output_tail.load(std::memory_order_relaxed);
      int const head = output_head.load(std::memory_order_acquire);
      if (tail != head) {
        uint8_t flush_buf[OUTPUT_BUF_SIZE];
        int count = 0;
        while (tail != head && count < OUTPUT_BUF_SIZE - 1) {
          uint8_t const ch = output_buf[tail];
          tail = (tail + 1) % OUTPUT_BUF_SIZE;
          // Convert CPC carriage return to telnet CR+LF
          if (ch == 0x0d) {
            flush_buf[count++] = '\r';
        ...
        if (count > 0) {
          ssize_t const sent =
              ::write(client_fd, flush_buf, static_cast<size_t>(count));
          if (sent < 0) {
            ::close(client_fd);
            client_fd = -1;
            client_connected.store(false);
