...
// Check if a string is a Z80 register name (used to distinguish regs from
// labels)
namespace {
bool is_register(const std::string& s) {
  static const char* const regs[] = {
      "A",    "B",    "C",    "D",    "E",   "H",    "L",    "F",
      "AF",   "BC",   "DE",   "HL",   "SP",  "IX",   "IY",   "IXH",
      "IXL",  "IYH",  "IYL",  "I",    "R",   "AF'",  "(HL)", "(BC)",
      "(DE)", "(SP)", "(IX)", "(IY)", "(C)", nullptr};
  std::string u = to_upper(s);
  for (int i = 0; regs[i]; i++)
    if (u == regs[i]) return true;
  // Also check (IX+*) patterns
  return u.size() >= 4 && u[0] == '(' &&
         (u.substr(1, 2) == "IX" || u.substr(1, 2) == "IY");
}
}  // namespace
...
bool tokenize_expr(const std::string& expr,
  ...
  try {
    while (i < expr.size()) {
      ...
      // They are prefixes when a value is expected (at start, after op, after
      // '(', after '~')
      bool const expect_value = tokens.empty() ||
      ...
      if (c == '&' && expect_value && i + 1 < expr.size() &&
          isxdigit(static_cast<unsigned char>(expr[i + 1]))) {
        i++;  // skip &
        size_t const start = i;
        while (i < expr.size() && isxdigit(static_cast<unsigned char>(expr[i])))
          i++;
        int32_t const val = static_cast<int32_t>(
            std::stoul(expr.substr(start, i - start), nullptr, 16));
      ...
      if (c == '%' && expect_value && i + 1 < expr.size() &&
        ...
        int32_t const val = static_cast<int32_t>(
            std::stoul(expr.substr(start, i - start), nullptr, 2));
        tokens.push_back({ExprTokenType::NUMBER, val, ""});
        continue;
      }
      ...
      if (c == '\'') {
        ...
        int32_t const val = static_cast<unsigned char>(expr[i + 1]);
        i += 2;
      ...
      if (c == '$') {
        ...
        if (i + 1 < expr.size() &&
          ...
          size_t const start = i;
          while (i < expr.size() &&
                 isxdigit(static_cast<unsigned char>(expr[i])))
            i++;
          int32_t const val = static_cast<int32_t>(
              std::stoul(expr.substr(start, i - start), nullptr, 16));
          tokens.push_back({ExprTokenType::NUMBER, val, ""});
      ...
      if (c == '#') {
        ...
        while (i < expr.size() && isxdigit(static_cast<unsigned char>(expr[i])))
          i++;
        if (i == start) {
          error = "expected hex digit after #";
          return false;
        }
        int32_t const val = static_cast<int32_t>(
            std::stoul(expr.substr(start, i - start), nullptr, 16));
      ...
      // Note: & hex prefix and % binary prefix are handled above (before
      // operators)

      // 0x hex prefix
      if (c == '0' && i + 1 < expr.size() &&
      ...
      if (isdigit(static_cast<unsigned char>(c))) {
        ...
        if (i < expr.size() && (expr[i] == 'h' || expr[i] == 'H')) {
          ...
          while (i < expr.size() &&
            ...
            i++;
          int32_t const val =
              static_cast<int32_t>(std::stol(expr.substr(start, i - start)));
          tokens.push_back({ExprTokenType::NUMBER, val, ""});
        }
        continue;
      }
      ...
      if (isalpha(static_cast<unsigned char>(c)) || c == '_' || c == '.') {
        ...
        tokens.push_back(
            {ExprTokenType::SYMBOL, 0, expr.substr(start, i - start)});
        continue;
      }
...
bool Z80Assembler::eval_expr(const std::string& expr,
                             ...
                             std::string& error) {
  std::string const trimmed = trim(expr);
  if (trimmed.empty()) {
  ...
  // Recursive descent with left-to-right evaluation (no operator precedence —
  // Maxam compat) Forward declarations for mutual recursion
  struct Parser {
    ...
    bool parse_expr(int32_t& val) {
      ...
      while (pos < tokens.size() && tokens[pos].type == ExprTokenType::OP) {
        std::string const op = tokens[pos].text;
        pos++;
        int32_t rhs;
...
std::vector<Z80Assembler::Line> Z80Assembler::parse(const std::string& source) {
  ...
  while (std::getline(stream, raw_line)) {
    ...
    // Actually, ':' as statement separator is after labels, so we handle label
    // first
    std::string const trimmed = trim(raw_line);
    ...
    if (!rest.empty() && (isalpha(static_cast<unsigned char>(rest[0])) ||
                          rest[0] == '_' || rest[0] == '.')) {
      size_t end = 0;
      while (end < rest.size() && is_label_char(rest[end])) end++;
      std::string const candidate = rest.substr(0, end);

      // If followed by ':', it's definitely a label
      // If followed by space and then a mnemonic, it might be a label or a
      // mnemonic
      if (end < rest.size() && rest[end] == ':') {
        ...
        std::string const upper_cand = to_upper(candidate);
    ...
    for (size_t si = 0; si < statements.size(); si++) {
      const std::string& stmt = statements[si];
      if (stmt.empty()) continue;

      // Split into mnemonic + operands
      size_t const space_pos = stmt.find_first_of(" \t");
...
int Z80Assembler::directive_size(const Line& line,
  ...
  if (line.mnemonic == "DEFB" || line.mnemonic == "DB" ||
      line.mnemonic == "BYTE") {
    return count_defb_bytes(line.operands);
  }
  if (line.mnemonic == "DEFW" || line.mnemonic == "DW" ||
      line.mnemonic == "WORD") {
    return count_comma_items(line.operands) * 2;
  }
  if (line.mnemonic == "DEFS" || line.mnemonic == "DS" ||
      line.mnemonic == "RMEM") {
...
bool Z80Assembler::encode_directive(const Line& line,
  ...
  if (line.mnemonic == "DEFB" || line.mnemonic == "DB" ||
    ...
    for (size_t i = 0; i <= line.operands.size(); i++) {
      ...
      if (c == ',' || i == line.operands.size()) {
        std::string const expr = trim(current_expr);
        current_expr.clear();
  ...
  if (line.mnemonic == "DEFW" || line.mnemonic == "DW" ||
    ...
    std::string current_expr;
    for (size_t i = 0; i <= line.operands.size(); i++) {
      char const c = (i < line.operands.size()) ? line.operands[i] : ',';
      if (c == ',' || i == line.operands.size()) {
        std::string const expr = trim(current_expr);
        current_expr.clear();
        if (expr.empty()) continue;
  ...
  if (line.mnemonic == "DEFS" || line.mnemonic == "DS" ||
    ...
    auto comma = count_str.find(',');
    if (comma != std::string::npos) {
      std::string const fill_str = trim(count_str.substr(comma + 1));
      count_str = trim(count_str.substr(0, comma));
      int32_t fill_val;
    ...
    for (int i = 0; i < count_val; i++) output.push_back(fill);
    return true;
  }
...
// Build a mnemonic pattern from parsed mnemonic+operands for lookup in opcode
// table. E.g., mnemonic="LD", operands="A,(IX+5)" → tries patterns like "LD
// A,(IX+*)" Returns the matched opcode entry and the evaluated operand values.
...
bool try_match_opcode(const std::string& mnemonic,
  ...
  for (auto& op_str : ops) {
    ...
    if (upper_op.size() >= 5 && upper_op[0] == '(' &&
      ...
      std::string const reg = upper_op.substr(1, 2);
      // Find the +/- sign
      size_t const sign_pos = 3;
      if (sign_pos < upper_op.size() && upper_op[sign_pos] == '+') {
        // (IX+expr) → (IX+*)
        std::string const expr_part =
            op_str.substr(4, op_str.size() - 5);  // strip (IX+ and )
        int32_t val;
        std::string eval_err;
        if (Z80Assembler::eval_expr(expr_part, symbols, current_addr, val,
                                    eval_err)) {
          alts.push_back({"(" + reg + "+*)", val, true});
        }
      } else if (sign_pos < upper_op.size() && upper_op[sign_pos] == '-') {
        // (IX-expr) → (IX+*) with negated value
        std::string const expr_part = op_str.substr(4, op_str.size() - 5);
        int32_t val;
        std::string eval_err;
        if (Z80Assembler::eval_expr(expr_part, symbols, current_addr, val,
                                    eval_err)) {
    ...
    if (upper_op.size() >= 3 && upper_op[0] == '(' && upper_op.back() == ')') {
      std::string const inner = op_str.substr(1, op_str.size() - 2);
      std::string const upper_inner = to_upper(inner);
      // Skip if it's a register indirect: (HL), (BC), (DE), (SP), (C), (IX),
      // (IY)
      if (upper_inner != "HL" && upper_inner != "BC" && upper_inner != "DE" &&
          upper_inner != "SP" && upper_inner != "C" &&
          upper_inner.substr(0, 2) != "IX" &&
          upper_inner.substr(0, 2) != "IY") {
        int32_t val;
        std::string eval_err;
        if (Z80Assembler::eval_expr(inner, symbols, current_addr, val,
                                    eval_err)) {
    ...
    if (!upper_op.empty() && upper_op[0] != '(') {
      ...
      if (!is_register(upper_op)) {
        int32_t val;
        std::string eval_err;
        if (Z80Assembler::eval_expr(op_str, symbols, current_addr, val,
                                    eval_err)) {
          alts.push_back({"*", val, true});
          alts.push_back({"**", val, true});
        }
  ...
  // For 3 operands: rare but possible (e.g. "ld (ix+*),*" has 3 parts after
  // split)

  auto try_lookup = [&](const std::string& pattern,
                        const std::vector<PatternOp>& selected) -> bool {
  ...
  if (candidates.size() == 1) {
    for (auto& alt : candidates[0]) {
      std::string const pattern = mnemonic + " " + alt.pattern;
    ...
    for (auto& a0 : candidates[0]) {
      for (auto& a1 : candidates[1]) {
        std::string const pattern =
            mnemonic + " " + a0.pattern + "," + a1.pattern;
    ...
    for (auto& a0 : candidates[0]) {
      for (auto& a1 : candidates[1]) {
        for (auto& a2 : candidates[2]) {
          std::string const pattern =
              mnemonic + " " + a0.pattern + "," + a1.pattern + "," + a2.pattern;
...
int Z80Assembler::instruction_size(const std::string& mnemonic,
                                   const std::string& operands,
                                   std::string& error) {
  // Quick approach: try to match and use the opcode entry's length field
  OperandMatch match;
  std::map<std::string, word> const dummy_syms;
  ...
  // the mnemonic to guess size. Clear error — we'll catch real errors in
  // pass 2.
  error.clear();
  ...
  for (size_t i = 0; i < ops.size(); i++) {
    ...
    if (is_register(upper) || upper == "NZ" || upper == "Z" || upper == "NC" ||
        upper == "C" || upper == "PO" || upper == "PE" || upper == "P" ||
        upper == "M" || upper == "(C)" || upper == "(HL)" || upper == "(BC)" ||
        upper == "(DE)" || upper == "(SP)") {
      new_operands += upper;
    } else if (upper[0] == '(' &&
               (upper.substr(1, 2) == "IX" || upper.substr(1, 2) == "IY")) {
  ...
  if (try_match_opcode(mnemonic, new_operands, dummy_with_zeros, 0, match,
                       error)) {
    error.clear();
    return match.opcode->length;
  }
...
bool Z80Assembler::encode_instruction(
  ...
  if (op->is_relative && !match.operand_values.empty()) {
    ...
    int32_t const target = match.operand_values[0];
    int32_t const offset = target - (current_addr + op->length);
    ...
    while (*m) {
      if (*m == '*' && *(m + 1) == '*') {
        ...
        int32_t const val = match.operand_values[val_idx++];
        output.push_back(static_cast<byte>(val & 0xFF));
        output.push_back(static_cast<byte>((val >> 8) & 0xFF));
        m += 2;
      } else if (*m == '*') {
...
bool Z80Assembler::pass1(std::vector<Line>& lines,
  ...
  for (auto& line : lines) {
    ...
    // Handle label
    if (!line.label.empty()) {
      std::string const upper_label = to_upper(line.label);
...
bool Z80Assembler::pass2(const std::vector<Line>& lines,
  ...
  for (auto& line : lines) {
    ...
    if (is_directive(line.mnemonic)) {
      ...
      ok = encode_instruction(line.mnemonic, line.operands, symbols,
                              current_addr, output, err);
    }
...
AsmResult Z80Assembler::assemble(const std::string& source) {
  ...
  if (!pass2(lines, result.symbols, result.errors, true, result.start_addr,
             result.end_addr, result.bytes_written)) {
    return result;
  }
...
AsmResult Z80Assembler::check(const std::string& source) {
  ...
  if (!pass2(lines, result.symbols, result.errors, false, result.start_addr,
             result.end_addr, result.bytes_written)) {
    return result;
  }
