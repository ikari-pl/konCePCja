...
word Amplitudes_AY[16] = {0,     836,   1212,  1773,  2619,  3875,
...
int Level_AR[32], Level_AL[32], Level_BR[32], Level_BL[32], Level_CR[32],
    Level_CL[32];
int LevelTape;
...
void Case_EnvType_8() { PSG.AmplitudeEnv = (PSG.AmplitudeEnv - 1) & 31; }
...
void Case_EnvType_12() { PSG.AmplitudeEnv = (PSG.AmplitudeEnv + 1) & 31; }
...
inline void Synthesizer_Logic_Q() {
  ...
  Noise_Counter.Hi++;
  if ((!(Noise_Counter.Hi & 1)) &&
      (Noise_Counter.Hi >= (PSG.RegisterAY.Noise << 1))) {
    Noise_Counter.Hi = 0;
    Noise.Seed = (((((Noise.Seed >> 13) ^ (Noise.Seed >> 16)) & 1) ^ 1) |
                  Noise.Seed << 1) &
                 0x1ffff;
  }
...
void Synthesizer_Stereo16() {
  ...
  *reinterpret_cast<dword*>(CPC.snd_bufferptr) =
      val.d;  // write to mixing buffer
  CPC.snd_bufferptr += 4;
  Left_Chan = 0;
...
void Synthesizer_Stereo8() {
  ...
  *reinterpret_cast<word*>(CPC.snd_bufferptr) =
      val.w.l;  // write to mixing buffer
  CPC.snd_bufferptr += 2;
  Left_Chan = 0;
  Right_Chan = Left_Chan;
  psg_check_buffer_wrap();
...
void Synthesizer_Mono16() {
  ...
  *reinterpret_cast<word*>(CPC.snd_bufferptr) =
      static_cast<word>(m_out);  // write to mixing buffer
  CPC.snd_bufferptr += 2;
  Left_Chan = 0;
  psg_check_buffer_wrap();
}
...
void Synthesizer_Mono8() {
  ...
  LoopCount.Re += LoopCountInit;
  *reinterpret_cast<byte*>(CPC.snd_bufferptr) =
      128 + Left_Chan / Tick_Counter;  // write to mixing buffer
  CPC.snd_bufferptr++;
  Left_Chan = 0;
  psg_check_buffer_wrap();
}
...
void Calculate_Level_Tables() {
  ...
  for (i = 0, b = 255; i < 256;
       i++) {  // calculate the 256 levels of the Digiblaster/Soundplayer
...
void InitAYCounterVars() {
  CPC.snd_cycle_count_init.both = static_cast<int64_t>(
      rint((4000000 * ((CPC.speed * 25) / 100.0)) /
           freq_table[CPC.snd_playback_rate] *
           4294967296.0));  // number of Z80 cycles per sample
  LoopCountInit = static_cast<int64_t>(
      rint(1000000.0 / (4000000.0 * ((CPC.speed * 25) / 100.0)) / 8.0 *
           CPC.snd_cycle_count_init
               .both));  // number of AY counter increments per sample
  LOG_INFO("Timing: CPC.speed: " << CPC.speed << " - freq: "
                                 << freq_table[CPC.snd_playback_rate]);
  LOG_INFO("Timing: z80 cycles per sample: "
           << CPC.snd_cycle_count_init.both / 4294967296.0
           << " - LoopCountInit: " << LoopCountInit / 4294967296.0);
  LoopCount.Re = LoopCountInit;
}
