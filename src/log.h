...
#define LOG_TO(stream, level, message)                                        \
  stream << (level) << " " << __FILE__ << ":" << __LINE__ << " - " << message \
         << std::endl;  // NOLINT(misc-macro-parentheses): Not having
                        // parentheses around message is a feature, it allows
                        // using streams in LOG macros
