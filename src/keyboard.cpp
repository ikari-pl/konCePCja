...
const std::map<const char, const CPC_KEYS> InputMapper::CPCkeysFromChars = {
    ...
    // TODO(sebhz): Need to map non ASCII chars present on the CPC keyboard -
    // maybe by using their ISO-8859-1 code
    {'&', CPC_AMPERSAND},
    {'#', CPC_HASH},
    {'"', CPC_DBLQUOTE},
    {'\'', CPC_QUOTE},
    ...
    {'@', CPC_AT},
    {'^', CPC_POWER},
    // Control keys (CPC_CTRL_a to CPC_CTRL_9) are not ont the virtual keyboard
    // (yet?)
    //{ '~', {0, KMOD_NONE} } // should be pound but it's not part of base ascii
    //(it's in extended ASCII)
};
      ...
      case 2:
        if (SDLkeysFromStrings.count(pch) == 0) {
          LOG_ERROR("Unknown SDL key or modifier "
                    << pch << " found in mapping file. Ignoring it.");
          result.valid = false;
          return result;
        }
...
bool InputMapper::load_layout(const std::string& filename) {
  std::filebuf fb;
  char line[MAX_LINE_LENGTH];  // sufficient for now ! TODO(sebhz): proper
                               // malloc'ing etc...
  ...
  if (is_directory(filename) || (fb.open(filename, std::ios::in) == nullptr)) {
    ...
    while (is.good()) {
      ...
      // This triggers the error even if the line just fits, but relying on
      // failbit doesn't work (it's set also at EOF) so that will have to do.
      if (strlen(line) >= MAX_LINE_LENGTH - 1) {
        LOG_ERROR("Mapping '" << filename << "' contains a line longer than "
                              << MAX_LINE_LENGTH << " char, this is invalid: '"
                              << line << "'");
        valid = false;
      }
...
void InputMapper::init() {
  ...
  for (const auto& mapping : CPCkeysFromChars) {
    if (SDLkeysymFromCPCkeys.count(mapping.second) != 0) {
      ...
      SDLkeysFromChars[mapping.first] = std::make_pair(
...
CPCScancode InputMapper::CPCscancodeFromKeysym(SDL_Keycode key,
  ...
  // Map right alt to Mode (AltGr). Not clear what determines whether SDL2 uses
...
void InputMapper::set_joystick_emulation() {
  ...
  for (int n = 0; n < 6; n++) {
    int cpc_idx =
        joy_layout[n][1];  // get the CPC key to change the assignment for
    if (cpc_idx) {
      PCKey pc_idx = SDLkeysymFromCPCkeys[cpc_idx];  // SDL key corresponding to
                                                     // the CPC key to remap
...
void applyKeypressDirect(CPCScancode cpc_key,
  ...
  if (pressed) {
    keyboard_matrix[static_cast<byte>(cpc_key) >> 4].fetch_and(
        ...
        std::memory_order_relaxed);  // key is being held down
    if (cpc_key & MOD_CPC_SHIFT) {   // CPC SHIFT key required?
      keyboard_matrix[0x25 >> 4].fetch_and(
          ~bit_values[0x25 & 7],
          std::memory_order_relaxed);  // key needs to be SHIFTed
    } else {
      keyboard_matrix[0x25 >> 4].fetch_or(
          bit_values[0x25 & 7],
          std::memory_order_relaxed);  // make sure key is unSHIFTed
    }
    if (cpc_key & MOD_CPC_CTRL) {  // CPC CONTROL key required?
      keyboard_matrix[0x27 >> 4].fetch_and(
          ~bit_values[0x27 & 7],
          std::memory_order_relaxed);  // CONTROL key is held down
    } else {
      keyboard_matrix[0x27 >> 4].fetch_or(
          bit_values[0x27 & 7],
          std::memory_order_relaxed);  // make sure CONTROL key is released
    }
  } else {
    keyboard_matrix[static_cast<byte>(cpc_key) >> 4].fetch_or(
        bit_values[static_cast<byte>(cpc_key) & 7],
        std::memory_order_relaxed);  // key has been released
    if (release_modifiers) {
      ...
      keyboard_matrix[0x27 >> 4].fetch_or(
          bit_values[0x27 & 7],
          std::memory_order_relaxed);  // make sure CONTROL key is not held down
    }
  }
}
