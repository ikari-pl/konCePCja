...
constexpr size_t SILICON_DISC_SIZE =
    SILICON_DISC_BANKS * SILICON_DISC_BANK_SIZE;  // 256K
...
struct SiliconDisc {
  ...
  // Get pointer to a specific bank's 64K region (bank_index 0-3, mapped from
  // expansion banks 4-7)
  uint8_t* bank_ptr(int bank_index) const {
    if (!data || bank_index < 0 || bank_index >= SILICON_DISC_BANKS)
      return nullptr;
