...
void HpglPlotter::feed_byte(uint8_t byte) {
  ...
  if (c == ';') {
    ...
    if (c != '\r' && c != '\n') {
      cmd_buf_ += c;
      // LB (Label) has no ';' terminator — label text follows the two-letter
      // mnemonic directly, terminated by the label terminator (default
      // ETX=0x03). Switch to label mode as soon as the mnemonic is complete.
      if (cmd_buf_.size() == 2 &&
          toupper(static_cast<unsigned char>(cmd_buf_[0])) == 'L' &&
          toupper(static_cast<unsigned char>(cmd_buf_[1])) == 'B') {
        in_label_ = true;
        label_buf_.clear();
        cmd_buf_.clear();
      }
...
void HpglPlotter::parse_params(const std::string& params,
...
void HpglPlotter::process_command(const std::string& cmd) {
  ...
  char const m0 = static_cast<char>(toupper(cmd[0]));
  char const m1 = static_cast<char>(toupper(cmd[1]));
  std::string const params = cmd.substr(2);
  std::vector<float> p;
...
float HpglPlotter::tx(float x) const {
  ...
  float const frac = (x - sc_xmin_) / (sc_xmax_ - sc_xmin_);
  return p1_x_ + (frac * (p2_x_ - p1_x_));
}

float HpglPlotter::ty(float y) const {
  if (!scaling_active_) return y;
  float const frac = (y - sc_ymin_) / (sc_ymax_ - sc_ymin_);
...
void HpglPlotter::cmd_PD(const std::vector<float>& p) {
  pen_down_ = true;
  for (size_t i = 0; i + 1 < p.size(); i += 2) move_to(tx(p[i]), ty(p[i + 1]));
}
...
void HpglPlotter::cmd_PR(const std::vector<float>& p) {
  for (size_t i = 0; i + 1 < p.size(); i += 2) {
    float const dx = scaling_active_
                         ? (p[i] / (sc_xmax_ - sc_xmin_)) * (p2_x_ - p1_x_)
                         : p[i];
    float const dy = scaling_active_
                         ? (p[i + 1] / (sc_ymax_ - sc_ymin_)) * (p2_y_ - p1_y_)
                         : p[i + 1];
...
void HpglPlotter::cmd_AA(const std::vector<float>& p) {
  if (p.size() < 3) return;
  float const cx = tx(p[0]);
  float const cy = ty(p[1]);
  float const sweep = p[2];
  float const dx = pen_x_ - cx;
  float const dy = pen_y_ - cy;
...
void HpglPlotter::cmd_EA(const std::vector<float>& p) {
  if (p.size() < 2) return;
  float const x2 = tx(p[0]);
  float const y2 = ty(p[1]);
  if (selected_pen_ > 0) {
    float const x1 = pen_x_;
    float const y1 = pen_y_;
...
void HpglPlotter::cmd_ER(const std::vector<float>& p) {
  if (p.size() < 2) return;
  std::vector<float> const abs_p = {pen_x_ + p[0], pen_y_ + p[1]};
  cmd_EA(abs_p);
}
...
void HpglPlotter::cmd_LB(const std::string& text) {
  ...
  float const char_w_units =
      char_width_ * HPGL_UNITS_PER_MM * 10.0f;  // cm → mm → units
...
void HpglPlotter::cmd_SC(const std::vector<float>& p) {
  if (p.size() >= 4) {
    ...
    // Reject degenerate ranges — division by (xmax-xmin) or (ymax-ymin) would
    // be undefined
...
bool plotter_export_svg_segments(const std::vector<PlotSegment>& segments,
  ...
  float const svg_w = static_cast<float>(HPGL_MAX_X) / HPGL_UNITS_PER_MM;  // mm
  float const svg_h = static_cast<float>(HPGL_MAX_Y) / HPGL_UNITS_PER_MM;
  ...
  for (int pen = 1; pen <= 2; pen++) {
    ...
    fprintf(f,
            "  <g stroke=\"%s\" fill=\"none\" stroke-width=\"%.1f\" "
    ...
    for (const auto& seg : segments) {
      ...
      float const fy1 = HPGL_MAX_Y - seg.y1;
      float const fy2 = HPGL_MAX_Y - seg.y2;
      ...
      switch (seg.type) {
        ...
        case PlotPrimitive::Arc: {
          ...
          float const r = seg.radius;
          float const sa = seg.start_angle * static_cast<float>(M_PI) / 180.0f;
          float const ea = (seg.start_angle + seg.sweep_angle) *
                           static_cast<float>(M_PI) / 180.0f;
          float const sx = seg.x1 + (r * std::cos(sa));
          float const sy = HPGL_MAX_Y - (seg.y1 + (r * std::sin(sa)));
          float const ex = seg.x1 + (r * std::cos(ea));
          float const ey = HPGL_MAX_Y - (seg.y1 + (r * std::sin(ea)));
          int const large_arc = std::abs(seg.sweep_angle) > 180 ? 1 : 0;
          int const sweep_flag =
              seg.sweep_angle > 0 ? 0 : 1;  // Y flip inverts sweep
          fprintf(f,
                  "    <path d=\"M%.1f,%.1f A%.1f,%.1f 0 %d,%d %.1f,%.1f\"/>\n",
                  sx, sy, r, r, large_arc, sweep_flag, ex, ey);
          break;
        }

        case PlotPrimitive::Label: {
          float const font_size = char_height_cm * HPGL_UNITS_PER_MM * 10.0f;
          fprintf(f,
                  "    <text x=\"%.1f\" y=\"%.1f\" "
                  "font-family=\"monospace\" font-size=\"%.1f\" "
                  "fill=\"%s\" stroke=\"none\">%s</text>\n",
                  seg.x1, HPGL_MAX_Y - seg.y1, font_size, pen_color(pen),
                  xml_escape(seg.text).c_str());
          break;
        }
      }
    }
