...
class M4HttpServer {
 public:
  ...
  // port <= 0: bind an ephemeral TCP port; use port() after the server thread
  // starts.
  ...
  void set_port_mapping(uint16_t cpc_port, uint16_t host_port,
                        bool user_override = true);
  void remove_port_mapping(uint16_t cpc_port);
  uint16_t resolve_host_port(
      uint16_t cpc_port);  // returns host port for CPC port
 ...
 private:
  ...
  std::string build_dir_txt(
      const std::string& path);  // M4-compatible dir listing
  ...
  // Status snapshots — written by main thread in drain_pending(), read by HTTP
  // thread.
