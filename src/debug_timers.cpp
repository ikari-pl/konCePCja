...
int32_t DebugTimerManager::timer_stop(int32_t id, uint64_t tstate) {
  ...
  uint64_t const elapsed_tstates = tstate - t.start_tstate;
  // CPC runs at 4MHz → 1 T-state = 0.25µs → elapsed_µs = tstates / 4
  uint32_t const us = static_cast<uint32_t>(elapsed_tstates / 4);

  t.last_us = us;
  t.count++;
  t.total_us += us;
  t.min_us = std::min(us, t.min_us);
  t.max_us = std::max(us, t.max_us);
