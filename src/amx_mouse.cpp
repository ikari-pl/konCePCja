...
void amx_mouse_update(float dx, float dy, uint32_t sdl_buttons) {
  ...
  int const whole_x = static_cast<int>(g_amx_mouse.accum_x);
  int const whole_y = static_cast<int>(g_amx_mouse.accum_y);
  if (whole_x != 0) {
...
void amx_mouse_row_select(int line) {
  ...
  if (now_row9 && g_amx_mouse.row9_was_deselected) {
    ...
    if (g_amx_mouse.mickey_x > 0)
      g_amx_mouse.mickey_x--;
    else if (g_amx_mouse.mickey_x < 0)
      g_amx_mouse.mickey_x++;
    if (g_amx_mouse.mickey_y > 0)
      g_amx_mouse.mickey_y--;
    else if (g_amx_mouse.mickey_y < 0)
      g_amx_mouse.mickey_y++;
    g_amx_mouse.row9_was_deselected = false;
  }
...
namespace {
byte amx_kbd_read_hook(int line) {
...
namespace {
void amx_kbd_line_hook(int line) { amx_mouse_row_select(line); }
}  // namespace
