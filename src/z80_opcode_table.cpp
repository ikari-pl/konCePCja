...
// Key = (prefix << 8) | opcode
namespace {
std::unordered_map<int, const Z80Opcode*> s_disasm_lookup;
}  // namespace
...
namespace {
std::unordered_map<std::string, bool> s_mnemonic_keywords;
}  // namespace
...
namespace {
std::vector<const Z80Opcode*> s_empty_vec;
}  // namespace

namespace {
int prefix_to_key(OpcodePrefix p) {
  switch (p) {
...
void z80_opcode_table_init() {
  ...
  for (const auto& op : g_z80_opcodes) {
    // Disassembler lookup: prefix_key | opcode
    int const key = (prefix_to_key(op.prefix) << 8) | op.opcode;
...
const Z80Opcode* z80_opcode_lookup(OpcodePrefix prefix, byte opcode) {
  int const key = (prefix_to_key(prefix) << 8) | opcode;
  auto it = s_disasm_lookup.find(key);
  return it != s_disasm_lookup.end() ? it->second : nullptr;
}

const std::vector<const Z80Opcode*>& z80_asm_lookup(
...
int z80_opcode_to_legacy_key(const Z80Opcode& op) {
  switch (op.prefix) {
    case OpcodePrefix::NONE:
      return op.opcode;
    case OpcodePrefix::CB:
      return 0xCB00 | op.opcode;
    case OpcodePrefix::ED:
      return 0xED00 | op.opcode;
    case OpcodePrefix::DD:
      return 0xDD00 | op.opcode;
    case OpcodePrefix::FD:
      return 0xFD00 | op.opcode;
    case OpcodePrefix::DDCB:
      return 0xDDCB00 | op.opcode;
    case OpcodePrefix::FDCB:
      return 0xFDCB00 | op.opcode;
  }
  return op.opcode;
}

std::string z80_opcode_table_to_txt() {
  // Output order matches original z80_opcodes.txt: NONE, ED, CB, DD, DDCB, FD,
  // FDCB
  static const OpcodePrefix output_order[] = {
      OpcodePrefix::NONE, OpcodePrefix::ED, OpcodePrefix::CB,  OpcodePrefix::DD,
      OpcodePrefix::DDCB, OpcodePrefix::FD, OpcodePrefix::FDCB};

  std::ostringstream oss;
  for (auto prefix : output_order) {
    for (const auto& op : g_z80_opcodes) {
      if (op.prefix != prefix) continue;
      int const key = z80_opcode_to_legacy_key(op);
