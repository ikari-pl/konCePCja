...
FileBackend::FileBackend(const std::string& input_path,
...
static inline void tcp_set_nonblocking(int s) {
  u_long m = 1;
...
uint8_t TcpSocketBackend::recv() {
  ...
  if (sockfd_ >= 0) {
    uint8_t buf[256];
    ssize_t n = ::recv(sockfd_, reinterpret_cast<char*>(buf),
...
void SIRomManager::load(byte** rom_map, const std::string& rom_path) {
  ...
  static const char* rom_names[] = {"serial.rom", "SERIAL.ROM", "si_rom.bin",
                                    "SI_ROM.BIN", nullptr};
...
void SerialInterface::apply_config() {
  ...
  if (backend) {
    ...
    // For the plotter backend, intercept BDOS serial calls at the emulator
    // level — equivalent to what the real AMSIf BIOS patch did.  All hooks
    // simulate RET, bypassing BDOS/BIOS entirely. C=4 (PUNCH/AUX-OUT): HP-GL
    // data from GSX drivers (e.g. DDHP7470.PRL). C=5 (L_WRITE/LIST): CR/LF
    // terminators from same drivers. C=3 (A_READ/AUX-IN): plotter responses
    // (ENQ→ACK, OS;→"16\r", OD;→dimensions). C=6 (D_CIO, E=$FF): XON/XOFF poll
    // — handled in z80.cpp, always returns $11.
