...
struct MemoryBus {
  ...
  byte read_raw(word addr) const {
    return *(read_banks[addr >> 14] + (addr & 0x3fff));
  }

  void write_raw(word addr, byte v) const {
  ...
  // viewer).
  byte* bank_read_ptr(int bank) const { return read_banks[bank]; }

  byte* bank_write_ptr(int bank) const { return write_banks[bank]; }
};
