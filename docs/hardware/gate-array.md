...
pixels in a pattern that minimises the Gate Array's logic. **Authority: the
cpcwiki Gate Array "Video memory structure" table**
(`references/cpcwiki/Gate_Array.txt:369`): the mode-0 byte (bit 7→0) is
`A0 B0 A2 B2 A1 B1 A3 B3` (A = left pixel, B = right; the digit is the pen-bit
index). An earlier version of this section **inverted** the pen-bit significance
in modes 0 and 1 (treating byte bit 7 as the pen MSB instead of the LSB) — the
class of the shipped mode-0 decode bug (`2313200`); the tests
`video_test.cpp::ModeDecode*` are the ground truth.
...
```
  ...
  Bit role: lo  lo  lo  lo  hi  hi  hi  hi

  Pixel 0 = bit3 << 1 | bit7    → ink 0-3
  Pixel 1 = bit2 << 1 | bit6    → ink 0-3
  Pixel 2 = bit1 << 1 | bit5    → ink 0-3
  Pixel 3 = bit0 << 1 | bit4    → ink 0-3
```
...
```
  ...
  Pen bit:  b0  b0  b2  b2  b1  b1  b3  b3

  Pixel 0 = bit1 << 3 | bit5 << 2 | bit3 << 1 | bit7    → ink 0-15
  Pixel 1 = bit0 << 3 | bit4 << 2 | bit2 << 1 | bit6    → ink 0-15
```
...
```
  Pixel 0: bit3 << 1 | bit7 = 0 << 1 | 1 = 1    → ink 1
  Pixel 1: bit2 << 1 | bit6 = 1 << 1 | 0 = 2    → ink 2
  Pixel 2: bit1 << 1 | bit5 = 1 << 1 | 1 = 3    → ink 3
  Pixel 3: bit0 << 1 | bit4 = 1 << 1 | 0 = 2    → ink 2
  Result: 1 2 3 2
```

**Mode 0** (2 pixels, 4 bits each):
```
  Pixel 0: bit1<<3 | bit5<<2 | bit3<<1 | bit7 = 1<<3 | 1<<2 | 0<<1 | 1 = 13  → ink 13
  Pixel 1: bit0<<3 | bit4<<2 | bit2<<1 | bit6 = 1<<3 | 0<<2 | 1<<1 | 0 = 10  → ink 10
  Result: 13 10
```

This shows why Mode 0 pixel manipulation in software is painful: to set pixel
0 to ink 5 (`0101` = pen bits b0=1,b1=0,b2=1,b3=0), you need to write byte bits
7=1, 3=0, 5=1, 1=0 — scattered across the byte. Most games use lookup tables.
