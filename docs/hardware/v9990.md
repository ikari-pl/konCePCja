...
| Pin | Name | Dir | Function |
|-----|------|-----|----------|
| 98-99 | XTAL1, XTAL2 | I | 21MHz crystal oscillator |
| 113 | MCKIN | I | 14MHz master clock input (alternative to XTAL) |
...
| **P#7** | System Control | W | System reset, master clock select |
...
When NII=0 and RII=0, the register pointer auto-increments after every read or write to P#3 — this allows burst-programming multiple consecutive registers.
...
|-----|------|------|----------|
| **R#0** | VRAM Write Address Low | 7-0 | VRAM write pointer bits 7-0 |
| **R#1** | VRAM Write Address Mid | 7-0 | VRAM write pointer bits 15-8 |
| **R#2** | VRAM Write Address High | 7: AII, 6-0: addr 18-16 | Bit 7 = Address Increment Inhibit |
| **R#3** | VRAM Read Address Low | 7-0 | VRAM read pointer bits 7-0 |
| **R#4** | VRAM Read Address Mid | 7-0 | VRAM read pointer bits 15-8 |
| **R#5** | VRAM Read Address High | 7: AII, 6-0: addr 18-16 | Bit 7 = Address Increment Inhibit |
| **R#6** | Screen Mode 0 | 7-6: DSPM, 5-4: DCKM, 3-2: XIMM, 1-0: CLRM | Display mode + image width + colour depth |
| **R#7** | Screen Mode 1 | 7: HSCN, 6: C25, 5: SM1, 4: SM, 3: PAL, 2: EO, 1: IL, 0: YSE | Scan freq, overscan, PAL, interlace |
| **R#8** | Control | 7: DISP, 6: SPD, 5: YSE, 4: VWTE, 3: VWM, 2-0: reserved | Display enable, sprite disable, superimpose |
...
| **R#13** | Palette Control | 7-4: PLT, 3-0: YUV/palette offset | Palette type + offset |
| **R#14** | Palette Pointer | 7-2: palette No., 1-0: RGB counter | Which entry to access via P#1 |
...
| **R#52** | Opcode | 7-4: CMD, 3-0: reserved | Command to execute (trigger on write) |
...
R#6 is the master mode selector. Its four fields choose everything about the display:
...
b7  b6 │ b5  b4 │ b3  b2 │ b1  b0
 DSPM  │  DCKM  │  XIMM  │  CLRM
       ...
       │        └───── Image width: 00=256, 01=512, 10=1024, 11=2048
       └──── Dot clock: 00=DLCLK, 01=DHCLK, 10=DLCLK+DHCLK
 Display type:
...
|------|-----------|---------|------------|----------|-------|
| **B0** | varies | — | 192×212 | Yes | Narrow overscan |
| **B1** | 5.4 MHz | 15.7 KHz (NTSC) | 256×212 | No | Standard bitmap |
| **B2** | 7.2 MHz | 15.7 KHz (NTSC) | 384×240 | Yes | Wide overscan |
| **B3** | 10.7 MHz | 15.7 KHz (NTSC) | 512×212 | No | Hi-res bitmap |
| **B4** | 14.3 MHz | 15.7 KHz (NTSC) | 768×240 | Yes | Ultra-wide overscan |
| **B5** | 21.5 MHz | 25.3 KHz | 640×400 | No | VGA-class (OA mode) |
| **B6** | 25.2 MHz | 31.5 KHz | 640×480 | No | VGA-class (OA mode) |
| **B7** | varies | — | 1280×212 | No | Ultra hi-res (limited) |
...
|------|--------|---------|-------------|
| PP | Palette Per Pixel | 64 from 32,768 | Indexed: 6-bit pixel → 64-entry LUT → 15-bit RGB |
| BYUV | Bitmap YUV | 32,768 | Direct: 5-bit Y, 5-bit U, 5-bit V per pixel |
| BYUVP | Bitmap YUV+Palette | 32,768 | YUV pixels with palette offset |
| BYJK | Bitmap YJK | 19,268 | MSX2+ compatible YJK encoding (4 pixels share J,K) |
| BYJKP | Bitmap YJK+Palette | 19,268 | YJK with palette override |
...
- 64 entries, each 3 bytes (R, G, B)
- Each component is 5 bits (0-31), stored in bits 4-0
- Bit 5 of the RED byte is the **YS** (superimpose) flag
- Total palette RAM: 64 × 3 = 192 bytes

### Palette Access Protocol (via P#1)

```
1. Write R#14 with: [palette_no << 2 | 0]  (select entry, reset RGB counter)
2. Write P#1: RED data   (5 bits + YS flag in bit 5)
3. Write P#1: GREEN data (5 bits)
4. Write P#1: BLUE data  (5 bits)
   — palette pointer auto-increments to next entry
```

The ternary counter (R→G→B) cycles automatically, allowing burst writes of entire palettes.
...
4. Write P#3 = addr[18:16] | (AII << 7)  (R#2 set)
...
### Logical Operations (R#45, bits 3-0)

| Value | Operation | Description |
|-------|-----------|-------------|
| 0x0 | DC=SC | Direct copy (destination = source) |
| 0x1 | DC=SC AND DC | AND |
| 0x2 | DC=SC OR DC | OR |
| 0x3 | DC=SC XOR DC | XOR |
| 0x4 | DC=NOT SC | Complement source |
| 0x8 | DC=0 | Clear |
| 0xC | DC=1 | Set |
| Others | — | Various NOT-AND/NOT-OR/NOT-XOR combinations |

**TP (Transparent) bit** (R#45, bit 4): When set, source pixels equal to 0 are not written (sprite-style transparency).
...
; Example: LMMV (fill rectangle at DX=100, DY=50, 64×32 pixels with colour &FF)
    ld a, 4         ; Select R#36 (auto-increment)
    out (P4), a
    ld a, 100       ; DX low
    out (P3), a
    ld a, 0         ; DX high
    ...
    ld a, 0         ; NY high
    out (P3), a
    ; Now set argument and logop
    ld a, 0x40      ; Select R#44
    out (P4), a
    ld a, 0         ; Argument: no direction flags
    out (P3), a
    ld a, 0         ; LogOp: direct copy
    out (P3), a
    ld a, &FF       ; Write mask low
    out (P3), a
    ld a, &FF       ; Write mask high
    out (P3), a
    ld a, &FF       ; FC0 = fill colour
    out (P3), a
    ld a, 0         ; FC1 (for 16bpp modes)
    out (P3), a
    ; Skip BC0/BC1 for LMMV
    ld a, 0x34      ; Select R#52 (opcode register)
    out (P4), a
    ld a, &20       ; Opcode 0x2 = LMMV (fill)
...
- **Pattern base**: Set by R#25, shared with pattern screen generator
...
Reading P#6 returns the pending interrupt flags. Interrupts are **not auto-cleared** — you must write 1 to the corresponding bit in P#6 to acknowledge/clear it.
...
| 4 | BD | Border detect — search command found boundary |
| 3-2 | — | Reserved |
| 1 | EO | Even/Odd field (interlace) |
...
.wait: djnz .wait
    ...
    ld a, 6                     ; Select R#6
    out (P4), a
    ld a, &91                   ; DSPM=10 (Bx), DCKM=01 (DHCLK), XIMM=00 (256w), CLRM=01 (4bpp)
    out (P3), a
    ...
    ld a, &0E                   ; R#14 = palette pointer: entry 0, R counter
    out (P4), a
    ld a, 0
    out (P3), a                 ; Entry 0, R=0 counter
...
struct V9990 {
    ...
    // Palette
    uint8_t palette[192];      // 64 entries × 3 bytes (R,G,B)
    uint8_t palette_ptr;       // Auto-cycling R→G→B counter
...
1. **R#8 bit 5 (YSE)**: Enables superimposition mode
2. **Palette bit 5 (YS per entry)**: Each palette colour can be individually set as "transparent to external video"
3. **Physical pin *YS**: Active low when the V9990 wants the external video to show through
...
- Yamaha V9990 Application Manual (LSI-2499903), September 1992
- OpenMSX source code: `src/video/v9990/` (V9990.hh, V9990CmdEngine.hh, V9990VRAM.hh)
