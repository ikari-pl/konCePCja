...
When the Gate Array detects VSYNC it arms a 2-HSYNC countdown; two HSYNCs later
it **resets its interrupt counter to 0** — and if the counter had already reached
**≥32** it fires an interrupt then too (the "save margin", so a due interrupt near
the VSYNC boundary is not skipped). This synchronises interrupts to the frame.
(This is the hardware-correct *Longshot* model — see `gate-array-device.md` §4 and
`gate_array.cpp:ga_raster_count`. An earlier version of this text said "resets to
2", which was inaccurate.)
