...
| `romdis` | 1 | wired-OR, expansions | /ROMDIS: silences the internal ROM read decode (multiface-device.md §1) |
| `ramdis` | 1 | wired-OR, expansions | /RAMDIS: silences the internal RAM (reads at once; writes via the memory Device's one-tick commit latch, memory-device.md §4b) |
...
### 4.4 AyBus — the PPI ⇆ PSG point-to-point bus (+ the keyboard connector)

| Field | Bits | Driver | Meaning |
|------|------|--------|---------|
| `da` | 8 | PPI on write, AY on read | DA0–7 data bus |
| `bdir` | 1 | PPI (Port C bit 7) | bus direction |
| `bc1` | 1 | PPI (Port C bit 6) | bus control 1 |
| `kbd_row` | 4 | PPI (Port C bits 0..3) | selected keyboard row |
| `row_ext` | 8 | wired-AND, connector devices | external column lines of the selected row, idle 0xFF — joystick-port peripherals pull bits LOW, the PSG ANDs them into the port-A read (amx-mouse-device.md §1) |

`Bus = { CpuBus cpu; VidBus vid; RamBus ram; AyBus ay; TapeBus tape; Clocks clk; }`
...
Each entry: lines driven/observed, clock enable, spec, and conformance oracle.
All landed devices follow the uniform contract (§5): caller-owned state, no heap,
tick/reset/state_size/save/load.

- **Gate Array** ([`hardware/gate-array-device.md`](hardware/gate-array-device.md)) —
  drives `clk.*` (÷4 CPU, ÷16 CRTC/PSG, phase), `cpu.wait` (released only in µs
  slot 1 — the I/O-cycle stretch), `cpu.irq` (300 Hz raster counter off HSYNC, VSYNC
  resync), `ram.addr`/`ram.fetch` (two video fetches per µs, phases 12/14); observes
  `vid.hsync/vsync`, the M1+IORQ ack (clears bit 5), and I/O writes A15=0,A14=1
  (pen/ink/mode/ROM-enable). Oracle: legacy `cc_op[]` timing + boot.
- **Z80 (CPC Z80A, NMOS)** ([`hardware/z80.md`](hardware/z80.md)) — micro-op M-cycle
  FSM, octal decode, MEMPTR/Q, interrupts, drive-and-hold on the ÷4 clock. Memory
  M-cycles hold T1 to the µs grid; I/O cycles free-run T1 and stretch on `cpu.wait`;
  interrupt-gated Banks-2018 block-I/O flag corrections. Oracles: **FUSE 1356/1356**,
  the 1872-case cc_* sweep, hand-written flag vectors.
- **CRTC types 0–3** ([`hardware/crtc-device.md`](hardware/crtc-device.md)) — drives
  `vid.*` (HSYNC/VSYNC/DISPEN/MA/RA), answers the per-type read functions on
  `cpu.data`; observes A14-low I/O. Physical counter widths (8-bit HCC / 5-bit RA /
  7-bit VCC) with equality end-tests, write masks, r7match VSYNC edge, type-1 row-0
  re-read. Oracle: legacy engine + crtc.md; 64 µs/312-line standard frame.
- **PPI 8255** ([`hardware/ppi-device.md`](hardware/ppi-device.md)) — A11-low I/O;
  ports A/B/C + control (mode-set clears latches; reset = 0x9B all-input); drives the
  private AY bus (`ay.*`) and reads `vid.vsync` into port B. Oracle: legacy +
  firmware keyboard scan.
- **PSG AY-3-8912** ([`hardware/psg-device.md`](hardware/psg-device.md)) — reached
  ONLY via `ay.*` (BDIR/BC1 latch/write/read); 16-register file with write masks;
  tone ÷8-toggle (= f/16·period), 17-bit LFSR noise ÷16, 10-shape envelope ÷256;
  reg-14 keyboard column read-back for `ay.kbd_row`. Oracle: legacy synth rates +
  WAV capture measurement (261.6 Hz middle C).
- **memory** ([`hardware/memory-device.md`](hardware/memory-device.md)) — answers
  `mreq` (ROM overlays win reads; writes pass through to banked RAM) and the video
  fetch port (`ram.fetch` → base-64K only, never banked/ROM). 6128 PAL banking
  (A15-low + data 11xxxxxx; 8 configs; dk'tronics + Yarek 4 MB extended banks),
  multi-ROM select (A13-low; 256 caller-attached ROMs, BASIC fallback). Oracle:
  legacy ga_init_banking/ga_memory_manager; BASIC bank/HIMEM proofs.
- **video** ([`hardware/video-device.md`](hardware/video-device.md)) — the GA's pixel
  serializer: consumes `ram.data` fetches (latch at phase 13/15), beam-timed full
  visible raster incl. border (`ink[16]`), mode 0/1/2 pen-plane decode, 32-colour
  palette. Non-bus sink: caller framebuffer (§9a). Oracle: legacy gfx_decode_byte +
  golden pixels + real-firmware boot screen.
- **FDC µPD765A** ([`hardware/fdc-device.md`](hardware/fdc-device.md)) — A10=0,A7=0
  I/O (A8: motor latch vs FDC; A0: MSR vs data); three-phase command FSM, polled
  non-DMA, no INT (unconnected on CPC); caller-attached DSK (std+extended, parsed in
  place). "Fast FDC" V1 timing (deferred: MFM pacing, rotation — the flux Stage 2).
  Oracle: legacy fdc.cpp semantics + AMSDOS `cat` end-to-end.
- **flux (pure functions)** ([`hardware/flux-media.md`](hardware/flux-media.md), in
  progress) — Greaseweazle/SuperCard-Pro SCP → PLL/MFM decode → standard DSK; feeds
  `fdc_attach_disk`. Stage 2 turns the medium into a rotating first-class object.

Test-only stubs (always-on clock, script injectors, RAM harnesses) live with the
tests; they implement the same Device shape.
...
| R8 | **RamBus supersedes R4**: the GA owns the DRAM address mux as a separate RAM-side port (`Bus.ram`) — the Z80 address bus never reaches the RAM pins; CPU slot = phases 0–11, video fetches at 12/14; video fetches never see ROM overlays or banking | 2026-07-03 |
| R9 | **Per-M-cycle WAIT**: every memory M-cycle's T1 holds to the µs grid (falsified the M1-only roundup model against cc_op); I/O cycles instead free-run T1 and stretch on the real `cpu.wait` (released only in slot 1) | 2026-07-03 |
| — | Private point-to-point buses for board-internal wiring (AyBus PPI⇆PSG) rather than overloading the CPU bus | 2026-07-01 |
| — | **TapeBus** (`Bus.tape`): the three cassette wires — `rdata` (deck drives, POST-SCHMITT level: the mainboard's conditioning circuit squares the analog signal, so a CDT decoder and a live microphone source are interchangeable drivers), `motor` (PPI PC4 drives the relay), `wdata` (PPI PC5, reserved for the save path). See tape-device.md / ppi-device.md | 2026-07-03 |
| — | Caller-attached live wiring (framebuffer, key matrix, expansion RAM, ROM table, DSK media): pointers persist across reset/load, never serialized | 2026-07-03 |
| — | Observable-state gating: undocumented block-I/O flag corrections applied only at interrupt acceptance (FUSE pins the uninterrupted path) | 2026-07-03 |
