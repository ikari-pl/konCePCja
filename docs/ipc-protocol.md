...
| `step over [N]` | Execute N instructions, skipping over calls. A `CALL` is skipped with an ephemeral breakpoint at `pc+len`; an `RST` is stepped into and its frame finished instead, because the CPC firmware restarts (`&08`/`&10`/`&18`/`&28`) carry inline operands and do not resume at `pc+1`. Timeout: 5s. A breakpoint/watchpoint hit inside the skipped callee returns `OK breakpoint-hit`. |
| `step out` | Finish the current stack frame: run until a `RET`/`RETI`/`RETN` (or a `JP (HL)`-style computed return) retires **and** leaves SP above the entry SP. Correct from any point in the function — a `POP` that merely raises SP does not end it. `CALL` callees are skipped at full speed via an ephemeral breakpoint; `RST` vectors are stepped through. Timeout: 5s. A breakpoint/watchpoint inside a skipped callee returns `OK breakpoint-hit` with the usual debug context. `ERR 409 no-progress` when the frame can never return (top-level code, a `JP $` spin, no machine attached) or the instruction budget is exhausted. |
| `step to <addr>` | Run until PC reaches addr (ephemeral breakpoint). Timeout: 5s. If a different breakpoint or a watchpoint fires on the way, returns `OK breakpoint-hit` instead of reporting a successful run-to-cursor. |
