...
#ifdef __APPLE__
        // On macOS SDL_CaptureMouse() is a no-op (Cocoa_CaptureMouse just returns
        // true), so SDL_EVENT_MOUSE_MOTION stops the instant the cursor leaves the
        // window. During an ImGui window drag/resize a button is held, so the
        // idle-only fallback below (MouseButtonsDown == 0) never fires and the
        // operation freezes the moment a fast drag outruns the pointer out of
        // bounds. Allow the global-mouse-position fallback during drags too so the
        // drag keeps following the cursor off-window. Still gated on
        // hovered_window == nullptr, so it only engages once the cursor has
        // actually left every window; motion events resume the normal path the
        // instant it re-enters one.
        const bool allow_global_while_dragging = true;
#else
        const bool allow_global_while_dragging = false;
#endif
        if (hovered_window == nullptr && bd->MouseCanUseGlobalState && !is_relative_mouse_mode &&
            (bd->MouseButtonsDown == 0 || allow_global_while_dragging))
        {
