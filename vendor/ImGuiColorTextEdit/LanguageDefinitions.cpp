// Vendored dependency: ImGuiColorTextEdit — see TextEditor.h for upstream
// URL and license (SPDX-License-Identifier: MIT). Relocated to vendor/
// verbatim. The Z80Assembly() language definition at the bottom of this
// file is konCePCja's own downstream addition, following the upstream
// pattern used by the other language definitions in this file.
...
// ---------- Z80 Assembly tokenizer helpers ----------
...
static bool TokenizeZ80Number(const char* in_begin, const char* in_end, const char*& out_begin, const char*& out_end)
{
	...
	// &hex (Amstrad convention)
	if (*p == '&' && p + 1 < in_end && IsHexDigit(p[1]))
	{
		p++;
		while (p < in_end && IsHexDigit(*p)) p++;
		out_begin = in_begin;
		out_end = p;
		return true;
	}
	...
	// $ as standalone current-PC: only if not followed by hex digit or alpha
	if (*p == '$')
	{
		if (p + 1 < in_end && IsHexDigit(p[1]))
		{
			// $hex
			p++;
			while (p < in_end && IsHexDigit(*p)) p++;
		...
		// standalone $
		if (p + 1 >= in_end || (!isalnum(static_cast<unsigned char>(p[1])) && p[1] != '_'))
		{
			out_begin = in_begin;
			out_end = p + 1;
			return true;
	...
	// %binary
	if (*p == '%' && p + 1 < in_end && IsBinDigit(p[1]))
	{
		p++;
		while (p < in_end && IsBinDigit(*p)) p++;
		out_begin = in_begin;
		out_end = p;
		return true;
	}
	...
	// 0xhex
	if (*p == '0' && p + 1 < in_end && (p[1] == 'x' || p[1] == 'X'))
	{
		p += 2;
		if (p < in_end && IsHexDigit(*p))
		{
			while (p < in_end && IsHexDigit(*p)) p++;
			out_begin = in_begin;
			out_end = p;
	...
	// Decimal or hex-with-h-suffix: starts with digit
	if (*p >= '0' && *p <= '9')
	{
		// Could be decimal, or could be FFh-style hex
		const char* start = p;
		while (p < in_end && IsHexDigit(*p))
			p++;
		// Check for 'h' or 'H' suffix
		if (p < in_end && (*p == 'h' || *p == 'H'))
		{
			p++;
			out_begin = in_begin;
		...
		// Otherwise pure decimal (only digits 0-9)
		// Reset and scan only digits
		p = start;
...
static bool TokenizeZ80CharLiteral(const char* in_begin, const char* in_end, const char*& out_begin, const char*& out_end)
{
	const char* p = in_begin;
	if (*p == '\'')
	{
		p++;
		if (p < in_end && *p == '\\') p++; // escape
		if (p < in_end) p++; // the character
		if (p < in_end && *p == '\'')
		{
			out_begin = in_begin;
			out_end = p + 1;
...
{
	const char* p = in_begin;
	// Z80 labels can start with alpha, underscore, or dot (local labels)
	if ((*p >= 'a' && *p <= 'z') || (*p >= 'A' && *p <= 'Z') || *p == '_' || *p == '.')
	{
		...
		while (p < in_end && ((*p >= 'a' && *p <= 'z') || (*p >= 'A' && *p <= 'Z') ||
		       (*p >= '0' && *p <= '9') || *p == '_' || *p == '\''))
			p++;
		// Skip trailing colon (label marker) — include it in the token
		if (p < in_end && *p == ':') p++;
		out_begin = in_begin;
		out_end = p;
		return true;
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::Z80Assembly()
{
	static bool inited = false;
	static LanguageDefinition langDef;
	if (!inited)
	{
		// Z80 mnemonics (stored uppercase — engine uppercases tokens before lookup when !mCaseSensitive)
		static const char* const keywords[] = {
			"LD", "ADD", "ADC", "SUB", "SBC", "AND", "OR", "XOR", "CP", "INC", "DEC",
			"JP", "JR", "CALL", "RET", "PUSH", "POP", "NOP", "HALT", "DI", "EI",
			"RST", "BIT", "SET", "RES", "RL", "RR", "RLC", "RRC", "SLA", "SRA", "SRL", "SLL",
			"RLD", "RRD", "EX", "EXX",
			"LDI", "LDIR", "LDD", "LDDR", "CPI", "CPIR", "CPD", "CPDR",
			"IN", "OUT", "INI", "INIR", "IND", "INDR", "OUTI", "OTIR", "OUTD", "OTDR",
			"DAA", "CPL", "NEG", "CCF", "SCF", "DJNZ", "IM", "RETI", "RETN"
		};
		for (auto& k : keywords)
			langDef.mKeywords.insert(k);

		// Registers and condition flags as known identifiers
		static const char* const registers[] = {
			"A", "B", "C", "D", "E", "H", "L", "F", "I", "R",
			"AF", "BC", "DE", "HL", "SP", "IX", "IY", "IXH", "IXL", "IYH", "IYL",
			"AF'",
			// Condition codes
			"NZ", "Z", "NC", "PO", "PE", "P", "M"
		};
		...
		{
			Identifier id;
			id.mDeclaration = "Register/Condition";
			langDef.mIdentifiers.insert(std::make_pair(std::string(r), id));
		}

		// Directives as preprocessor identifiers
		static const char* const directives[] = {
			"ORG", "EQU", "DEFL", "DEFB", "DB", "DEFW", "DW", "DEFS", "DS", "END"
		};
		for (auto& d : directives)
		{
			Identifier id;
			id.mDeclaration = "Assembler directive";
		...
		langDef.mTokenize = [](const char* in_begin, const char* in_end, const char*& out_begin, const char*& out_end, PaletteIndex& paletteIndex) -> bool
		{
			...
			else if (TokenizeZ80CharLiteral(in_begin, in_end, out_begin, out_end))
				paletteIndex = PaletteIndex::CharLiteral;
			else if (TokenizeZ80Number(in_begin, in_end, out_begin, out_end))
				paletteIndex = PaletteIndex::Number;
			else if (TokenizeZ80Identifier(in_begin, in_end, out_begin, out_end))
				paletteIndex = PaletteIndex::Identifier;
			else if (TokenizeZ80Punctuation(in_begin, in_end, out_begin, out_end))
				paletteIndex = PaletteIndex::Punctuation;
