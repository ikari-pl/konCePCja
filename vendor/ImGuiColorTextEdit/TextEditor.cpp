...
void TextEditor::SetCustomLanguageDefinition(const LanguageDefinition* aDefinition)
{
	mLanguageDefinitionId = LanguageDefinitionId::None;
	mLanguageDefinition = aDefinition;
	mRegexList = std::make_shared<RegexList>();
	if (aDefinition)
	{
		for (const auto& r : aDefinition->mTokenRegexStrings)
			mRegexList->mValue.push_back(
