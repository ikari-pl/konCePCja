...
# Single source of truth for the binary version string.  Managed by
# release-please (simple strategy updates version.txt; bare semver, no "v").
# Mirror the same value in CMakeLists.txt's project(... VERSION) — that one
# drives CPack / installer metadata.  See chore: keep-version-in-sync if it
# ever drifts.
KONCPC_VERSION := $(shell cat version.txt 2>/dev/null)
